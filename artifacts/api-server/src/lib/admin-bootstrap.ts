  ...
  try {
    ...
    if (existing) {
      if (existing.role !== "admin" || !existing.emailVerified) {
        ...
        logger.info({ email: rawEmail }, "Promoted user to admin via ADMIN_EMAIL");
      }
      ...
      if (newPwd && newPwd.length >= 6) {
        ...
        await db
          ...
          .set({ passwordHash })
          .where(eq(usersTable.id, existing.id));
        logger.info({ email: rawEmail }, "Reset admin password via ADMIN_PASSWORD");
      }
      return;
    }
    ...
    logger.info({ email: rawEmail }, "Created admin account via ADMIN_EMAIL/ADMIN_PASSWORD");
  } catch (err) {
