...
router.post("/admin/certificates/issue", requireAdmin, async (req, res, next) => {
  try {
    ...
    if (existing) {
      res
        .status(409)
        .json({ error: "Active certificate already exists for this user/course/tier" });
      return;
    }
...
router.get("/admin/certificates", requireAdmin, async (req, res, next) => {
  try {
    ...
    if (status === "revoked") {
      conds.push(sql`${certificatesTable.revokedAt} IS NOT NULL` as ReturnType<typeof eq>);
    }
    if (search) {
      const like = `%${search}%`;
      conds.push(
  ...
  async (req, res, next) => {
    try {
      ...
      const [updated] = await db
        ...
        .where(
          and(eq(certificatesTable.id, id), isNull(certificatesTable.revokedAt)),
        )
        .returning();
      if (!updated) {
        res.status(404).json({ error: "Certificate not found or already revoked" });
        return;
      }
