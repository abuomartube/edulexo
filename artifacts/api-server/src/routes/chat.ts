...
async function loadAuthorsByIds(ids: string[]): Promise<Map<string, AuthorInfo>> {
...
const TOPICS: Record<string, { en: string; ar: string }[]> = {
  travel: [
    { en: "Describe a place you visited recently.", ar: "صف مكاناً زرته مؤخراً." },
    { en: "What's your dream travel destination?", ar: "ما هي وجهة سفر أحلامك؟" },
    { en: "Tell us about a memorable trip.", ar: "أخبرنا عن رحلة لا تُنسى." },
    { en: "Beach vacation or mountain adventure?", ar: "إجازة شاطئ أم مغامرة جبلية؟" },
    { en: "What do you usually pack for a trip?", ar: "ماذا تحزم عادة للسفر؟" },
  ],
  work: [
    { en: "Describe your dream job.", ar: "صف وظيفة أحلامك." },
    { en: "Do you prefer working in a team or alone?", ar: "هل تفضل العمل في فريق أم منفرداً؟" },
    { en: "What skill would you like to learn next?", ar: "ما المهارة التي تود تعلمها لاحقاً؟" },
    { en: "Talk about a challenge you overcame at work.", ar: "تحدث عن تحدٍ تغلبت عليه في العمل." },
    { en: "Office or remote work — which is better?", ar: "المكتب أم العمل عن بُعد — أيهما أفضل؟" },
  ],
  daily: [
    { en: "Describe your perfect morning routine.", ar: "صف روتين صباحك المثالي." },
    { en: "What's your favorite meal of the day?", ar: "ما وجبتك المفضلة في اليوم؟" },
    { en: "How do you usually spend your weekend?", ar: "كيف تقضي عطلة نهاية الأسبوع عادة؟" },
    { en: "Talk about your favorite hobby.", ar: "تحدث عن هوايتك المفضلة." },
    { en: "Describe your hometown.", ar: "صف مدينتك." },
  ],
  ielts: [
    { en: "Describe a person who has influenced you.", ar: "صف شخصاً أثّر فيك." },
    { en: "Talk about a book that changed your perspective.", ar: "تحدث عن كتاب غيّر منظورك." },
    { en: "Describe a piece of technology you can't live without.", ar: "صف قطعة تقنية لا تستطيع العيش بدونها." },
    { en: "Talk about an important decision you made.", ar: "تحدث عن قرار مهم اتخذته." },
    { en: "Describe a goal you want to achieve in 5 years.", ar: "صف هدفاً تريد تحقيقه خلال 5 سنوات." },
  ],
  study: [
    { en: "What's the best way to learn English?", ar: "ما أفضل طريقة لتعلم الإنجليزية؟" },
    { en: "Describe your favorite teacher.", ar: "صف معلمك المفضل." },
    { en: "Online learning vs. classroom — your view?", ar: "التعلم الإلكتروني مقابل الصف — رأيك؟" },
    { en: "What subject would you like to teach?", ar: "ما المادة التي تود تدريسها؟" },
    { en: "Tips for memorizing new vocabulary.", ar: "نصائح لحفظ مفردات جديدة." },
  ],
};

const ICE_BREAKERS: { en: string; ar: string }[] = [
  { en: "If you could have dinner with anyone, who would it be?", ar: "إذا كان بإمكانك تناول العشاء مع أي شخص، فمن سيكون؟" },
  { en: "What's the last movie that made you laugh?", ar: "ما آخر فيلم أضحكك؟" },
  { en: "What's your favorite English word and why?", ar: "ما كلمتك الإنجليزية المفضلة ولماذا؟" },
  { en: "Tea or coffee?", ar: "شاي أم قهوة؟" },
  { en: "What's one thing on your bucket list?", ar: "ما الشيء الموجود في قائمة أمنياتك؟" },
  { en: "Cats or dogs?", ar: "قطط أم كلاب؟" },
  { en: "What song are you listening to lately?", ar: "ما الأغنية التي تسمعها مؤخراً؟" },
...
router.get("/chat/leaderboard", requireAuth, async (req, res, next) => {
  try {
    ...
    const me = myRow
      ...
      : { totalXp: 0, level: 1, messagesSent: 0, voiceNotesSent: 0, imagesSent: 0 };
...
router.get("/chat/dm/threads", requireAuth, async (req, res, next) => {
  try {
    ...
    const previews: { threadId: string; body: string | null; kind: string; createdAt: Date }[] = [];
...
router.post("/chat/dm/threads", requireAuth, async (req, res, next) => {
  try {
    ...
    if (existing) {
      res.json({ thread: { id: existing.id, otherUserId, otherUserName: other.name } });
      return;
    }
...
router.get(
  ...
  async (req, res, next) => {
    try {
      ...
      res.json({
        messages,
        otherUser: { id: otherUserId, name: authors.get(otherUserId)?.name ?? "User" },
      });
    } catch (err) {
      next(err);
    }
  ...
  async (req, res, next) => {
    try {
      ...
      await db
        ...
        .values({ roomId: room.id, userId: targetId, mutedUntil, banned: false })
        .onConflictDoUpdate({
          target: [chatRoomMembershipTable.roomId, chatRoomMembershipTable.userId],
