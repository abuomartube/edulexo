...
const warmIpTextCounts = new Map<string, { count: number; windowStart: number }>();

function rateLimitOk(ip: string, max: number, store: Map<string, number[]>): boolean {
  const now = Date.now();
...
function warmTextBudgetOk(ip: string, want: number): { ok: boolean; allowed: number } {
  const now = Date.now();
...
async function generateAndCache(text: string, voice: AllowedVoice): Promise<Buffer> {
