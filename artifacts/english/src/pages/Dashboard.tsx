...
import { Loader2, KeyRound, Sparkles, GraduationCap, Rocket } from "lucide-react";
import {
  ENGLISH_TIERS,
  bestTier,
  isTierUnlocked,
    ...
    >
      ...
      <div className="relative mx-auto max-w-6xl space-y-8">
        ...
        {top && (
          <button
            onClick={onGoToTools}
            className="group w-full relative overflow-hidden rounded-2xl p-5 text-start transition-all hover:scale-[1.01] active:scale-[0.99]"
            style={{
              background: "linear-gradient(135deg, rgba(124,58,237,0.25) 0%, rgba(99,102,241,0.15) 100%)",
              border: "1px solid rgba(124,58,237,0.35)",
              boxShadow: "0 8px 32px -8px rgba(124,58,237,0.4)",
            }}
          >
            <div className="pointer-events-none absolute -top-10 -end-10 w-40 h-40 rounded-full opacity-30 blur-3xl" style={{ background: "#7c3aed" }} />
            <div className="relative flex items-center gap-4">
              <div className="flex items-center justify-center w-12 h-12 rounded-xl bg-gradient-to-br from-purple-500 to-indigo-600 shadow-lg ring-1 ring-white/20">
                <Rocket className="h-6 w-6 text-white" />
              </div>
              <div className="flex-1 min-w-0">
                <h2 className="text-lg font-bold text-white">{t("hub_title")}</h2>
                <p className="text-xs text-white/50">{t("hub_subtitle")}</p>
              </div>
              <div className="text-white/40 group-hover:text-white/70 transition">
                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d={lang === "ar" ? "M15 19l-7-7 7-7" : "M9 5l7 7-7 7"} />
                </svg>
              </div>
            </div>
          </button>
