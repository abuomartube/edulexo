// AUTO-GENERATED canonical Orwell assignment catalog. Mirrors artifacts/flashcards/src/data/orwell-assignments.ts.
// Used server-side to validate assignmentId and to use the canonical prompt for AI grading
// (so clients cannot fabricate prompts or IDs).

export type Category = "task1" | "task2" | "paragraph";

export interface OrwellCatalogEntry {
  id: string;
  category: Category;
  prompt: string;
}

export const ORWELL_CATALOG: ReadonlyArray<OrwellCatalogEntry> = [
  {
    id: "task1-1",
    category: "task1",
    prompt:
...
const BY_ID = new Map<string, OrwellCatalogEntry>(
  ORWELL_CATALOG.map((a) => [a.id, a]),
);

export function getOrwellEntry(id: string): OrwellCatalogEntry | undefined {
  return BY_ID.get(id);
}
