...
export const STUDENT_DAILY_CALL_THRESHOLD = Number(
  process.env["STUDENT_DAILY_CALL_THRESHOLD"] || 20,
);
export const DAILY_COST_ALERT_USD = Number(
  process.env["DAILY_COST_ALERT_USD"] || 10,
);
...
async function checkAndAlert(email: string): Promise<void> {
  ...
  if (studentCount > STUDENT_DAILY_CALL_THRESHOLD) {
    ...
    if (!(await alertAlreadySent(key))) {
      ...
      logger.info(
        { email, studentCount, recipient: getAdminAlertEmail() },
        "Student daily call threshold alert raised",
      );
    }
  }
  ...
  const [costRow] = await db
    .select({
      total: sql<number>`coalesce(sum(${aiUsageEventsTable.costUsd}), 0)::float8`,
    })
    .from(aiUsageEventsTable)
  ...
  if (totalCost > DAILY_COST_ALERT_USD) {
    ...
    if (!(await alertAlreadySent(key))) {
      ...
      await sendAdminAlert({ subject, text, html });
      logger.info(
        { totalCost, recipient: getAdminAlertEmail() },
        "Daily AI cost threshold alert raised",
      );
    }
  }
}
