  ...
  for (const q of test.questions) {
    ...
    if (q.type === "mcq") {
      ...
      displayStudentAnswer =
        Number.isFinite(sn) && sn >= 0 && sn < q.options.length
          ? q.options[sn]
          : null;
      ...
      if (
        ...
        studentAnswer.length === q.answers.length
      ) {
        ...
        displayStudentAnswer = studentAnswer.map((s) => {
          const sn = typeof s === "number" ? s : Number(s);
          return Number.isFinite(sn) && sn >= 0 && sn < q.options.length
            ? q.options[sn]
            : null;
        });
      } else {
      ...
      isCorrect = isCompletionMatch(
        typeof studentAnswer === "string" ? studentAnswer : "",
        q.answer,
        q.acceptable,
      );
      displayStudentAnswer =
        typeof studentAnswer === "string" ? studentAnswer : null;
    }
...
export function buildAnalysisFallback(
  ...
  const lines: string[] = [];
  lines.push(
    `You scored ${grade.score} out of ${grade.total} (${grade.percent}%) on "${test.title}".`,
  );
  if (strengths.length)
    lines.push(`Strengths: you did well on ${strengths.join(", ")} questions.`);
  if (weaknesses.length)
    lines.push(
      `Focus area: practise more ${weaknesses.join(", ")} questions — these were the hardest for you in this test.`,
    );
  if (!strengths.length && !weaknesses.length) {
    lines.push(
      `Your performance was even across the question types. Keep practising to push your accuracy higher.`,
    );
  }
  lines.push(
    `Tip: when listening, try to predict the answer before you hear it — note keywords from the question first.`,
  );
  return lines.join(" ");
}
