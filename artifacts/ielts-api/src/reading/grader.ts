...
function readableStudentAnswer(
  item: ReadingItem,
  qid: string,
  raw: unknown,
): unknown {
  ...
  switch (item.type) {
    ...
    case "tfng": {
      ...
      if (v === "ng" || v === "not given" || v === "notgiven")
        return "NOT GIVEN";
      return raw;
    }
    case "ynng": {
...
export function gradeItem(
  ...
  for (const sub of item.questions) {
    ...
    switch (item.type) {
      ...
      case "matching_features": {
        const target =
          typeof key.value === "number" ? key.value : Number(key.value);
      ...
      case "scanning": {
        ...
        if (sub.mcqOptions && sub.mcqOptions.length > 0) {
          ...
          isCorrect = isStringMatch(
            typeof studentRaw === "string" ? studentRaw : "",
            key,
          );
        }
        break;
