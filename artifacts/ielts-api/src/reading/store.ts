...
async function upsertMissingSeedItems(): Promise<void> {
  try {
    ...
    logger.info({ count: missing.length }, "[reading] inserted missing seed items");
  } catch (err) {
...
async function migrateLegacyScanningItems(): Promise<void> {
  try {
    ...
    for (const row of rows) {
      ...
      if (rowFirstVal !== undefined && typeof rowFirstVal === typeof seedFirstVal) continue;
      await db
        .update(introReadingItems)
        .set({ instructions: seed.instructions, questions: seed.questions, answerKey: seed.answerKey, updatedAt: new Date() })
        .where(eq(introReadingItems.slug, row.slug));
      migrated += 1;
    }
    if (migrated > 0) {
      logger.info({ migrated }, "[reading] re-synced scanning items to current bank shape");
    }
  } catch (err) {
...
function rowToRow(r: typeof introReadingItems.$inferSelect): ReadingItemRow {
  return { ...rowToItem(r), id: r.id, createdAt: r.createdAt, updatedAt: r.updatedAt };
}
...
export async function loadAllItems(): Promise<ReadingItem[]> {
  ...
  const rows = await db
    .select()
    .from(introReadingItems)
    .orderBy(asc(introReadingItems.level), asc(introReadingItems.type), asc(introReadingItems.sortOrder), asc(introReadingItems.id));
  return rows.map(rowToItem);
}

export async function loadItemsByLevelAndType(level: ReadingLevel, type: ReadingType): Promise<ReadingItem[]> {
  await initReadingItems();
  const rows = await db
    .select()
    .from(introReadingItems)
    .where(and(eq(introReadingItems.level, level), eq(introReadingItems.type, type)))
...
export async function loadItemBySlug(slug: string): Promise<ReadingItem | null> {
  await initReadingItems();
  const [r] = await db.select().from(introReadingItems).where(eq(introReadingItems.slug, slug));
...
export async function createItem(input: ReadingItemUpsertInput): Promise<ReadingItemRow> {
  ...
  if (sortOrder === undefined) {
    const [{ maxOrder }] = await db
      .select({ maxOrder: sql<number | null>`COALESCE(MAX(${introReadingItems.sortOrder}), -1)` })
      .from(introReadingItems)
      .where(and(eq(introReadingItems.level, input.level), eq(introReadingItems.type, input.type)));
...
export async function deleteItem(slug: string): Promise<boolean> {
  await initReadingItems();
  const result = await db.delete(introReadingItems).where(eq(introReadingItems.slug, slug)).returning();
