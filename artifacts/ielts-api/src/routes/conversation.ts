...
router.post("/conversation/chat", async (req, res): Promise<void> => {
  ...
  if (!(await verifyChurchillAccess(studentEmail))) {
    res.status(403).json({
      error:
        "Free Conversation is available for Intro and Comprehensive plan students only.",
    });
    return;
  }
  ...
  const ctx: ChatMessage[] =
    safeMessages.length > 0
      ...
      : [
          {
            role: "user",
            content: isStart ? "(start the conversation now)" : "(say hi)",
          },
        ];
  ...
  try {
    ...
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: FEEDBACK_PROMPT(topic || "general conversation"),
        },
...
router.post("/conversation/sessions", async (req, res): Promise<void> => {
  ...
  try {
    const [conv] = await db
      ...
      .returning({
        id: introConversations.id,
        createdAt: introConversations.createdAt,
      });
