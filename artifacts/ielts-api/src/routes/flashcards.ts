  ...
  const stats = await Promise.all(
    levels.map(async (level) => {
      ...
      const knownCardIds = await db
        ...
        .orderBy(
          progressTable.flashcardId,
          sql`${progressTable.reviewedAt} desc`,
        );
      ...
      const knownCount = knownForLevel.filter((p) =>
        levelCardIds.has(p.flashcardId),
      ).length;
...
router.post("/progress", async (req, res): Promise<void> => {
  ...
  const [record] = await db
    ...
    .values({ flashcardId, known, reviewedAt: new Date(), email })
    .returning();
...
router.get("/progress/summary", async (req, res): Promise<void> => {
  ...
  const knownIds = new Set(
    latestProgress.filter((p) => p.known).map((p) => p.flashcardId),
  );
  ...
  const byLevel = await Promise.all(
    levels.map(async (level) => {
      ...
      const levelCards = await db
        ...
        .where(eq(flashcardsTable.level, level));
      const levelCardIds = new Set(levelCards.map((c) => c.id));
      const knownInLevel = [...knownIds].filter((id) =>
        levelCardIds.has(id),
      ).length;
...
router.get("/bookmarks", async (req, res): Promise<void> => {
  ...
  const rows = await db
    .select({ flashcardId: bookmarksTable.flashcardId })
    .from(bookmarksTable)
    .where(eq(bookmarksTable.email, email));
...
router.post("/bookmarks/:id", async (req, res): Promise<void> => {
  ...
  const existing = await db
    ...
    .from(bookmarksTable)
    .where(
      and(eq(bookmarksTable.flashcardId, id), eq(bookmarksTable.email, email)),
    );
  if (existing.length > 0) {
    await db
      ...
      .where(
        and(
          eq(bookmarksTable.flashcardId, id),
          eq(bookmarksTable.email, email),
        ),
      );
...
router.get("/xp", async (req, res): Promise<void> => {
  const email = verifyStudentEmail(req);
  if (!email) {
    res.json({ total: 0, todayXp: 0, level: 1, levelName: "Starter" });
    return;
  }
...
router.get("/weak-words", async (req, res): Promise<void> => {
  ...
  const rows = await db
    ...
    .innerJoin(
      flashcardsTable,
      eq(weakWordsTable.flashcardId, flashcardsTable.id),
    )
...
router.post("/weak-words/add", async (req, res): Promise<void> => {
  ...
  for (const fid of flashcardIds) {
    const existing = await db
      ...
      .where(
        and(
          eq(weakWordsTable.email, email),
          eq(weakWordsTable.flashcardId, fid),
        ),
      )
      .limit(1);
    ...
    if (existing.length > 0) {
      await db
        .update(weakWordsTable)
        .set({
          wrongCount: sql`${weakWordsTable.wrongCount} + 1`,
...
router.post("/weak-words/add-by-word", async (req, res): Promise<void> => {
  ...
  const existing = await db
    .select({ id: weakWordsTable.id })
    .from(weakWordsTable)
    .where(
      and(
        eq(weakWordsTable.email, email),
        eq(weakWordsTable.flashcardId, flashcard.id),
      ),
    )
    .limit(1);
  ...
  if (existing.length > 0) {
    ...
    await db
      .insert(weakWordsTable)
      .values({ email, flashcardId: flashcard.id });
  }
  res.json({ added: 1 });
});
  ...
  const [row] = await db
    ...
    .where(
      and(
        ...
        eq(userDataTable.key, "weak_phrasal_verbs"),
      ),
    )
    .limit(1);
  const ids = row?.value
    ? row.value.split(",").map(Number).filter(Boolean)
    : [];
  res.json({ ids });
});
...
router.post("/phrasal-verbs/weak/toggle", async (req, res): Promise<void> => {
  ...
  const ids = new Set<number>(
    row?.value ? row.value.split(",").map(Number).filter(Boolean) : [],
  );
  const wasWeak = ids.has(id);
  if (wasWeak) ids.delete(id);
  else ids.add(id);
...
router.get("/quiz", async (req, res): Promise<void> => {
  ...
  const count = Math.min(
    parseInt((req.query.count as string) ?? "10", 10),
    100,
  );
  ...
  const questions = shuffled.map((card) => {
    const distractors = allCards
      ...
      .slice(0, 3)
      .map((c) => c.arabic);
    const options = [...distractors, card.arabic].sort(
      () => Math.random() - 0.5,
    );
...
router.get("/fill-blank", async (req, res): Promise<void> => {
  ...
  const withSentences = pool.filter(
    (c) =>
      c.exampleSentence &&
      c.exampleSentence.toLowerCase().includes(c.english.toLowerCase()),
  );
  const sample = [...withSentences]
    .sort(() => Math.random() - 0.5)
    .slice(0, count);
...
router.get("/srs/due", async (req, res): Promise<void> => {
  ...
  const srsRecords = await db
    ...
    .from(cardSrsTable)
    .where(
      and(eq(cardSrsTable.email, email), lte(cardSrsTable.nextReviewAt, now)),
    );
  ...
  const allSrsIds = new Set(
    (
      await db
        .select({ id: cardSrsTable.flashcardId })
        .from(cardSrsTable)
        .where(eq(cardSrsTable.email, email))
    ).map((r) => r.id),
  );
...
router.post("/srs/:id", async (req, res): Promise<void> => {
  ...
  const [existing] = await db
    ...
    .from(cardSrsTable)
    .where(
      and(eq(cardSrsTable.flashcardId, id), eq(cardSrsTable.email, email)),
    );
  ...
  if (!existing) {
    ...
    if (known) {
      ...
      if (reviewCount === 1) intervalDays = 1;
        ...
        intervalDays = Math.round(existing.intervalDays * existing.easeFactor);
  ...
  if (!existing) {
    await db.insert(cardSrsTable).values({
      flashcardId: id,
      nextReviewAt,
      intervalDays,
      easeFactor,
...
router.get("/activity-position/:activity", async (req, res): Promise<void> => {
  const email = verifyStudentEmail(req);
  if (!email) {
    res.json({ position: 0, filters: "{}" });
    return;
  }
  const activity = req.params.activity;
  const [row] = await db
    .select()
    .from(activityPositionTable)
    .where(
      and(
        eq(activityPositionTable.email, email),
        eq(activityPositionTable.activity, activity),
      ),
    )
    .limit(1);
...
router.put("/activity-position/:activity", async (req, res): Promise<void> => {
  ...
  const filters =
    typeof rawFilters === "string" && rawFilters.length <= 500
      ? rawFilters
      : "{}";
  await db
    .insert(activityPositionTable)
...
router.post("/quiz-scores", async (req, res): Promise<void> => {
  ...
  if (!mode || !level || typeof total !== "number" || total < 0) {
  ...
  if (!validModes.includes(mode) || !validLevels.includes(level)) {
    res.status(400).json({ error: "Invalid mode or level" });
    return;
  }
...
router.get("/user-data-prefix/:prefix", async (req, res): Promise<void> => {
  ...
  const rows = await db
    ...
    .where(
      and(
        eq(userDataTable.email, email),
        ilike(userDataTable.key, `${prefix}%`),
      ),
    );
...
router.get("/user-data/:key", async (req, res): Promise<void> => {
  ...
  const [row] = await db
    ...
    .where(
      and(
        eq(userDataTable.email, email),
        eq(userDataTable.key, req.params.key),
      ),
    )
    .limit(1);
...
router.put("/user-data/:key", async (req, res): Promise<void> => {
  ...
  if (key === "avatar" && value !== "") {
    if (!/^data:image\/(png|jpeg|jpg|webp|gif);base64,/.test(value)) {
      res
        .status(400)
        .json({ error: "avatar must be a base64-encoded image data URL" });
      return;
    }
    if (value.length > limit) {
      res.status(413).json({
        error: "Avatar image is too large — please use a smaller photo",
      });
      return;
    }
  } else if (key === "name") {
    ...
    if (value && !/^[A-Za-z][A-Za-z\s\-'.]{0,99}$/.test(value)) {
      res.status(400).json({ error: "Name must use English letters only" });
      return;
    }
...
router.delete("/progress/reset", async (req, res): Promise<void> => {
  ...
  await db
    .delete(activityPositionTable)
    .where(eq(activityPositionTable.email, email));
  ...
  await db
    ...
    .where(and(eq(userDataTable.email, email), ne(userDataTable.key, "tier")));
  res.json({ success: true });
});
