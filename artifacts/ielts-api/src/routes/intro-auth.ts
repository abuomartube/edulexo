...
router.post("/auth/intro/register", async (req, res): Promise<void> => {
  ...
  if (existing.length > 0) {
    ...
    if (s.status === "denied") {
      res.status(403).json({
        error:
          "Your registration has been denied. Please contact the administrator.",
      });
    } else {
      res.status(409).json({
  ...
  try {
    await db.transaction(async (tx) => {
      const consumed = await tx
        ...
        .where(
          and(
            eq(introAccessCodes.code, codeClean),
            isNull(introAccessCodes.usedAt),
          ),
        )
        .returning();
...
router.post("/auth/intro/login", async (req, res): Promise<void> => {
  ...
  if (student.status === "denied") {
    res.status(403).json({
      status: "denied",
      error: "Your account has been denied. Please contact the administrator.",
    });
    return;
  }
...
router.post(
  "/admin/intro/students/:id/approve",
...
router.post(
  "/admin/intro/students/:id/reject",
  async (req, res): Promise<void> => {
    if (!(await requireAdmin(req, res))) return;
    const id = Number(req.params.id);
    const [updated] = await db
...
router.post(
  "/admin/intro/students/:id/set-expiry",
  async (req, res): Promise<void> => {
    if (!(await requireAdmin(req, res))) return;
    const id = Number(req.params.id);
    const { expiresAt } = req.body ?? {};
...
router.delete(
  ...
  async (req, res): Promise<void> => {
    ...
    if (existing.usedAt) {
      res
        .status(400)
        .json({ error: "Cannot delete a code that has already been used" });
      return;
    }
