...
import {
  ...
  type ListeningTest,
  type Segment,
} from "../listening/testBank";
import {
  gradeTest,
  buildAnalysisFallback,
  type GradeResult,
} from "../listening/grader";
import {
  buildAudioManifest,
...
async function requireStudentAccess(
  ...
  if (tier === "advance") {
    res.status(403).json({
      error:
        "Listening tests are available in the Intro and Comprehensive plans only.",
    });
    return null;
  }
...
router.get("/listening/tests", async (req, res) => {
  ...
  try {
    ...
    const completedMap = new Map<
      string,
      { score: number; total: number; percent: number; createdAt: Date }
    >();
    for (const c of completed) {
      completedMap.set(c.testId, {
        score: c.score,
        total: c.total,
        percent: c.percent,
  ...
  try {
    if (studentId) {
      const [existing] = await db
        ...
        .where(
          and(
            eq(introListeningAttempts.studentId, studentId),
            eq(introListeningAttempts.testId, test.id),
          ),
        );
      if (existing) {
        res.json({
...
router.post("/listening/tests/:testId/submit", async (req, res) => {
  ...
  try {
    ...
    if (existing) {
      res
        .status(409)
        .json({ error: "Test already completed", attempt: existing });
      return;
    }
    ...
    const analysis = await generateAnalysis(test, grade).catch(() =>
      buildAnalysisFallback(test, grade),
    );
    ...
    try {
      ...
      if (isPgUniqueViolation(err)) {
        ...
        if (again) {
          res
            .status(409)
            .json({ error: "Test already completed", attempt: again });
          return;
        }
      }
      throw err;
  ...
  try {
    const [existing] = await db
      ...
      .where(
        and(
          eq(introListeningAttempts.studentId, studentId),
          eq(introListeningAttempts.testId, req.params.testId),
        ),
      );
    if (!existing) {
    ...
    messages: [
      {
        role: "system",
        content:
          "You are a kind A2 English listening tutor giving short, simple feedback.",
      },
...
listeningAdminRouter.post(
  "/listening/admin/preview-segment",
  async (req, res) => {
    if (!(await requireAdmin(req, res))) return;
...
function parseTestPayload(
  ...
  if (![1, 2, 3, 4].includes(sectionId))
    throw new Error("sectionId must be 1, 2, 3 or 4");
  ...
  const transcript: Segment[] = b.transcript.map((s, i) => {
    if (!s || typeof s !== "object")
      throw new Error(`Segment ${i + 1}: invalid`);
    const seg = s as Record<string, unknown>;
    const voice = typeof seg.voice === "string" ? seg.voice : "";
    const text = typeof seg.text === "string" ? seg.text.trim() : "";
    if (!VOICES.has(voice))
      throw new Error(`Segment ${i + 1}: voice must be 'alloy' or 'nova'`);
  ...
  if (
    typeof answerKeyInput !== "object" ||
    answerKeyInput === null ||
    Array.isArray(answerKeyInput)
  ) {
  ...
  for (let i = 0; i < b.questions.length; i++) {
    ...
    if (type === "mcq") {
      const options = Array.isArray(q.options)
        ...
        : [];
      if (options.length < 2)
        throw new Error(`Question ${id}: MCQ needs at least 2 options`);
      ...
      const options = Array.isArray(q.options)
        ...
        : [];
      if (items.length === 0)
        throw new Error(`Question ${id}: matching needs at least 1 item`);
      if (options.length === 0)
        throw new Error(`Question ${id}: matching needs at least 1 option`);
      const answers = Array.isArray(akEntry.answers)
        ? akEntry.answers.map((x) => Number(x))
        : [];
      if (answers.length !== items.length) {
        throw new Error(
          `Question ${id}: matching needs ${items.length} answers (one per item)`,
        );
      }
      ...
      const context =
        typeof q.context === "string" && q.context.trim()
          ...
          : undefined;
      const answer =
        typeof akEntry.answer === "string" ? akEntry.answer.trim() : "";
...
listeningAdminRouter.post("/listening/admin/tests", async (req, res) => {
  ...
  try {
    ...
    res
      .status(400)
      .json({ error: err instanceof Error ? err.message : "Invalid payload" });
    return;
  }
  try {
...
listeningAdminRouter.put("/listening/admin/tests/:slug", async (req, res) => {
  ...
  let parsed: ParsedTestPayload;
  try {
    parsed = parseTestPayload(
      { ...(req.body as object), slug: req.params.slug },
      { requireSlug: false },
    );
  } catch (err) {
    res
  ...
  try {
    ...
    try {
      ...
      req.log.warn(
        ...
        "Failed to load previous transcript for orphan cleanup",
      );
    }
...
listeningAdminRouter.post("/listening/admin/tests/move", async (req, res) => {
  ...
  if (!Array.isArray(body.targetSlugs) || !Array.isArray(body.sourceSlugs)) {
    res
      .status(400)
      .json({ error: "targetSlugs and sourceSlugs must be arrays" });
    return;
  }
  const parseSlugList = (raw: unknown[], label: string): string[] | null => {
    ...
    for (const s of raw) {
      if (typeof s !== "string" || !s.trim()) {
        res
          .status(400)
          .json({ error: `${label} must contain non-empty strings` });
        return null;
      }
      out.push(s.trim());
    }
  ...
  try {
    const result = await moveTestToSection(
      slug,
      targetSectionId,
      targetSlugs,
      sourceSlugs,
...
listeningAdminRouter.delete(
  ...
  async (req, res) => {
    ...
    try {
      ...
      try {
        const deletedHashes = collectReferencedHashes([
        ...
        const toDelete = Array.from(deletedHashes).filter(
          (h) => !stillReferenced.has(h),
        );
...
listeningAdminRouter.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      req.log.info(
        { scanned: summary.scanned, deleted: summary.deleted },
        `[listening] cleanup-audio: scanned ${summary.scanned}, deleted ${summary.deleted}`,
      );
...
listeningStorageRouter.get(
  "/listening/storage/public-objects/*filePath",
  async (req, res) => {
    try {
