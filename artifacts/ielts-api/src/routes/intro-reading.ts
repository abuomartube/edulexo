...
async function requireStudentAccess(
  ...
  if (tier === "advance") {
    res.status(403).json({
      error:
        "Reading tests are available in the Intro and Comprehensive plans only.",
    });
    return null;
  }
...
async function requireAdmin(req: Request, res: Response): Promise<boolean> {
  ...
  try {
    ...
    if (
      ...
      !require("node:crypto").timingSafeEqual(pwBuf, expBuf)
    ) {
...
router.get("/reading/levels-types", async (req, res) => {
  ...
  try {
    ...
    for (const it of items)
      counts[`${it.level}:${it.type}`] =
        (counts[`${it.level}:${it.type}`] ?? 0) + 1;
    res.json({
      levels: [
        {
          id: "a2",
          label: "A2 — Elementary",
          description: "Short, simple texts with literal answers.",
        },
        {
          id: "b1",
          label: "B1 — Intermediate",
          description: "Longer texts with paraphrasing and synonyms.",
        },
      ],
      types: READING_TYPES,
      counts,
  ...
  try {
    const [existing] = await db
      ...
      .where(
        and(
          eq(introReadingAttempts.studentId, studentId),
          eq(introReadingAttempts.itemSlug, item.slug),
        ),
      );
    if (existing) {
      res.json({
...
router.post("/reading/items/:slug/submit", async (req, res) => {
  ...
  try {
    ...
    if (existing) {
      res
        .status(409)
        .json({ error: "Item already completed", attempt: existing });
      return;
    }
    ...
    try {
      ...
      if (isPgUniqueViolation(err)) {
        ...
        if (again) {
          res
            .status(409)
            .json({ error: "Item already completed", attempt: again });
          return;
        }
      }
      throw err;
...
function parseParagraphs(v: unknown): { label: string; text: string }[] {
  ...
  return v.map((p, i) => {
    if (!p || typeof p !== "object")
      throw new Error(`Paragraph ${i + 1}: invalid`);
...
function parseAnswerEntry(
  ...
  if (!raw || typeof raw !== "object")
    throw new Error(`Question ${qid}: missing answer key entry`);
  const ak = raw as Record<string, unknown>;
  const explanation = asString(ak.explanation).trim();
  if (!explanation)
    throw new Error(`Question ${qid}: answer explanation is required`);
  ...
  if (type === "mcq" || type === "skimming") {
    ...
    if (!Number.isInteger(idx) || idx < 0 || idx >= mcqOptionsLen) {
      throw new Error(
        `Question ${qid}: answer index out of range (0..${mcqOptionsLen - 1})`,
      );
    }
    value = idx;
    ...
    if (mcqOptionsLen > 0) {
      ...
      if (ak.acceptable !== undefined) {
        acceptable = parseStringArray(
          ak.acceptable,
          `Question ${qid}: acceptable`,
        ).filter(Boolean);
      }
    }
    ...
    if (!Number.isInteger(idx) || idx < 0 || idx >= optionsLen) {
      throw new Error(
        `Question ${qid}: answer index out of range (0..${optionsLen - 1})`,
      );
    }
    value = idx;
  } else if (type === "tfng") {
    ...
    if (!TFNG_VALUES.has(v))
      throw new Error(
        `Question ${qid}: answer must be one of true / false / ng`,
      );
    value = v;
  } else if (type === "ynng") {
    const v = asString(ak.value).toLowerCase().trim();
...
function parseItemPayload(
  ...
  if (type === "matching_headings" && paragraphs.length === 0) {
    throw new Error(
      "Matching headings questions require at least one paragraph",
    );
  }

  const optionsRaw =
    b.options === undefined || b.options === null ? [] : b.options;
  ...
  for (let i = 0; i < b.questions.length; i++) {
    ...
    if (type === "mcq" || type === "skimming") {
      const opts2 = parseStringArray(
        raw.mcqOptions,
        `Question ${id}: mcqOptions`,
      ).filter(Boolean);
      if (opts2.length < 2)
        throw new Error(`Question ${id}: needs at least 2 options`);
      if (type === "skimming" && opts2.length !== 4)
        throw new Error(`Question ${id}: skimming requires exactly 4 options`);
      mcqOptions = opts2;
    } else if (type === "scanning") {
      const hasOptionsField =
        raw.mcqOptions !== undefined && raw.mcqOptions !== null;
      if (hasOptionsField) {
        const opts2 = parseStringArray(
          raw.mcqOptions,
          `Question ${id}: mcqOptions`,
        ...
        if (opts2.length > 0) {
          if (opts2.length !== 4)
            throw new Error(
              `Question ${id}: scanning MCQ questions require exactly 4 options`,
            );
          mcqOptions = opts2;
        }
      }
  ...
  const persistedOptions =
    type === "matching_headings" || type === "matching_features" ? options : [];
...
readingAdminRouter.get("/reading/admin/analytics", async (req, res) => {
  ...
  try {
    ...
    const byItem = new Map<
      string,
      { level: string; type: string; attempts: number; totalPct: number }
    >();
    for (const r of rows) {
      const existing = byItem.get(r.itemSlug) ?? {
        level: r.level,
        type: r.type,
        attempts: 0,
        totalPct: 0,
