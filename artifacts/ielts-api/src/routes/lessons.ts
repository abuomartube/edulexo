...
function toEmbedUrl(input: string): string {
  ...
  const idMatch = t.match(
    /(?:vimeo\.com\/(?:video\/|channels\/[^/]+\/|groups\/[^/]+\/videos\/)?)(\d{6,12})(?:\/([a-zA-Z0-9]+))?/,
  );
  if (idMatch) {
    const id = idMatch[1];
...
router.post("/lessons/:id/complete", async (req, res): Promise<void> => {
  ...
  if (!Number.isFinite(lessonId)) {
    res.status(400).json({ error: "Bad lesson id" });
    return;
  }
...
router.delete("/lessons/:id/complete", async (req, res): Promise<void> => {
  ...
  await db
    ...
    .where(
      and(
        eq(lessonCompletionsTable.email, email),
...
router.post("/admin/lessons", async (req, res): Promise<void> => {
  ...
  if (!vimeoUrl || typeof vimeoUrl !== "string" || !isValidVimeoUrl(vimeoUrl)) {
    res
      .status(400)
      .json({ error: "A valid Vimeo URL or numeric video ID is required" });
    return;
  }
  ...
  const [maxRow] = await db
    .select({
      max: sql<number>`coalesce(max(${lessonsTable.orderIndex}), 0)::int`,
    })
    .from(lessonsTable)
...
router.patch("/admin/lessons/:id", async (req, res): Promise<void> => {
  ...
  if (typeof vimeoUrl === "string" && isValidVimeoUrl(vimeoUrl))
    updates.vimeoUrl = vimeoUrl.trim();
...
router.get(
  "/admin/lesson-completion-counts",
