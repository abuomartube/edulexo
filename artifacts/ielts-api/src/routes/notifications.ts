...
import { db, pushSubscriptionsTable, xpEventsTable } from "@workspace/ielts-db";
...
async function sendDailyReminders() {
  ...
  try {
    ...
    const todayXp = await db
      .select({ email: xpEventsTable.email })
      .from(xpEventsTable)
    ...
    logger.info(
      { sent, skipped, total: allSubs.length },
      "Daily push reminders completed",
    );
  } catch (err) {
...
export function startReminderScheduler() {
  ...
  logger.info(
    "Push notification reminder scheduler started (daily at 20:00 UTC)",
  );
}
