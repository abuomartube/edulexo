...
import {
  db,
  orwellSubmissionsTable,
  orwellCoachSummariesTable,
  xpEventsTable,
  accessRequestsTable,
...
async function isStudentApproved(email: string): Promise<boolean> {
  ...
  const rows = await db
    .select({
      ...
      expiresAt: accessRequestsTable.expiresAt,
    })
  ...
  const cacheUntil = row.expiresAt
    ? Math.min(ttlExpiry, row.expiresAt.getTime())
    : ttlExpiry;
...
router.get("/orwell/next", async (req, res): Promise<void> => {
  ...
  const rows = await db
    .select({
      id: orwellSubmissionsTable.assignmentId,
  ...
  const skippedIds = new Set(
    rows.filter((r) => r.status === "skipped").map((r) => r.id),
  );
...
router.post("/orwell/skip", async (req, res): Promise<void> => {
  ...
  const { assignmentId, category } = req.body as {
    assignmentId: string;
    category: string;
  };
  if (!assignmentId || !CATEGORIES.has(category)) {
  ...
  await db
    ...
    .onConflictDoNothing({
      target: [
        orwellSubmissionsTable.email,
        orwellSubmissionsTable.assignmentId,
      ],
    });
...
function freeCheckTryAcquire(
  email: string,
): { ok: true } | { ok: false; reason: string; retryAfterMs?: number } {
  const now = Date.now();
  const recent = (freeCheckRecent.get(email) ?? []).filter(
    (t) => now - t < FREECHECK_RATE_WINDOW_MS,
  );
  if (recent.length >= FREECHECK_RATE_MAX) {
    ...
    return {
      ok: false,
      reason: "Free Check is rate-limited. Please wait a moment and try again.",
      retryAfterMs,
    };
  }
  ...
  if (active >= FREECHECK_MAX_CONCURRENT_PER_EMAIL) {
    return {
      ok: false,
      reason:
        "You already have a Free Check in progress. Please wait for it to finish.",
    };
  }
  recent.push(now);
  freeCheckRecent.set(email, recent);
...
async function streamFreeCheck(
  ...
  const stampLabel =
    mode === "task1" ? "Task 1" : mode === "task2" ? "Task 2" : "Paragraph";
...
function ocrTryAcquire(email: string): boolean {
  ...
  if (existing !== undefined && now - existing < OCR_LOCK_TIMEOUT_MS)
    return false;
  ocrInFlight.set(email, now);
  return true;
}
function ocrRelease(email: string) {
  ocrInFlight.delete(email);
}
...
router.post(
  ...
  async (req, res): Promise<void> => {
    ...
    if (!ocrTryAcquire(email)) {
      res.status(429).json({
        error: "Another upload is still being processed. Please wait.",
      });
      return;
    }
    ...
    try {
      ...
      if (isHeic) {
        try {
          ...
          res.status(400).json({
            error:
              "Could not read this HEIC photo. Try saving it as JPEG and uploading again.",
          });
          return;
        }
        ...
        messages: [
          ...
          },
          {
            ...
            content: [
              {
                type: "text",
                text: "Transcribe the handwritten essay in this photo exactly as written.",
              },
              {
                type: "image_url",
      ...
      if (!text || text === "NO_TEXT_FOUND") {
        res.status(422).json({
          error:
            "Couldn't read any text from this photo. Try a clearer, better-lit picture.",
        });
        return;
      }
      ...
      if (status === 401 || status === 402) {
        res
          .status(502)
          .json({ error: "Vision service is unavailable right now." });
        return;
      }
      console.error("OCR failed:", err);
      res.status(500).json({
        error: "Could not extract text from that photo. Please try again.",
      });
    } finally {
      ocrRelease(email);
    }
  ...
  (
    ...
    if (err && typeof err === "object" && "code" in err) {
      ...
      if (code === "LIMIT_FILE_SIZE") {
        res
          .status(413)
          .json({ error: "Image is too large. Maximum size is 10 MB." });
        return;
      }
    }
    if (err instanceof Error && err.message === "unsupported_format") {
      res.status(415).json({
        error:
          "Unsupported file format. Please upload JPG, JPEG, PNG, or HEIC.",
      });
      return;
    }
    next(err);
...
router.post("/orwell/submit", async (req, res): Promise<void> => {
  ...
  if (isFreeCheck) {
    ...
    const mode: FreeCheckMode = (allowedModes as string[]).includes(
      bodyMode ?? "",
    )
    ...
    if (!slot.ok) {
      if (slot.retryAfterMs)
        res.setHeader(
          "Retry-After",
          String(Math.ceil(slot.retryAfterMs / 1000)),
        );
  ...
  if (claim.length === 0) {
    ...
    if (existing?.status === "submitted") {
      res
        .status(409)
        .json({ error: "This assignment has already been submitted." });
    } else {
      res.status(409).json({
        error:
          "This assignment is currently being graded. Please wait a moment and try again.",
      });
    }
    return;
  }
  ...
  const wordCount = text.trim().split(/\s+/).filter(Boolean).length;
  const scores = (parsed as { scores?: { taskResponse?: { band?: number } } })
    .scores;
  const band =
    typeof scores?.taskResponse?.band === "number"
      ? scores.taskResponse.band
      : null;
  const overallBand =
    typeof (parsed as { overallBand?: number }).overallBand === "number"
  ...
  try {
    ...
    if (awarded > 0) {
      await db
        ...
        .values({ email, activity: "essay_check", xp: awarded });
    }
  } catch (err) {
...
async function fetchHistoryDetail(emailParam: string, id: number) {
  ...
  if (row.compareReport) {
    try {
      compareReport = JSON.parse(row.compareReport);
    } catch {
      compareReport = null;
    }
  }
...
async function buildCompareForId(
  emailParam: string,
  id: number,
  force: boolean,
) {
  ...
  if (!older.text)
    return {
      error: "This submission has no saved writing to compare.",
      status: 400,
    } as const;
  ...
  if (!newer || !newer.text) {
    return {
      error:
        "You haven't submitted a newer piece of this type yet — once you do, you'll be able to compare progress.",
      status: 409,
    } as const;
  }
  ...
  if (!force && older.compareReport) {
    try {
      ...
      if (
        ...
        (cached as { _newerId?: number })._newerId === newer.id
      ) {
  ...
  try {
    const report = await generateCompareReport({
      older: {
        ...
        taskTypeLabel: older.taskTypeLabel,
      },
...
async function buildCoachSummary(emailParam: string, force: boolean) {
  ...
  const items: CoachInputItem[] = recent.map((r) => {
    ...
    if (r.feedback) {
      try {
        ...
        const strengths = Array.isArray(f.strengths)
          ? f.strengths.join(" • ")
          : typeof f.strengths === "string"
            ? f.strengths
            : "";
        const improvements = Array.isArray(f.improvements)
          ? f.improvements.join(" • ")
          : typeof f.improvements === "string"
            ? f.improvements
            : "";
        feedbackExcerpt =
          `Strengths: ${strengths}\nImprovements: ${improvements}`.slice(
            0,
            500,
          );
      } catch {
  ...
  try {
    ...
    await db
      ...
      .values({
        email: emailParam,
        atSubmissionCount: milestone,
        summary: JSON.stringify(summary),
      })
      .onConflictDoUpdate({
        target: [
          orwellCoachSummariesTable.email,
          orwellCoachSummariesTable.atSubmissionCount,
        ],
    ...
    return {
      error: "AI summary failed. Please try again.",
      status: 500,
    } as const;
  }
}
...
router.get(
  "/admin/students/:email/orwell-history",
  async (req, res): Promise<void> => {
    if (!verifyAdmin(req)) {
      res.status(401).json({ error: "Unauthorized" });
      return;
...
router.get(
  "/admin/students/:email/orwell-history/:id",
  async (req, res): Promise<void> => {
    if (!verifyAdmin(req)) {
      res.status(401).json({ error: "Unauthorized" });
      return;
    ...
    const studentEmail = req.params.email.trim().toLowerCase();
    const id = Number(req.params.id);
    if (!studentEmail || !Number.isInteger(id) || id <= 0) {
...
router.get(
  "/admin/students/:email/orwell-coach-summary",
  async (req, res): Promise<void> => {
    if (!verifyAdmin(req)) {
      res.status(401).json({ error: "Unauthorized" });
      return;
