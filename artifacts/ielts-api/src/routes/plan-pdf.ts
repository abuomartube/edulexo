...
const LOGO_PATH = path.join(
  WORKSPACE_ROOT,
  "artifacts/flashcards/public/4ielts-logo.png",
);
...
router.post("/plan-pdf", async (req, res): Promise<void> => {
  ...
  if (
    !headerEmail ||
    !headerToken ||
    !safeEqual(headerToken, makeStudentToken(headerEmail))
  ) {
    res
      .status(401)
      .json({ error: "Please log in again to download your plan." });
    return;
  }
  ...
  for (const c of payload.coverage) {
    if (
      !isStr(c?.emoji, 8) ||
      !isStr(c?.label, MAX_STR) ||
      !isStr(c?.labelAr, MAX_STR) ||
      !isStr(c?.detail, MAX_STR) ||
      !isStr(c?.detailAr, MAX_STR) ||
      typeof c?.pct !== "number" ||
  ...
  for (const d of payload.days) {
    if (
      ...
      !Number.isFinite(d.dayIndex) ||
      !isStr(d?.weekday, MAX_STR) ||
      !isStr(d?.dateLabel, MAX_STR) ||
      !Array.isArray(d?.tasks) ||
      d.tasks.length > MAX_TASKS_PER_DAY
    ) {
    ...
    for (const t of d.tasks) {
      if (
        !isStr(t?.label, MAX_STR) ||
        !isStr(t?.labelAr, MAX_STR) ||
        !isStr(t?.emoji, 8) ||
  ...
  if (
    !isStr(payload.name, MAX_STR) ||
    !isStr(payload.levelLabel, MAX_STR) ||
    !isStr(payload.startISO, 64) ||
    !isStr(payload.startLabel, MAX_STR) ||
    !isStr(payload.endLabel, MAX_STR) ||
    !isStr(payload.generatedAt, MAX_STR) ||
  ...
  try {
    ...
    if (!res.headersSent) {
      res
        .status(500)
        .json({ error: "PDF generation failed. Please try again." });
    }
  } finally {
    if (browser) {
      try {
