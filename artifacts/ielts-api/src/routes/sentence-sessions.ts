...
function sanitizeItem(raw: unknown): SentenceItem | null {
  ...
  const attempts = Math.max(
    1,
    Math.min(20, Math.floor(Number(r["attempts"]) || 1)),
  );
  ...
  return {
    ...
    isCorrect,
    errorHighlight:
      r["errorHighlight"] == null ? null : clampStr(r["errorHighlight"], 500),
...
function summarize(items: SentenceItem[]) {
  ...
  const avgVocabBand = total
    ? items.reduce((s, i) => s + (i.vocabBand || 0), 0) / total
    : 0;
  const avgGrammarBand = total
    ? items.reduce((s, i) => s + (i.grammarBand || 0), 0) / total
    : 0;
  ...
  const KEYS = [
    ...
    {
      key: "missing the target word",
      re: /\bdid(n['’]?| not) use\b|\bmissing\b.*\bword\b/i,
    },
...
router.get("/sentence-sessions/:id", async (req, res) => {
  ...
  const [row] = await db
    ...
    .where(
      and(
        eq(sentenceSessionsTable.id, id),
        eq(sentenceSessionsTable.email, email),
      ),
    );
  if (!row) {
