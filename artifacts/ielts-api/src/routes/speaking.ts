...
function extractAndValidateReport(
  text: string,
...
router.post("/speaking/report", async (req, res) => {
  ...
  try {
    ...
    const transcript = trimmed
      .map(
        (m) =>
          `${m.role === "assistant" ? "EXAMINER" : "STUDENT"}: ${m.content}`,
      )
      .join("\n\n");
...
OUTPUT FORMAT — respond with ONE JSON object only. No prose, no markdown, no code fences. The JSON must match this exact shape:
{
  ...
  "improvements": ["improvement area 1", "improvement area 2", "improvement area 3"]
    ...
    if (!report) {
      console.error(
        "[speaking/report] Could not extract a valid report. Raw output (first 600 chars):",
        raw.slice(0, 600),
      );
      res.status(500).json({
        error: "invalid_report",
        message:
          "Churchill AI returned an unexpected response. Please try again.",
      });
      return;
    }
    ...
    try {
      ...
      if (status === 429) {
        res.status(402).json({
          ...
          message:
            "OpenAI account has no billing credits. Please add a payment method at platform.openai.com.",
        });
      } else if (status === 401) {
        res.status(401).json({
          error: "invalid_key",
...
router.post("/speaking/tts", async (req, res) => {
  try {
    ...
    const rawSpeed =
      typeof (req.body as { speed?: unknown }).speed === "number"
    ...
    const voice: Voice = ALLOWED_VOICES.includes(rawVoice as Voice)
      ? (rawVoice as Voice)
      : "onyx";
