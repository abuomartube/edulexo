...
async function getTodayXpTotal(
  email: string,
  activity: string,
): Promise<number> {
...
async function awardXp(
  ...
  if (awarded > 0) {
    db.insert(xpEventsTable)
      ...
      .catch((err) =>
        logger.warn({ err, email, activity, awarded }, "XP insert failed"),
      );
  }
  return awarded;
}
...
async function loadOrGenerateQuiz(storyId: number): Promise<QuizQuestion[]> {
  ...
  if (cached) {
    try {
      ...
      try {
        ...
        logger.warn(
          { err: delErr, storyId },
          "Failed to clear bad cached quiz",
        );
      }
    }
  }
  ...
  return await generateQuizForStory(
    storyId,
    story.title,
    story.content,
    story.level,
  ...
  try {
    ...
    if (xpAwarded > 0) {
      ...
      db.insert(xpEventsTable)
        ...
        .catch((err) =>
          logger.warn({ err, email, xpAwarded }, "story_quiz XP insert failed"),
        );
    }
    res.json({
...
router.post("/stories/:id/written-feedback", async (req, res) => {
  ...
  if (responseText.length < 20) {
    res
      .status(400)
      .json({ error: "Please write at least a couple of sentences." });
    return;
  }
  if (responseText.length > 2000) {
    res
      .status(400)
      .json({ error: "Response is too long (max 2000 characters)." });
    return;
  }
  ...
  try {
    ...
    const xpAwarded = Math.max(
      0,
      Math.min(25, STORY_WRITING_DAILY_CAP - todaySoFar),
    );
    if (xpAwarded > 0) {
      // Fire-and-forget so the response isn't blocked on the INSERT round-trip.
      db.insert(xpEventsTable)
        ...
        .catch((err) =>
          logger.warn(
            { err, email, xpAwarded },
            "story_writing XP insert failed",
          ),
        );
    }
    ...
    res
      .status(500)
      .json({ error: "Could not analyse your response. Please try again." });
  }
});
