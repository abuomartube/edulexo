...
router.post("/whisper", uploadAudio, async (req, res): Promise<void> => {
  ...
  if (!studentEmail) {
    res
      .status(401)
      .json({ error: "Authentication required. Please log in and try again." });
    return;
  }
  ...
  if (tier === "advance") {
    res.status(403).json({
      error:
        "Speech recognition is not included in the Advance plan. Upgrade to the Comprehensive plan to access Churchill voice features.",
    });
    return;
  }
  ...
  if (!req.file) {
    res.status(400).json({
      error:
        "Missing audio field. Send a multipart/form-data request with an 'audio' file field.",
    });
    return;
  }
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Speech-to-text service is not configured." });
    return;
  }
  ...
  try {
    ...
    if (status === 429) {
      ...
      res
        .status(500)
        .json({ error: "Speech-to-text service is misconfigured." });
    } else {
      req.log.error(
        { err, email: studentEmail },
