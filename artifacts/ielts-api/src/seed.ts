...
import { db, storiesTable, flashcardsTable } from "@workspace/ielts-db";
...
async function reconcileFlashcardTranslations() {
  ...
  const byKey = new Map<
    string,
    { id: number; arabic: string; exampleAr: string | null }
  >();
  ...
  for (const seed of rows) {
    ...
    if (cur.arabic !== newArabic) {
      await db.execute(
        sql`UPDATE flashcards SET arabic = ${newArabic} WHERE id = ${cur.id}`,
      );
      arabicUpdates++;
    }
    if (cur.exampleAr !== newExample) {
      await db.execute(
        sql`UPDATE flashcards SET example_sentence_arabic = ${newExample} WHERE id = ${cur.id}`,
      );
      exampleUpdates++;
    }
  }
...
async function seedStoriesData() {
  ...
  const result = await db.execute(
    sql`SELECT COUNT(*)::int AS cnt FROM stories`,
  );
  ...
  if (count > 0) {
    ...
    logger.info(
      { old: count, new: expected },
      "Reseeding stories with full set",
    );
  }
...
export async function runSeed() {
  try {
    ...
    const result = await db.execute(
      sql`SELECT COUNT(*)::int AS cnt FROM flashcards`,
    );
    const count = (result.rows[0] as { cnt: number }).cnt;
    if (count < EXPECTED_COUNT) {
      logger.info(
        { existing: count, expected: EXPECTED_COUNT },
        "Seeding flashcards database...",
      );
      if (count > 0) {
      ...
      logger.info(
        { count: (seedData as SeedRow[]).length },
    ...
    const a1Result = await db.execute(
      sql`SELECT COUNT(*)::int AS cnt FROM flashcards WHERE level = 'A1'`,
    );
    const a1Count = (a1Result.rows[0] as { cnt: number }).cnt;
    if (a1Count > 0) {
      await db.execute(
        sql`UPDATE flashcards SET level = 'A2' WHERE level = 'A1'`,
      );
