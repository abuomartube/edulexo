...
test.describe("Landing page (unauthenticated)", () => {
  test("shows Log In and Register buttons", async ({ page }) => {
    ...
    await expect(page.getByRole("button", { name: /Log In/i }).first()).toBeVisible({ timeout: 8000 });
  ...
  test("clicking Log In reveals email + password fields", async ({ page }) => {
    ...
    await expect(page.getByPlaceholder(/email/i).first()).toBeVisible({ timeout: 6000 });
    await expect(page.getByPlaceholder(/password/i).first()).toBeVisible();
  });

  test("clicking Register reveals email, password and access-code fields", async ({ page }) => {
    await page.goto(appUrl("/"));
    await page.getByRole("button", { name: /Register/i }).first().click();
    await expect(page.getByPlaceholder(/email/i).first()).toBeVisible({ timeout: 6000 });
    await expect(page.getByPlaceholder(/7K9M2P4XAB|e\.g\./i).first()).toBeVisible();
  });
});
...
test.describe("Registration error handling", () => {
  test("invalid access code shows an error message", async ({ page }) => {
    ...
    await page.getByPlaceholder(/7K9M2P4XAB|e\.g\./i).first().fill("XXXX-XXXX-XXXX");
...
test.describe("Advance/complete session login (mocked)", () => {
  test("valid credentials store session token and unlock the app", async ({ page }) => {
    await page.route("**/api-ielts/access/login", (route) =>
      route.fulfill({
        status: 200,
        contentType: "application/json",
    ...
    await page.getByRole("button", { name: /Log In/i }).first().click();
    await expect(page.getByPlaceholder(/email/i).first()).toBeVisible({ timeout: 6_000 });
    ...
    await page.getByRole("button", { name: /sign in|log in/i }).last().click();
    await expect(page.getByRole("link", { name: /Study Mode/i }).first()).toBeVisible({ timeout: 12_000 });
  });
  ...
  test("complete-tier session login also unlocks the app", async ({ page }) => {
    ...
    await page.getByPlaceholder(/password/i).first().fill("TestPassword123!");
    await page.getByRole("button", { name: /sign in|log in/i }).last().click();
    await expect(page.getByRole("link", { name: /Churchill AI/i }).first()).toBeVisible({ timeout: 12_000 });
  });
});
...
test.describe("SSO-launch / token-bootstrap regression", () => {
  test("stored token triggers session/save + session/check and unlocks gate without login UI", async ({
    ...
    await page.addInitScript(() => {
      localStorage.setItem("4ielts_email", JSON.stringify({ email: "sso@4ielts.com", token: "sso-token-xyz" }));
      localStorage.setItem("lexo_tour_completed", "1");
      try { sessionStorage.setItem("exitCommentDismissed", "1"); } catch { /* ignore */ }
    });
...
test.describe("Complete intro-auth flow (mocked)", () => {
  test("register shows pending state, then approved login reaches home", async ({ page }) => {
    ...
    await page.getByPlaceholder(/7K9M2P4XAB|e\.g\./i).first().fill("VALID-CODE-0001");
    await page.getByRole("button", { name: /create account|register|sign up/i }).last().click();
    await expect(
  ...
  test("approved student logs in and reaches the app home screen", async ({ page }) => {
    ...
    await page.route("**/api-ielts/access/login", (route) =>
      route.fulfill({
        status: 200,
        contentType: "application/json",
        body: JSON.stringify({ status: "approved", token: "intro-approved-token" }),
      }),
    );
    await page.route("**/api-ielts/session/save", (route) =>
      route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ ok: true }) }),
    ...
    await page.route("**/api-ielts/auth/intro/me", (route) =>
      route.fulfill({
        status: 200,
        contentType: "application/json",
        body: JSON.stringify({ user: { id: 99, email: "approved@4ielts.com", status: "approved" } }),
      }),
    );
    await page.route("**/api-ielts/me/tier", (route) =>
      route.fulfill({
...
test.describe("Full intro-auth flow (live API)", () => {
  ...
  test("student can register and lands on pending screen", async ({ page, request }) => {
    const adminPwd = process.env["ADMIN_PASSWORD"] || "";
    test.skip(!adminPwd, "ADMIN_PASSWORD not set");
    ...
    await page.goto(appUrl("/"));
    await expect(page.getByText(/(pending|awaiting|review|approval)/i).first()).toBeVisible({ timeout: 10000 });
  });
  ...
  test("admin can approve the student", async ({ request }) => {
    ...
    const list = await request.get(`${ADMIN_URL}/students`, { headers: { "x-admin-password": adminPwd } });
    expect(list.ok()).toBeTruthy();
    const students = (await list.json()) as Array<{ id: number; email: string }>;
    const student = students.find((s) => s.email === testEmail);
    expect(student).toBeTruthy();

    const approve = await request.post(`${ADMIN_URL}/students/${student!.id}/approve`, {
      headers: { "x-admin-password": adminPwd },
      data: {},
    });
  ...
  test("approved student logs in and reaches home", async ({ page, request }) => {
    const adminPwd = process.env["ADMIN_PASSWORD"] || "";
    test.skip(!adminPwd, "ADMIN_PASSWORD not set");
    test.skip(!testEmail, "test email not set");
    ...
    await page.addInitScript(
      ({ em, tok }) => {
        ...
        localStorage.setItem("4ielts_email", JSON.stringify({ email: em, token: tok }));
    ...
    await page.goto(appUrl("/"));
    await expect(page.getByText(/Your Tools|Churchill|Listening Practice/i).first()).toBeVisible({ timeout: 12000 });
  });
});
