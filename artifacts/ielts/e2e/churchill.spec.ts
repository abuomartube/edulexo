  ...
  test("mic permission granted, VAD mounts, speech triggers Whisper, AI reply renders", async ({
    ...
    await page.route("**/api-ielts/whisper", async (route) => {
      ...
      await route.fulfill({
        status: 200,
        contentType: "application/json",
        body: JSON.stringify({ transcript: "I enjoy exploring new cultures and tasting local food." }),
      });
    });
    ...
    await page.evaluate(() => {
      const vad = (window as unknown as Record<string, { onSpeechEnd: (a: Float32Array) => void }>)["__fakeVad"];
