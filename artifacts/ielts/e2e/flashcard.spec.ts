...
async function setupFlashcardMocks(page: Page) {
  ...
  await page.route("**/api-ielts/flashcards/*", (route) => {
    ...
    if (url.includes("/categories") || url.includes("/levels"))
  ...
  await page.route("**/api-ielts/xp**", (route) =>
    route.fulfill({
      status: 200,
      contentType: "application/json",
      body: JSON.stringify({ xp: 0, streak: 0 }),
    }),
  );
...
test.describe("Flashcard Study — advance tier", () => {
  ...
  test("study page renders card deck with word and arabic translation", async ({
    page,
  }) => {
    ...
    await expect(page.getByText("abandon").first()).toBeVisible({
      timeout: 8_000,
    });
  });

  test("flip card reveals Arabic translation and example sentence", async ({
    page,
  }) => {
    await page.goto(appUrl("/study"));
  ...
  test('"Got it!" marks card as known and advances to next card', async ({
    page,
  }) => {
    ...
    page.on("request", (req) => {
      if (
        req.url().includes("/api-ielts/progress") &&
        req.method() === "POST"
      ) {
        try {
          progressPosts.push(JSON.parse(req.postData() ?? "{}"));
        } catch {
          /* ignore */
        }
      }
    ...
    await expect
      .poll(() => progressPosts.some((p) => p.known === true), {
        timeout: 5_000,
      })
      .toBe(true);
  ...
  test('"Still Learning" marks card as unknown and advances', async ({
    page,
  }) => {
    const progressPosts: Array<{ flashcardId: number; known: boolean }> = [];
    page.on("request", (req) => {
    ...
    const stillLearningBtn = page
      .getByRole("button", { name: /Still Learning/i })
      .last();
    ...
    await expect
      .poll(() => progressPosts.some((p) => p.known === false), {
        timeout: 5_000,
      })
      .toBe(true);
  });
...
test.describe("Flashcard Study — tier gating", () => {
  test("intro tier sees a locked/upgrade screen on /study", async ({
    page,
  }) => {
