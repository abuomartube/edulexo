...
const MOCK_SECTIONS = [
  {
    ...
    description: "Conversations about everyday activities.",
  },
  {
    id: 2,
...
async function setupListeningMocks(
...
test.describe("Listening — intro tier full flow", () => {
  test("section-list → test-list → player → answer both MCQs → submit → result", async ({
    ...
    await expect(page.getByText("A Morning Routine")).toBeVisible({
      timeout: 6_000,
    });
    ...
    await expect(page.getByText("A Morning Routine").first()).toBeVisible({
      timeout: 8_000,
    });

    // Both questions should render
    await expect(
      page.getByText("What time does the man wake up?"),
    ).toBeVisible();
    await expect(
      page.getByText("What does the woman drink in the morning?"),
    ).toBeVisible();
    ...
    await expect(
      page.getByText(/Excellent work|answered.*correctly/i),
    ).toBeVisible({
      timeout: 5_000,
    });
  });

  test("advance tier is blocked from Listening and sees upgrade CTA", async ({
    page,
  }) => {
    await loginAsAdvanceOrComplete(
      page,
      "advance@listening.invalid",
      "advance",
    );
    await page.goto(appUrl("/intro-listening"));
  ...
  test("complete tier CAN access Listening (all features included)", async ({
    page,
  }) => {
    await loginAsAdvanceOrComplete(
      page,
      "complete@listening.invalid",
      "complete",
    );
    await setupListeningMocks(page);
    await page.goto(appUrl("/intro-listening"));
...
test.describe("Listening — additional player interactions", () => {
  test("back button from test-list returns to section list", async ({
    page,
  }) => {
