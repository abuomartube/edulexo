...
const FREE_CHECK_RESULT = {
  ...
  scores: {
    taskResponse: {
      band: 7,
      feedback: "The response addresses the task clearly.",
    },
    coherenceCohesion: {
      band: 6,
      feedback: "Ideas are generally well-organised.",
    },
  ...
  improvements: [
    "Develop the conclusion further.",
...
const TASK1_RESULT = {
  ...
  wordCount: 18,
  wordCountWarning:
    "Your essay is too short — please write at least 150 words for Task 1.",
  scores: {
...
async function setupOrwellMocks(
...
test.describe("Orwell AI — advance tier", () => {
  ...
  test("intro screen renders Orwell AI branding and start button", async ({
    page,
  }) => {
    await page.goto(appUrl("/essay-checker"));

    // "Orwell AI" appears in the nav link and the h1 — target the heading
    await expect(page.getByRole("heading", { name: "Orwell AI" })).toBeVisible({
      timeout: 10_000,
    });
    await expect(
      page.getByText("Structured IELTS Writing Assignments"),
    ).toBeVisible();
  ...
  test("Free Check full flow: intro → select → Free Check → freewriting → submit → result", async ({
    ...
    // ── Select screen ────────────────────────────────────────────────────────
    await expect(page.getByText("Orwell AI Assignments")).toBeVisible({
      timeout: 6_000,
    });
    ...
    if (await task2Option.isVisible()) {
      ...
      await page
        .locator("button")
        .filter({ hasText: /Task|Paragraph/i })
        .first()
        .click();
    }
    ...
    const submitBtn = page
      .getByRole("button", { name: /Submit|Check|Send/i })
      .last();
    ...
    await expect(page.getByText(/peoples|people/i).first()).toBeVisible({
      timeout: 5_000,
    });
    // Strength from mock
    await expect(
      page.getByText(/Clear thesis|thesis statement/i).first(),
    ).toBeVisible({
      timeout: 5_000,
    });
  });

  test("Task 1 structured flow: select → writing → submit → result", async ({
    page,
  }) => {
    await page.goto(appUrl("/essay-checker"));
    ...
    await expect(page.getByText(/too short|at least 150/i).first()).toBeVisible(
      { timeout: 5_000 },
    );
  });
});
...
test.describe("Orwell AI — complete tier", () => {
  test("complete tier can access essay-checker and reach select screen", async ({
    page,
  }) => {
