...
const MOCK_LEVELS_TYPES = {
  levels: [
    { id: "a2", label: "A2", description: "Pre-intermediate — short factual passages." },
    { id: "b1", label: "B1", description: "Intermediate — descriptive and narrative texts." },
  ],
  types: [
    {
      id: "tfng",
...
const MOCK_ITEM_DETAIL = {
  item: {
    ...
    options: [],
    questions: [
      { id: "q1", prompt: "The Sahara Desert is the largest desert in the world." },
    ...
    results: [
      {
        ...
        correctAnswer: "true",
        isCorrect: true,
        explanation: "The passage explicitly states the Sahara is the largest hot desert.",
      },
      {
        id: "q2",
        ...
        correctAnswer: "false",
        isCorrect: true,
        explanation: "The passage says the fennec fox has adapted to survive there.",
      },
    ],
...
async function setupReadingMocks(page: Parameters<Parameters<typeof test>[1]>[0]["page"]) {
...
test.describe("Reading — intro tier full flow", () => {
  test("level-picker → A2 → TFNG type → player → answer both questions → submit → result", async ({
    ...
    // ── Type picker ──────────────────────────────────────────────────────────
    await expect(
      page.getByText("True / False / Not Given"),
    ...
    await page.getByRole("button", { name: /^(Start|Continue|Review)$/i }).first().click();

    // ── Reading player — passage + questions ─────────────────────────────────
    await expect(
      page.getByText("The Sahara Desert").first(),
    ).toBeVisible({ timeout: 8_000 });
    // Passage text
    await expect(
      page.getByText(/largest hot desert/i),
    ).toBeVisible({ timeout: 6_000 });
