...
test.describe("Advance tier — core page smoke tests", () => {
  ...
  test("/sentence-builder renders Sentence Builder heading", async ({
    page,
  }) => {
...
test.describe("Complete tier — all features render", () => {
  ...
  test("Churchill (/free-conversation) renders topic-source screen", async ({
    ...
    await expect(
      page
        .getByText(
          /What would you like to talk about|Choose a topic|Your own topic/i,
        )
        .first(),
    ).toBeVisible({ timeout: 10_000 });
  });
    ...
    await page.route("**/api-ielts/reading/levels-types", (route) =>
      route.fulfill({
        ...
        body: JSON.stringify({
          ...
          types: [
            { id: "tfng", label: "True / False / Not Given", tagline: "..." },
          ],
          counts: { tfng: 5 },
        }),
      }),
  ...
  test("/essay-checker (Orwell AI) renders for complete tier", async ({
    page,
  }) => {
    await page.goto(appUrl("/essay-checker"));
    await expect(page.getByText("Not Included in Your Plan")).not.toBeVisible({
...
test.describe("Intro tier — core features render", () => {
  ...
  test("Churchill (/free-conversation) renders topic-source screen for intro tier", async ({
    ...
    await expect(
      page
        .getByText(/What would you like to talk about|Choose a topic/i)
        .first(),
    ).toBeVisible({ timeout: 10_000 });
  });
  ...
  test("Listening (/intro-listening) renders Attenborough AI heading with sections", async ({
      ...
      route.fulfill({
        ...
        body: JSON.stringify({
          sections: [
            ...
            {
              id: 2,
              title: "Section 2 — Education",
              description: "Academic topics.",
            },
          ],
          tests: [],
        }),
    ...
    await expect(page.getByText("Attenborough AI")).toBeVisible({
      timeout: 10_000,
    });
  ...
  test("intro tier: direct deep-link to /intro-listening resolves correctly", async ({
    ...
    await page.route("**/api-ielts/listening/tests", (route) =>
      route.fulfill({
        ...
        body: JSON.stringify({
          sections: [
            { id: 1, title: "Section 1 — Daily Life", description: "D." },
          ],
          tests: [],
        }),
      }),
...
test.describe("Advance tier — Orwell AI intro → select transition", () => {
  test("clicking start button transitions intro → select screen", async ({
    page,
  }) => {
    await loginAsAdvanceOrComplete(
      page,
      "advance@orwellreg.invalid",
      "advance",
    );
