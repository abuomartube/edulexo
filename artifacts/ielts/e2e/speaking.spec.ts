...
const MOCK_REPORT = {
  overallBand: 6.5,
  fluencyCoherence: { band: 7, comment: "Good fluency with minor hesitations." },
  lexicalResource: { band: 6, comment: "Adequate vocabulary." },
  grammaticalRange: { band: 6, comment: "Some minor grammatical errors." },
  pronunciation: { band: 7, tips: ["Work on word stress.", "Practise intonation."] },
...
test.describe("Churchill AI (/speaking) — idle screen", () => {
  ...
  test("complete tier can also access /speaking", async ({ page }) => {
    await loginAsAdvanceOrComplete(page, "speaking-complete@test.invalid", "complete");
    await page.goto(appUrl("/speaking"));
    await expect(
      page.getByRole("heading", { name: /Churchill AI/i }).first(),
    ).toBeVisible({ timeout: 10_000 });
...
test.describe("Churchill AI (/speaking) — text mode session", () => {
  test("starting text session streams Part 1 question into chat", async ({ page }) => {
    ...
    await expect(
      page.getByText(/hometown|tell me about/i).first(),
    ).toBeVisible({ timeout: 15_000 });

    // Part indicator header shows "Part 1" as active
    await expect(page.getByText("Part 1").first()).toBeVisible({ timeout: 5_000 });
  ...
  test("answering Part 1 question sends user message and receives examiner feedback", async ({
    ...
    await expect(
      page.getByText(/Very good|described your hometown|clearly and naturally/i).first(),
    ).toBeVisible({ timeout: 15_000 });
  ...
  test("FinalReport renders band score when report data is injected via mocked API", async ({
     ...
     */
    await loginAsAdvanceOrComplete(page, "speaking-report@test.invalid", "advance");
    await setupSpeakingMocks(page);
