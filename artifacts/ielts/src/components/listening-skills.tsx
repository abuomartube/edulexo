...
export default function ListeningSkills({
  ...
  const [phase, setPhase] = useState<Phase>(
    initialSection ? "test-list" : "section-list",
  );
  const [section, setSection] = useState<LSection | null>(
    initialSection ?? null,
  );
  ...
  const openSection = (sec: LSection) => {
    setSection(sec);
    setPhase("test-list");
  };
  const openTest = (t: LSkillTest) => {
    setTest(t);
    setPhase("test");
  };
  const backToList = () => {
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="bg-card border border-border rounded-3xl p-8 text-center space-y-3">
          ...
          <p className="text-muted-foreground">
            Targeted practice for each of the 4 IELTS Listening sections.
          </p>
          <p className="text-sm text-muted-foreground" dir="rtl" lang="ar">
            تدرب على كل قسم من أقسام الاستماع الأربعة على حدة
          </p>
        </div>
            ...
            return (
              ...
              >
                <div className="flex items-start gap-4">
                  <div
                    className={`w-14 h-14 rounded-2xl bg-gradient-to-br ${s.gradient} flex items-center justify-center text-3xl flex-shrink-0`}
                  >
                    {s.emoji}
                  </div>
                  <div className="flex-1 min-w-0 space-y-1">
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <span
                        className={`text-xs font-semibold px-2 py-0.5 rounded-full ${stats.done > 0 ? "bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400" : "bg-muted text-muted-foreground"}`}
                      >
      ...
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        <div className="bg-card border border-border rounded-3xl p-6">
          <div className="flex items-center gap-4">
            <div
              className={`w-14 h-14 rounded-2xl bg-gradient-to-br ${sec.gradient} flex items-center justify-center text-3xl`}
            >
              {sec.emoji}
            </div>
            <div>
                ...
                <div className="flex items-center justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs uppercase tracking-wide text-purple-600 dark:text-purple-400 font-semibold">
                        {labelFor(t.qType)}
                      </span>
                    </div>
                    <h4 className="font-bold mt-1 group-hover:text-purple-600 transition-colors">
                      {t.title}
                    </h4>
                  ...
                  <div className="flex items-center gap-2">
                    {prog ? (
                      <span
                        className={`text-xs font-bold px-2 py-1 rounded-full ${prog.correct === prog.total ? "bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400" : "bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400"}`}
                      >
...
function TestRunner({
  ...
  test: LSkillTest;
  onCancel: () => void;
  onFinish: (c: number, total: number, perItem: boolean[]) => void;
}) {
  ...
  const allAnswered = userAnswers.every((a, i) => {
    ...
    if (Array.isArray(it.answer))
      return Array.isArray(a) && a.length === it.answer.length;
  ...
  const submit = () => {
    const perItem = test.items.map((it, i) =>
      scoreListeningItem(it, userAnswers[i]),
    );
    ...
    <div className="max-w-4xl mx-auto space-y-5">
      ...
      <div className="bg-gradient-to-br from-purple-500/10 via-card to-card border border-border rounded-3xl p-6">
        <div className="flex items-center gap-2 text-xs font-semibold text-purple-600 dark:text-purple-400 uppercase tracking-wide">
          <Headphones className="w-3.5 h-3.5" /> Section {test.section} ·{" "}
          {labelFor(test.qType)}
        </div>
      ...
      <div className="bg-card border border-border rounded-2xl p-6">
        ...
        {!played && (
          <p className="mt-3 text-xs text-muted-foreground italic">
            ▶ Press play above to hear the audio first.
          </p>
        )}
      </div>
      ...
      >
        {allAnswered
          ? "Submit answers"
          : `Answer all ${test.items.length} questions to submit`}
      </button>
    </div>
  );
}
  ...
  return (
    <div className="bg-gradient-to-br from-indigo-500/10 via-card to-card border border-border rounded-2xl p-5 space-y-3">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          {state === "idle" && (
            <button
              onClick={play}
              className="px-4 py-2 rounded-xl bg-indigo-600 text-white text-sm font-bold flex items-center gap-2 hover:bg-indigo-700 transition-colors"
            >
          ...
          {state === "loading" && (
            <button
              disabled
              className="px-4 py-2 rounded-xl bg-indigo-600/50 text-white text-sm font-bold flex items-center gap-2 cursor-wait"
            >
          ...
          {state === "playing" && (
            <>
              <button
                onClick={pause}
                className="px-3 py-2 rounded-xl bg-amber-500 text-white text-sm font-bold flex items-center gap-2 hover:bg-amber-600 transition-colors"
              >
                <Pause className="w-4 h-4" /> Pause
              </button>
              <button
                onClick={stop}
                className="px-3 py-2 rounded-xl bg-rose-500 text-white text-sm font-bold flex items-center gap-2 hover:bg-rose-600 transition-colors"
              >
          ...
          {state === "paused" && (
            <>
              <button
                onClick={resume}
                className="px-3 py-2 rounded-xl bg-emerald-600 text-white text-sm font-bold flex items-center gap-2 hover:bg-emerald-700 transition-colors"
              >
          ...
          {(state === "playing" || state === "paused") && (
            <button
              onClick={replay}
              className="px-3 py-2 rounded-xl bg-muted text-foreground text-sm font-bold flex items-center gap-2 hover:bg-muted/70 transition-colors"
            >
      ...
      <p className="text-xs text-muted-foreground italic">
        🎧 Audio uses natural AI voices (alloy &amp; nova). Use headphones for
        the best experience. In a real IELTS exam you only hear the audio once —
        but here you may replay it for practice.
      </p>
    </div>
  );
}
...
function ResultView({
  ...
  const tone =
    correct === total
      ? { bg: "from-emerald-500 to-teal-500", label: "Perfect!" }
      : pct >= 60
        ? { bg: "from-amber-500 to-orange-500", label: "Good work" }
        : { bg: "from-rose-500 to-pink-500", label: "Keep practising" };

  return (
    <div className="max-w-4xl mx-auto space-y-5">
      <div
        className={`rounded-3xl p-8 text-white text-center bg-gradient-to-br ${tone.bg}`}
      >
        ...
        <p className="text-6xl font-black mt-2">
          {correct}
          <span className="text-3xl opacity-75"> / {total}</span>
        </p>
      ...
      <div className="bg-card border border-border rounded-2xl p-5">
        <h3 className="font-bold mb-4 flex items-center gap-2">
          <Sparkles className="w-4 h-4 text-purple-500" /> Question-by-question
        </h3>
        <div className="space-y-2">
          {test.items.map((it, i) => {
            ...
            return (
              <div
                key={i}
                className={`rounded-xl p-3 border ${isCorrect ? "bg-emerald-50 dark:bg-emerald-900/10 border-emerald-200 dark:border-emerald-800" : "bg-rose-50 dark:bg-rose-900/10 border-rose-200 dark:border-rose-800"}`}
              >
                  ...
                  <div className="flex-1 min-w-0 text-sm">
                    ...
                    <p className="text-xs text-muted-foreground mt-0.5">
                      ...
                      {it.acceptable && it.acceptable.length > 0 && (
                        <span className="text-muted-foreground">
                          {" "}
                          (also accepted: {it.acceptable.join(", ")})
                        </span>
                      )}
                    </p>
                  </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        <button
          onClick={onRetry}
          className="px-4 py-3 rounded-2xl bg-card border border-border font-semibold text-sm flex items-center justify-center gap-2 hover:bg-muted transition-colors"
        >
          <RotateCcw className="w-4 h-4" /> Try again
        </button>
        <button
          onClick={onMore}
          className="px-4 py-3 rounded-2xl bg-card border border-border font-semibold text-sm hover:bg-muted transition-colors"
        >
          More tests
        </button>
        <button
          onClick={onSecs}
          className="px-4 py-3 rounded-2xl bg-gradient-to-r from-purple-600 to-pink-600 text-white font-bold text-sm hover:shadow-lg transition-all"
        >
          All sections
        </button>
      </div>
