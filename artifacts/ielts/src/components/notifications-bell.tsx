...
export function NotificationsBell({
  ...
  const fetchUnread = useCallback(async () => {
    try {
      const res = await customFetch<{ unread: number }>(
        "/api-ielts/notifications/unread-count",
      );
  ...
  const fetchList = useCallback(async () => {
    setLoading(true);
    try {
      const res = await customFetch<{ notifications: StudentNotification[] }>(
        "/api-ielts/notifications",
      );
  ...
  async function markRead(n: StudentNotification) {
    ...
    try {
      await customFetch(`/api-ielts/notifications/${n.id}/read`, {
        method: "POST",
      });
    } catch {
      // Revert on failure.
      setItems((cur) =>
        cur.map((it) => (it.id === n.id ? { ...it, read_at: null } : it)),
      );
      fetchUnread();
    }
  }
            ...
            <div className="overflow-y-auto flex-1">
              {loading && items.length === 0 ? (
                ...
                <ul className="divide-y divide-border">
                  {items.map((n) => {
                    const meta =
                      TYPE_META[n.type] ?? TYPE_META["announcement"]!;
