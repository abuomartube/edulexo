...
async function postJson<T = AuthResponse>(
  path: string,
  body: unknown,
  ...
  try {
    ...
    return { error: "Connection error. Please try again." } as T & {
      error?: string;
    };
  }
}
...
const landingFeatures = [
  ...
  {
    icon: Sparkles,
    text: "Interactive flashcards + word pronunciation",
    color: "text-sky-400",
  },
  { icon: Brain, text: "Smart quizzes", color: "text-violet-400" },
  {
    icon: Headphones,
    text: "🎧 Listening and Reading Practice",
    color: "text-blue-400",
  },
  {
    icon: Mic,
    text: "Churchill AI (Speaking) + Topic Bank",
    color: "text-rose-400",
  },
  {
    icon: PenTool,
    text: "Orwell AI (Writing) + Templates Library",
...
function LandingReviews() {
  ...
  useEffect(() => {
    ...
    fetch("/api-ielts/admin/avatar")
      .then((r) => r.json())
      .then((d) => {
        if (d?.dataUrl) setAdminAvatar(d.dataUrl);
      })
      .catch(() => {});
  }, []);
  ...
  return (
    <section className="mt-12 md:mt-16">
      <div className="text-center mb-8">
        ...
        <p className="text-white/40 text-sm mt-2" dir="rtl" lang="ar">
          آراء طلابنا
        </p>
      </div>
            ...
            {r.adminReply && (
              <div className="mt-4 ml-4 border-l-2 border-teal-400/60 pl-4 bg-teal-500/10 rounded-r-xl py-3 pr-3">
                <div className="flex items-center gap-2 mb-1.5">
                  ...
                  <BadgeCheck
                    className="w-4 h-4 text-sky-400 fill-sky-400/20"
                    aria-label="Verified"
                  />
                ...
                {r.adminReplyAt && (
                  <p className="text-white/30 text-[10px] mt-1.5">
                    {new Date(r.adminReplyAt).toLocaleDateString("en-US", {
                      month: "short",
                      day: "numeric",
                      year: "numeric",
                    })}
        ...
        <main className="flex-1 flex flex-col lg:flex-row items-center gap-12 lg:gap-16">
          <div className="flex-1 text-center lg:text-left">
            ...
            <p className="text-lg sm:text-xl text-white/60 leading-relaxed mb-8 max-w-lg mx-auto lg:mx-0">
              Your AI-powered companion for IELTS success. Master vocabulary,
              ace your speaking and listening, practice reading and writing —
              all on one platform.
            </p>

            <p
              className="text-base text-white/40 mb-10 max-w-lg mx-auto lg:mx-0 font-cairo"
              dir="rtl"
              lang="ar"
            >
              رفيقك الذكي في رحلة الآيلتس — تعلّم المفردات، أتقن المحادثة
              والاستماع، تدرب على القراءة والكتابة - كل شيء في مكان واحد
            </p>
              ...
              <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-6 sm:p-8 shadow-2xl">
                ...
                <div className="space-y-3">
                  {landingFeatures.map((f, i) => (
                    ...
                    >
                      <div
                        className={`w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center shrink-0 group-hover:bg-white/10 transition-colors`}
                      >
        ...
        <footer className="mt-12 md:mt-16 pb-6 text-center">
          <div className="flex items-center justify-center gap-2 text-white/30 text-sm">
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="text-teal-400/60 hover:text-teal-400 font-semibold transition-colors"
            >
              4IELTS.com
            </a>
          </div>
...
export function PasswordGate({ children }: PasswordGateProps) {
  ...
  const startIntroPolling = useCallback((em: string) => {
    stopPolling();
    const poll = async () => {
      const result = await postJson("/api-ielts/auth/intro/check", {
        email: em,
      });
      if (result.status === "approved") {
        ...
        localStorage.removeItem("lexo-ielts:intro_pending_email");
        setError(
          "Your access request was not approved. Please contact your instructor.",
        );
  ...
  useEffect(() => {
    async function init() {
      ...
      if (introEmail && introToken) {
        try {
          const introMe = await fetch("/api-ielts/auth/intro/me", {
            headers: {
              "x-student-email": introEmail,
              "x-student-token": introToken,
            },
          })
            .then((r) => r.json())
            .catch(() => ({ user: null }));
          if (introMe?.user?.status === "approved") {
            localStorage.setItem(
              STORAGE_KEY,
              JSON.stringify({ email: introEmail, token: introToken }),
            );
            localStorage.setItem("lexo-ielts:tier", "intro");
            setIsIntroStudent(true);
            setPhase("unlocked");
      ...
      const introPendingEmail = localStorage.getItem(
        "lexo-ielts:intro_pending_email",
      );
      if (introPendingEmail) {
        try {
          const statusResult = await postJson("/api-ielts/auth/intro/check", {
            email: introPendingEmail,
          });
      ...
      if (lastEmail) {
        ...
        if (dbSession.status === "active" && dbSession.token) {
          localStorage.setItem(
            STORAGE_KEY,
            JSON.stringify({ email: lastEmail, token: dbSession.token }),
          );
  ...
  const handleLogin = useCallback(
    async (e: React.FormEvent) => {
      ...
      setInfo("");
      const normalizedEmail = email.trim().toLowerCase();
      const result = await postJson("/api-ielts/access/login", {
        email: normalizedEmail,
        password,
      });
      ...
      if (result.status === "needs_password_setup") {
        ...
        setError(
          result.error ??
            "Your account is from a previous version. Please reopen LEXO from the device you originally used, or contact your instructor.",
        );
        return;
      }
      if (result.status === "pending") {
      ...
      if (result.error?.toLowerCase().includes("no account found")) {
        const introResult = await postJson("/api-ielts/auth/intro/login", {
          email: normalizedEmail,
          password,
        });
        setLoading(false);
        if (introResult.status === "approved" && introResult.token) {
          ...
          setError(
            "Your subscription has expired. Please contact your instructor.",
          );
        } else {
          // Not found in either system
          setError(
            "No account found for this email. Please register with an access code first.",
          );
        }
        return;
      }
  ...
  if (phase === "unlocked")
    return <PasswordGateUnlocked>{children}</PasswordGateUnlocked>;
      ...
      <AuthShell>
        <div className="text-center">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mb-1">
            Your access to LEXO has expired. Please contact Abu Omar to renew
            your subscription.
          </p>
          <p
          ...
          <div className="flex items-center justify-center gap-2 text-xs text-teal-600 dark:text-teal-400 mb-5 bg-teal-50 dark:bg-teal-900/20 rounded-xl py-2.5 px-3">
            ...
            Checking for renewal every 10 seconds — this will unlock
            automatically once renewed.
          </div>
          <div className="space-y-3">
            <a
              href={WHATSAPP_VISITOR_URL}
            ...
            <button
              onClick={() => goToLogin(expiredEmailRef.current)}
    ...
    return (
      <AuthShell
        footer={
          <button
            onClick={() => goToLogin()}
            className="w-full mt-5 text-xs text-gray-400 hover:text-gray-600 underline"
          >
      ...
      >
        <div className="text-center">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mb-2">
            ...
            <span className="font-semibold">{email}</span>. Your account is
            waiting for admin approval. Once approved, you'll be able to log in
            here.
          </p>
          <p
    ...
    return (
      <AuthShell>
        <div className="text-center">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mb-1">
            Your account has been approved by the admin. Log in with your email
            and password to enter LEXO.
          </p>
          <p
            className="text-xs text-gray-400 dark:text-gray-500 mb-6"
            dir="rtl"
