...
function readLevelFilter(): LevelFilter {
  ...
  try {
    ...
    if (raw === "A2" || raw === "B1" || raw === "B2" || raw === "ALL")
      return raw;
  } catch {}
  return "ALL";
}
...
function writeLevelFilter(v: LevelFilter) {
  ...
  try {
    localStorage.setItem(LEVEL_FILTER_KEY, v);
  } catch {}
}
...
export function ReadingSkills({ onBack }: { onBack: () => void }) {
  ...
  const [selectedType, setSelectedType] = useState<SkillQuestionType | null>(
    null,
  );
  const [selectedExercise, setSelectedExercise] =
    useState<SkillExercise | null>(null);
  ...
  const [levelFilter, setLevelFilterState] = useState<LevelFilter>(() =>
    readLevelFilter(),
  );
  ...
  const pickExercise = (ex: SkillExercise) => {
    ...
    setUserAnswers(
      ex.items.map(() => (typeof ex.items[0]?.answer === "object" ? [] : null)),
    );
    setResultData(null);
    setPhase("exercise");
  };
  ...
  if (phase === "type-list") {
    const activeLevelMeta =
      levelFilter !== "ALL" ? LEVELS.find((l) => l.id === levelFilter) : null;
    return (
      <div className="space-y-6">
        ...
        <div className="bg-gradient-to-br from-violet-500 via-purple-500 to-fuchsia-500 rounded-3xl p-6 sm:p-8 text-white shadow-xl">
          ...
          <h2 className="text-2xl sm:text-3xl font-extrabold leading-tight">
            Reading by Question Type
          </h2>
          <p className="text-white/90 text-sm mt-2 max-w-2xl">
            Targeted exercises for each of the 17 IELTS Reading question types,
            now organised into 3 CEFR levels. Pick your level below, then choose
            a type to start practising.
          </p>
        </div>
        ...
        {activeLevelMeta && (
          <div
            className={`rounded-2xl p-4 text-white shadow-md bg-gradient-to-br ${activeLevelMeta.color}`}
          >
            <p className="text-xs font-bold uppercase tracking-widest text-white/80">
              {activeLevelMeta.label}
            </p>
            <p className="text-sm text-white/95 mt-1">
              {activeLevelMeta.shortDesc}
            </p>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {QUESTION_TYPES.map((qt) => {
            ...
            const exs =
              levelFilter === "ALL"
                ? allExs
                : allExs.filter((e) => e.level === levelFilter);
            ...
            return (
              ...
              >
                <div className="flex items-start gap-3">
                  <div
                    className={`w-12 h-12 rounded-2xl bg-gradient-to-br ${qt.color} flex items-center justify-center shrink-0 text-2xl shadow-md`}
                  >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <h3 className="font-extrabold text-foreground text-base leading-tight mt-0.5">
                      {qt.label}
                    </h3>
  ...
  if (phase === "exercise-list" && selectedType) {
    ...
    return (
      <div className="space-y-6">
        ...
        <div
          className={`rounded-3xl p-6 sm:p-7 text-white shadow-xl bg-gradient-to-br ${meta.color}`}
        >
          <p className="text-xs font-bold uppercase tracking-widest text-white/80">
            Type {meta.number}
          </p>
        ...
        {exs.length === 0 ? (
          <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-2xl p-5 text-sm text-amber-700 dark:text-amber-300">
            No exercises at this level yet. Try a different level above, or
            check back later.
          </div>
        ) : (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2 mb-0.5">
                      <span
                        className={`text-[10px] font-extrabold px-1.5 py-0.5 rounded ${LEVEL_PILL_CLASS[ex.level]}`}
                      >
                        {ex.level}
                      </span>
                  ...
                  {result ? (
                    <span
                      className={`text-[11px] font-extrabold tabular-nums px-2.5 py-1 rounded-full shrink-0 ${
                    ...
                    <span className="text-[11px] font-bold text-muted-foreground bg-muted px-2.5 py-1 rounded-full shrink-0">
                      New
                    </span>
                  )}
                  <ChevronRight className="w-4 h-4 text-muted-foreground group-hover:text-primary group-hover:translate-x-0.5 transition-all shrink-0" />
        ...
        return (
          ...
          >
            ...
            {count !== null && count !== undefined && (
              <span
                className={`text-[10px] tabular-nums px-1.5 rounded-full ${active ? "bg-white/30" : "bg-muted"}`}
              >
                {count}
              </span>
            )}
    ...
    <div className="space-y-5">
      ...
      <div className="bg-card border border-border rounded-2xl p-5">
        <div className="flex items-center gap-2 mb-1">
          <span
            className={`text-[10px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-full text-white bg-gradient-to-br ${meta.color}`}
          >
            {meta.emoji} {meta.label}
          </span>
          <span
            className={`text-[10px] font-extrabold px-1.5 py-0.5 rounded ${LEVEL_PILL_CLASS[exercise.level]}`}
          >
            {exercise.level}
          </span>
      ...
      <div className="bg-card border border-border rounded-2xl p-5 space-y-4">
        ...
        {exercise.visual && (
          <pre className="text-xs sm:text-sm font-mono whitespace-pre overflow-x-auto bg-muted/30 rounded-xl p-3 text-foreground border border-border">
            {exercise.visual}
          </pre>
        )}
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        {allowPartialSubmit ? (
          <p className="text-xs text-muted-foreground">
            ...
            answered. You can submit at any time.
          </p>
        ) : (
          <span />
        )}
    ...
    <div className="space-y-3 text-sm sm:text-[15px] leading-relaxed text-foreground">
      {paragraphs.map((p, i) => {
        ...
        if (m) {
          return (
            <p key={i}>
              <span className="inline-block font-extrabold text-primary mr-2">
                {m[1]}
              </span>
              {m[2]}
            </p>
...
function ItemsRunner({
        ...
        <div className="flex flex-wrap gap-2">
          {exercise.options!.map((o) => {
            ...
            return (
              ...
              >
                <span className="font-bold mr-1">{o.label}.</span>
                {o.text}
              </button>
            );
          })}
  ...
  if (
    ...
    exercise.type === "choose_title" ||
    exercise.type === "skimming"
  ) {
        ...
        <div className="flex flex-col gap-2">
          {exercise.options!.map((o) => {
            ...
            return (
              ...
              >
                <span className="font-extrabold mr-2">{o.label}.</span>
                {o.text}
              </button>
            );
          })}
      ...
      <div className="space-y-5">
        {exercise.items.map((item, i) => {
          ...
          return (
            ...
            >
              <p className="text-sm sm:text-[15px] font-semibold text-foreground whitespace-pre-line mb-2">
                {item.prompt}
              </p>
              <div className="flex flex-col gap-2">
                {opts.map((o) => {
...
function ResultView({
  ...
  return (
    <div className="space-y-5">
      {/* Score banner */}
      <div
        className={`rounded-3xl p-6 sm:p-7 text-white shadow-xl ${
          perfect
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-white/90 mt-0.5">
              {perfect
                ? "Perfect — every answer matched the passage."
                : pct >= 60
                  ? "Solid work. Read the analysis to lock in the trickier ones."
                  : "Keep going. The analysis below explains every answer."}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-card border border-border rounded-2xl p-5">
        ...
        <div className="space-y-2">
          {exercise.items.map((item, i) => {
            ...
            const userStr = skipped
              ? "—"
              : Array.isArray(ua)
                ? ua.join(", ")
                : (ua as string);
            const corr = Array.isArray(item.answer)
              ? item.answer.join(", ")
              : item.answer;
            ...
            const userOptText =
              !skipped && typeof ua === "string"
                ? findOptionText(ua)
                : undefined;
            const correctOptText =
              typeof item.answer === "string"
                ? findOptionText(item.answer)
                : undefined;
            return (
              <div
                key={i}
              ...
              >
                <div className="flex items-start gap-2">
                  {skipped ? (
                    <span className="w-4 h-4 rounded-full bg-amber-500 text-white text-[9px] font-extrabold flex items-center justify-center shrink-0 mt-0.5">
                      !
                    </span>
                  ) : ok ? (
                    ...
                    <div className="mt-1 space-y-0.5">
                      ...
                      {item.explanation && (
                        <p className="mt-1.5 text-foreground/85 italic">
                          <span className="font-bold not-italic">Why:</span>{" "}
                          {item.explanation}
                        </p>
                      )}
                    </div>
      ...
      <div className="flex flex-wrap gap-3 justify-end pb-4">
        <button
          ...
          className="px-4 py-2.5 rounded-xl bg-muted text-foreground font-semibold hover:bg-muted/80 transition-colors text-sm"
        >
          All Question Types
        </button>
        <button
        ...
        <button
          onClick={onRetry}
          className="px-5 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-bold transition-colors text-sm"
        >
          Try again
        </button>
      </div>
