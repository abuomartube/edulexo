...
function encouragement(score: number): string {
  ...
  if (score === 3)
    return "Solid effort — keep practising and you'll nail it next time. 💪";
  if (score === 2)
    return "Good try. Re-read the story and have another go — you're learning!";
  if (score === 1)
    return "Don't give up! Re-read the story carefully and try again.";
...
export function StoryQuiz({ storyId, onComplete }: StoryQuizProps) {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const e = err as { data?: { error?: string }; message?: string } | null;
      setSubmitError(
        e?.data?.error ?? e?.message ?? "Could not submit your answers.",
      );
    } finally {
      setSubmitting(false);
    }
        ...
        <div className="text-center">
          ...
          {results.xpAwarded > 0 && (
            <p className="mt-2 inline-flex items-center gap-1 text-sm font-medium text-amber-600 dark:text-amber-400">
              <Sparkles className="h-4 w-4" />+{results.xpAwarded} XP earned
            </p>
          )}
        </div>
                      ...
                      return (
                        <div
                          ...
                          className={cn(
                            ...
                            isCorrect &&
                              "bg-emerald-200/60 dark:bg-emerald-900/50 font-medium",
                            !isCorrect &&
                              isChosen &&
                              "bg-rose-200/60 dark:bg-rose-900/50",
                          )}
                        >
                          <span className="font-semibold w-5">{L}.</span>
                          <span className="flex-1">{r.choices[L]}</span>
                          {isCorrect && (
                            <CheckCircle2 className="h-4 w-4 text-emerald-700 dark:text-emerald-300 shrink-0" />
                          )}
                          {!isCorrect && isChosen && (
                            <XCircle className="h-4 w-4 text-rose-700 dark:text-rose-300 shrink-0" />
                          )}
                        </div>
                      );
                    })}
