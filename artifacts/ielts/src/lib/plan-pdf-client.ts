...
export async function downloadPlanPdf(
  ...
  if (startVal === null)
    tasks.push(
      customFetch<{ value: string }>("/api-ielts/user-data/plan_start_date")
        .then((r) => {
          startVal = r.value;
        })
        .catch(() => undefined),
  ...
  if (durVal === null)
    tasks.push(
      customFetch<{ value: string }>("/api-ielts/user-data/plan_duration_days")
        .then((r) => {
          durVal = Number(r.value);
        })
        .catch(() => undefined),
    );
  if (nameVal === null)
  ...
  if (!levelVal)
    throw new Error("Your study level isn't set yet. Finish onboarding first.");
  ...
  const coverage = buildCoverageRows(
    getPlanCoverage(levelVal, startISO, duration),
  );
  ...
  const safe =
    (payload.name || "Student").replace(/[^A-Za-z0-9]+/g, "-").slice(0, 40) ||
    "Student";
