...
export default function Antonyms() {
  ...
  const {
    ...
    save: savePosition,
    loadedRef,
  } = useActivityPosition("antonyms", skillFilter);

  const cards = useMemo(() => {
    if (skillFilter === "All Skills") return allAntonyms;
    return allAntonyms.filter(
      ...
      <div className="flex flex-col h-full max-w-2xl mx-auto animate-in fade-in duration-500">
        ...
        <div className="flex flex-col gap-3 mb-5">
          ...
          <p className="text-sm text-muted-foreground">
            {cards.length} cards · See the word → guess its opposite → flip to
            check
          </p>
        </div>
