  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-3">
        <span
          className={`text-xs font-bold uppercase tracking-wider px-2.5 py-1 rounded-full ${badgeColor[popup.kind]}`}
        >
...
function buildSegments(essay: string, result: EssayResult) {
  ...
  const addAnnotations = (
    ...
    for (const item of items) {
      ...
      while (searchFrom < essay.length) {
        const idx = essay
          .toLowerCase()
          .indexOf(phrase.toLowerCase(), searchFrom);
        if (idx === -1) break;
        annotations.push({
          start: idx,
  ...
  for (const ann of merged) {
    if (ann.start > cursor)
      segments.push({ text: essay.slice(cursor, ann.start) });
...
const hlBg: Record<string, string> = {
  grammar:
    "bg-red-200/80 dark:bg-red-800/50 hover:bg-red-300/80 dark:hover:bg-red-700/60",
  vocab:
    "bg-yellow-200/80 dark:bg-yellow-800/50 hover:bg-yellow-300/80 dark:hover:bg-yellow-700/60",
  coherence:
    "bg-blue-200/80 dark:bg-blue-800/50 hover:bg-blue-300/80 dark:hover:bg-blue-700/60",
};
    ...
    <div onClick={() => setPopup(null)} className="relative">
      ...
      <div className="flex items-center gap-6 mt-3 text-xs text-muted-foreground flex-wrap">
        <span className="flex items-center gap-1.5">
          <span className="inline-block w-3 h-3 rounded bg-red-300 dark:bg-red-700" />{" "}
          Grammar errors
        </span>
        <span className="flex items-center gap-1.5">
          <span className="inline-block w-3 h-3 rounded bg-yellow-300 dark:bg-yellow-700" />{" "}
          Vocabulary upgrades
        </span>
        <span className="flex items-center gap-1.5">
          <span className="inline-block w-3 h-3 rounded bg-blue-300 dark:bg-blue-700" />{" "}
          Coherence issues
        </span>
...
function EssayExampleBox({
  ...
  return (
    <div
      className={`rounded-2xl border p-5 flex flex-col gap-3 ${accentClass}`}
    >
      <div className="flex items-center justify-between gap-2 flex-wrap">
        <div className="flex items-center gap-2">
          <span
            className={`text-xs font-bold uppercase tracking-wider px-2.5 py-1 rounded-full ${badgeClass}`}
          >
            {label}
          </span>
    ...
    <div className={`rounded-2xl border p-5 space-y-3 ${accentClass}`}>
      <div className="flex items-center justify-between">
        ...
        >
          {copied ? (
            <>
              <CheckCircle2 className="w-3.5 h-3.5 text-green-500" /> نُسخ!
            </>
          ) : (
            <>
...
function ParaPopup({
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-3">
        <span
          className={`text-xs font-bold uppercase tracking-wider px-2.5 py-1 rounded-full ${paraBadgeColor[kind]}`}
        >
          {popup.type}
        </span>
        <button
...
function buildParaSegments(text: string, corrections: ParagraphCorrection[]) {
  ...
  for (const ann of merged) {
    if (ann.start > cursor)
      segments.push({ text: text.slice(cursor, ann.start) });
    segments.push({
      ...
      <div className="bg-card border border-border rounded-2xl p-5 space-y-3">
        <div className="flex items-center justify-between">
          ...
          {corrections.length > 0 && (
            <span className="text-xs text-muted-foreground font-medium">
              {corrections.length} error{corrections.length !== 1 ? "s" : ""} ·
              click to inspect
            </span>
          )}
        </div>
...
function pickNextAssignment(
  ...
  const all = subtypeFilter
    ? base.filter((a) => a.subtype === subtypeFilter)
    : base;
  ...
  const untouched = all.filter(
    (a) => !submitted.has(a.id) && !skipped.has(a.id),
  );
  if (untouched.length)
    return untouched[Math.floor(Math.random() * untouched.length)];
  const onlySkipped = all.filter((a) => skipped.has(a.id));
  if (onlySkipped.length)
    return onlySkipped[Math.floor(Math.random() * onlySkipped.length)];
  return null;
}
...
export default function EssayChecker() {
  ...
  const [paragraphResult, setParagraphResult] =
    useState<ParagraphResult | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [copied, setCopied] = useState(false);
  const [progress, setProgress] = useState<
    Record<Category, { submitted: number; skipped: number }>
  >({
  ...
  > = {
    task1: {
      label: "Task 1",
      emoji: "📊",
      sub: "Report / letter (≥150 words)",
    },
    task2: { label: "Task 2", emoji: "📝", sub: "Opinion essay (≥250 words)" },
    paragraph: {
      label: "Paragraph",
      emoji: "💬",
      sub: "Letter, email, or paragraph",
    },
  };
  ...
  const handlePhotoSelected = useCallback(
    (file: File | null) => {
      ...
      if (!okType) {
        setExtractError(
          "Unsupported format. Please upload JPG, JPEG, PNG, or HEIC.",
        );
        if (photoInputRef.current) photoInputRef.current.value = "";
        return;
      }
  ...
  const handleExtractText = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(
          data.error ?? "Could not extract text. Please try again.",
        );
      }
  ...
  const [categoryProgress, setCategoryProgress] =
  ...
  const taskType: TaskType =
    assignment?.category === "task1"
      ? "Task 1"
      : assignment?.category === "task2"
        ? "Task 2"
        : "Paragraph";
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch("/api-ielts/orwell/progress", {
          headers: getStudentAuthHeaders(),
        });
        if (!res.ok) return;
  ...
  const loadNextForCategory = useCallback(
    async (cat: Category, subtypeFilter?: string) => {
      setError(null);
      let prog: CategoryProgress | null = null;
      try {
        const res = await fetch(`/api-ielts/orwell/next?category=${cat}`, {
          headers: getStudentAuthHeaders(),
        });
  ...
  const handleSkip = useCallback(async () => {
    ...
    const updated: CategoryProgress = {
      ...
      skippedIds: Array.from(
        new Set([...(categoryProgress?.skippedIds ?? []), assignment.id]),
      ),
    };
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      if (!finalResult)
        throw new Error("Grading ended unexpectedly. Please try again.");
      ...
      setCategoryProgress((prev) => {
        const submittedIds = Array.from(
          new Set([...(prev?.submittedIds ?? []), assignment.id]),
        );
        const skippedIds = (prev?.skippedIds ?? []).filter(
          (id) => id !== assignment.id,
        );
  ...
  const handleCopyFreeReport = () => {
    ...
    const lines = [
      ...
      "",
      "GRAMMAR ERRORS",
      ...freeResult.grammarErrors.map(
        (e) => `• "${e.original}" → "${e.correction}" — ${e.explanation}`,
      ),
      "",
      "VOCABULARY UPGRADES",
      ...freeResult.vocabularyUpgrades.map(
        (v) => `• "${v.original}" → "${v.better}" — ${v.reason}`,
      ),
      "",
      "COHERENCE ISSUES",
      ...freeResult.coherenceIssues.map(
        (c) => `• "${c.original}" → "${c.correction}" — ${c.explanation}`,
      ),
      "",
      "STRENGTHS",
  ...
  const handleCopyReport = () => {
    ...
    const lines = [
      ...
      "",
      "GRAMMAR ERRORS",
      ...result.grammarErrors.map(
        (e) => `• "${e.original}" → "${e.correction}" — ${e.explanation}`,
      ),
      "",
      "VOCABULARY UPGRADES",
      ...result.vocabularyUpgrades.map(
        (v) => `• "${v.original}" → "${v.better}" — ${v.reason}`,
      ),
      "",
      "COHERENCE ISSUES",
  ...
  const handleCopyParagraphAll = () => {
    ...
    const lines = [
      ...
      "",
      "🔍 CORRECTIONS",
      ...(paragraphResult.corrections ?? []).map(
        (c) =>
          `• "${c.original}" → "${c.correction}" (${c.type}): ${c.explanation}`,
      ),
      "",
      "📄 CORRECTED VERSION",
      ...
      <div className="max-w-3xl mx-auto animate-in fade-in duration-500">
        ...
        {screen === "intro" && (
          <div
            ...
            style={{
              background:
                "linear-gradient(160deg, hsl(177,83%,28%) 0%, hsl(177,83%,32%) 60%, hsl(177,83%,28%) 100%)",
            }}
          >
            <div className="flex flex-col items-center text-center px-6 pt-10 pb-8 gap-6">
              <div>
                ...
                <p className="text-white/70 text-sm mt-1 font-medium">
                  Structured IELTS Writing Assignments
                </p>
                <p
                  style={{ color: "#C9A84C" }}
                  className="text-xs mt-0.5 font-semibold opacity-80"
                >
              ...
              <p className="text-white/80 text-sm leading-relaxed max-w-sm">
                Practise real IELTS-style writing prompts. Choose a category,
                get an assignment, write your response, and Orwell AI will grade
                it like an examiner. One assignment at a time — no copy-pasting
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 gap-3">
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm text-muted-foreground">
                    Paste any essay, paragraph, or writing — get instant Orwell
            ...
            <div className="bg-muted/30 border border-border rounded-2xl p-4 text-xs text-muted-foreground leading-relaxed">
              ...
              <ul className="space-y-1 list-disc list-inside">
                <li>
                  Task 1 has 20 assignments across 6 chart types. Task 2 has 25
                  assignments across 5 essay types.
                </li>
                <li>
                  Hit <strong>Refresh</strong> to skip and get a new one.
                </li>
        ...
        {screen === "task2subtype" && (
          <div className="space-y-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-xl font-extrabold text-foreground">
                  Task 2 — Choose Essay Type
                </h1>
                <p className="text-sm text-muted-foreground">
                  Each type uses a different IELTS essay structure
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 gap-3">
              {TASK2_SUBTYPES.map((sub) => {
                const totalForSub = ASSIGNMENTS_BY_CATEGORY.task2.filter(
                  (a) => a.subtype === sub.id,
                ).length;
                return (
                  <button
                  ...
                  <h2 className="font-extrabold text-foreground truncate">
                    ...
                    {category === "task2" && task2Subtype && (
                      <span className="text-muted-foreground font-semibold">
                        ...
                        ·{" "}
                        {
                          TASK2_SUBTYPES.find((s) => s.id === task2Subtype)
                            ?.label
                        }
                      </span>
                    )}
                    ...
                    {(() => {
                      ...
                      if (category === "task2" && task2Subtype) {
                        const subAssignments =
                          ASSIGNMENTS_BY_CATEGORY.task2.filter(
                            (a) => a.subtype === task2Subtype,
                          );
                        const subIds = new Set(subAssignments.map((a) => a.id));
                        const done = submitted.filter((id) =>
                          subIds.has(id),
                        ).length;
            ...
            {!assignment && (
              <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-2xl p-6 text-center">
                ...
                <p className="text-sm text-green-700 dark:text-green-400">
                  You've finished every assignment in this category. Great work
                  ...
                  <div className="rounded-2xl border border-primary/30 bg-primary/5 p-4 space-y-2">
                    ...
                    {streamingPreview ? (
                      ...
                      <p className="text-xs text-muted-foreground italic">
                        Reviewing your essay against the official IELTS band
                        descriptors…
                      </p>
                    )}
                  </div>
              ...
              <>
                <div className="flex items-center gap-3 pb-2 border-b border-border">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      Orwell AI · {wordCount} words analysed
                    </p>
                  </div>
                </div>
                ...
                <div className="flex gap-3 flex-wrap pb-6">
                  <Button
                    onClick={handleCopyParagraphAll}
                    variant="outline"
            ...
            {(taskType === "Task 1" || taskType === "Task 2") && result && (
              <>
                <div className="bg-gradient-to-br from-primary/10 to-primary/5 border border-primary/20 rounded-3xl p-6 text-center">
                  ...
                  <div className="text-8xl font-extrabold text-primary mb-1">
                    {result.overallBand}
                  </div>
                  <p className="text-sm text-muted-foreground">
                    {result.taskType} · {result.wordCount ?? wordCount} words
                    analysed
                  </p>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <BandCard
                    label="Task Response"
                    band={result.scores.taskResponse.band}
                    feedback={result.scores.taskResponse.feedback}
                  />
                  <BandCard
                    label="Coherence & Cohesion"
                    band={result.scores.coherenceCohesion.band}
                    feedback={result.scores.coherenceCohesion.feedback}
                  />
                  <BandCard
                    label="Lexical Resource"
                    band={result.scores.lexicalResource.band}
                    feedback={result.scores.lexicalResource.feedback}
                  />
                  <BandCard
                    label="Grammatical Range & Accuracy"
                    band={result.scores.grammaticalRange.band}
                    feedback={result.scores.grammaticalRange.feedback}
                  />
                </div>
                      ...
                      {result.grammarErrors.map((e, i) => (
                        ...
                        >
                          <div className="flex flex-wrap gap-2 items-center mb-1">
                            ...
                            <span className="text-[10px] uppercase font-bold px-2 py-0.5 rounded-full bg-red-100 text-red-700 dark:bg-red-900/40 dark:text-red-300">
                              {e.type}
                            </span>
                          </div>
                          <p className="text-xs text-muted-foreground leading-relaxed">
                  ...
                  <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-2xl p-5">
                    ...
                    <ul className="space-y-1.5">
                      {result.strengths.map((s, i) => (
                        <li
                          key={i}
                          className="text-xs text-green-800 dark:text-green-300 leading-relaxed flex gap-2"
                        >
                  ...
                  <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-2xl p-5">
                    ...
                    <ul className="space-y-1.5">
                      {result.improvements.map((item, i) => (
                        <li
                          key={i}
                          className="text-xs text-amber-800 dark:text-amber-300 leading-relaxed flex gap-2"
                        >
                          <span className="shrink-0">→</span>
                          {item}
                        </li>
                ...
                {(result.exampleEssayBand6 || result.exampleEssayBand8) && (
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mb-4">
                      See how this assignment can be answered at two different
                      levels. Use these as a study guide — not to copy.
                    </p>
                    <div className="space-y-4">
                      <EssayExampleBox
          ...
          <div className="space-y-5">
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-2 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Step 1 of 2 — choose what you want graded.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-card border border-border rounded-2xl p-5">
              <p className="text-sm text-muted-foreground leading-relaxed">
                Tell Orwell AI what kind of writing you're submitting so it can
                use the right rubric.
              </p>
            </div>
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-2 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Paste any writing — Orwell AI will give feedback &amp; an
                    IELTS band score.
                  </p>
                </div>
              </div>
            ...
            <div className="bg-card border border-border rounded-2xl p-5 space-y-3">
              <div className="flex items-center justify-between gap-2 flex-wrap">
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="text-[10px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-full bg-primary/10 text-primary">
                    {FREE_MODE_META[freeMode].emoji}{" "}
                    {FREE_MODE_META[freeMode].label}
                  </span>
                  <span className="text-[10px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-full bg-muted text-muted-foreground">
                    {freeMode === "paragraph"
                      ? "Writing Coach"
              ...
              <p className="text-sm text-muted-foreground leading-relaxed">
                {freeMode === "task1" &&
                  "Paste your IELTS Task 1 response (report or letter). Orwell AI will grade it using the official Task 1 rubric."}
                {freeMode === "task2" &&
                  "Paste your IELTS Task 2 essay. Orwell AI will grade it using the official Task 2 band descriptors."}
                {freeMode === "paragraph" &&
                  "Paste your paragraph, letter, or email. Orwell AI will give you grammar, vocabulary, and improved-version feedback."}
              </p>
            </div>
                  ...
                  >
                    ...
                    <div>
                      <p className="font-bold text-foreground">
                        Upload a photo of your handwritten essay
                      </p>
                  ...
                  <div className="rounded-2xl border border-border bg-card overflow-hidden">
                    {photoPreviewUrl ? (
                      ...
                      <div className="bg-muted/40 p-8 flex flex-col items-center justify-center gap-2">
                        ...
                        <p className="text-xs text-muted-foreground">
                          HEIC photo · preview not supported, but the file is
                          ready.
                        </p>
                      </div>
                    )}
              ...
              <div className="space-y-2">
                  ...
                  <div className="flex items-start gap-2 p-3 rounded-xl bg-primary/5 border border-primary/20 text-xs text-foreground">
                    ...
                    <p>
                      <span className="font-semibold">
                        Text extracted from your photo.
                      </span>{" "}
                ...
                <div className="relative">
                  <textarea
                    ...
                    placeholder={
                      extracted
                        ? "Review the extracted text and edit if needed…"
            ...
            <Button
              ...
              disabled={
                !canSubmitFree || (freeInputMode === "photo" && !extracted)
              }
              className="w-full rounded-full h-12 text-base font-semibold"
            >
              {loading ? (
            ...
            {loading && (
              <div className="rounded-2xl border border-primary/30 bg-primary/5 p-4 space-y-2">
                ...
                {streamingPreview ? (
                  ...
                  <p className="text-xs text-muted-foreground italic">
                    Reviewing your writing against the official IELTS band
                    descriptors…
                  </p>
                )}
              </div>
        ...
        {screen === "freeresult" && freeParagraphResult && (
          <div ref={resultRef} className="space-y-5">
            ...
            <ParagraphCard
              emoji="✅"
              title="Strengths"
              content={freeParagraphResult.strengths}
              accentClass="bg-green-50 dark:bg-green-900/20 border-green-200 dark:border-green-800"
            />
            <ParagraphCard
              emoji="⚠️"
              title="Areas for Improvement"
              content={freeParagraphResult.improvements}
              accentClass="bg-amber-50 dark:bg-amber-900/20 border-amber-200 dark:border-amber-800"
            />
            <AnnotatedParagraph
              text={freeText}
              corrections={freeParagraphResult.corrections ?? []}
            />
            <ParagraphCard
              emoji="📄"
              title="Corrected Version"
              content={freeParagraphResult.corrected}
              accentClass="bg-card border-border"
            />
            <ParagraphCard
              emoji="⭐"
              title="Better Version"
              content={freeParagraphResult.better}
              accentClass="bg-blue-50 dark:bg-blue-900/20 border-blue-200 dark:border-blue-800"
            />
            <ParagraphCard
              emoji="🎩"
              title="Formal Version"
              content={freeParagraphResult.formal}
        ...
        {screen === "freeresult" && freeResult && (
          <div ref={resultRef} className="space-y-6">
            <div className="bg-muted/30 border border-border rounded-2xl px-4 py-3 text-xs text-muted-foreground">
              <span className="font-semibold text-foreground">
                Free Check · {freeResult.taskType ?? "IELTS"}
              </span>
              <span className="mx-2">·</span>
            ...
            <div className="bg-gradient-to-br from-primary/10 to-primary/5 border border-primary/20 rounded-3xl p-6 text-center">
              ...
              <div className="text-8xl font-extrabold text-primary mb-1">
                {freeResult.overallBand}
              </div>
              <p className="text-sm text-muted-foreground">
                Free Check · {freeResult.wordCount ?? freeWordCount} words
                analysed
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <BandCard
                label="Task Response"
                band={freeResult.scores.taskResponse.band}
                feedback={freeResult.scores.taskResponse.feedback}
              />
              <BandCard
                label="Coherence & Cohesion"
                band={freeResult.scores.coherenceCohesion.band}
                feedback={freeResult.scores.coherenceCohesion.feedback}
              />
              <BandCard
                label="Lexical Resource"
                band={freeResult.scores.lexicalResource.band}
                feedback={freeResult.scores.lexicalResource.feedback}
              />
              <BandCard
                label="Grammatical Range & Accuracy"
                band={freeResult.scores.grammaticalRange.band}
                feedback={freeResult.scores.grammaticalRange.feedback}
              />
            </div>
            ...
            {(freeResult.exampleEssayBand6 || freeResult.exampleEssayBand8) && (
              <div>
                ...
                <p className="text-xs text-muted-foreground mb-4">
                  Orwell AI rewrote your piece at two stronger levels — use them
                  as a study guide, not to copy.
                </p>
                <div className="space-y-4">
                  <EssayExampleBox
                    label="Your Original Writing"
                    band="—"
                    text={freeText}
                    accentClass="bg-card border-border"
                    badgeClass="bg-muted text-muted-foreground"
                  ...
                  {freeResult.exampleEssayBand6 && (
                    <EssayExampleBox
                      label="Improved — Intermediate"
                      band="5.5 – 6"
                      text={freeResult.exampleEssayBand6}
                      accentClass="bg-amber-50 dark:bg-amber-900/20 border-amber-200 dark:border-amber-800"
                      badgeClass="bg-amber-100 text-amber-700 dark:bg-amber-900/60 dark:text-amber-300"
                    />
                  )}
                  {freeResult.exampleEssayBand8 && (
                    <EssayExampleBox
                      label="Improved — Advanced"
                      band="7 – 8"
                      text={freeResult.exampleEssayBand8}
                      accentClass="bg-teal-50 dark:bg-teal-900/20 border-teal-200 dark:border-teal-800"
                      badgeClass="bg-teal-100 text-teal-700 dark:bg-teal-900/60 dark:text-teal-300"
                    />
                  )}
