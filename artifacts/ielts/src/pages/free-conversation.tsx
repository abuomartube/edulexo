...
function getAuthHeaders(): Record<string, string> {
  try {
    ...
    if (raw) {
      const { email: e, token: t } = JSON.parse(raw) as {
        email?: string;
        token?: string;
      };
...
class FreeConvErrorBoundary extends Component<
    ...
    if (this.state.hasError) {
      return (
        <div
          ...
          style={{
            background: `linear-gradient(160deg, #071422 0%, ${NAVY} 40%, #0C2040 100%)`,
          }}
        >
          <div className="text-center space-y-4 max-w-sm">
            ...
            <p className="text-white/60 text-sm">
              {this.state.msg || "Please go back and try again."}
            </p>
            <button
...
function FreeConversationInner({ onBack }: { onBack: () => void }) {
  ...
  }, [messages, streaming, isAssistantTyping]);

  useEffect(() => {
    if (stage !== "chat" || sessionStartAt === 0) return;
    const id = setInterval(
      () => setDuration(Math.floor((Date.now() - sessionStartAt) / 1000)),
      1000,
    );
  ...
  // ---------- Voice (VAD + Whisper) ----------
  const float32ToWavBlob = useCallback(
  ...
  const triggerBargeIn = useCallback(() => {
    ...
    if (bargeInArmedAtRef.current === 0 || now < bargeInArmedAtRef.current)
      return;
  ...
  const startVoiceSession = useCallback(async (): Promise<boolean> => {
    ...
    try {
      if (!ttsAudioRef.current) {
        ...
        primer.preload = "auto";
        primer.src =
          "data:audio/mpeg;base64,SUQzBAAAAAAAI1RTU0UAAAAPAAADTGF2ZjU4Ljc2LjEwMAAAAAAAAAAAAAAA//tQwAADB8AhSmxhIIEVCSiJrDCQBTcu3UrAAAAAFhYsRkkmYZA=";
        primer.muted = false;
        primer.volume = 1;
        const playPromise = primer.play();
        if (playPromise && typeof playPromise.then === "function") {
          playPromise.catch(() => {
            /* unlock attempt; ignore failure */
          });
        }
    ...
    try {
      ...
      if (name === "NotAllowedError" || name === "SecurityError") {
        setError(
          "Microphone access was blocked. Please allow it in your browser and start again.",
        );
      } else if (name === "NotFoundError" || name === "OverconstrainedError") {
        setError("No microphone was found on this device.");
      } else {
        setError(
          "Could not access the microphone. Please switch to Text mode.",
        );
      }
      return false;
    }
    ...
    try {
      ...
      energyStream.getTracks().forEach((t) => {
        try {
          t.stop();
        } catch {
          /* ignore */
    ...
    try {
      ...
      setError(
        "Could not start voice mode. Please refresh and try again, or switch to Text mode.",
      );
      if (energyTimerRef.current) {
        clearInterval(energyTimerRef.current);
        energyTimerRef.current = null;
  ...
  const sendUserMessage = useCallback(
    async (text: string) => {
      ...
      const isCurrent = () =>
        myReqId === streamRequestIdRef.current && !ctrl.signal.aborted;
  ...
  const endConversation = useCallback(async () => {
    ...
    try {
      ...
      setFeedbackError(
        "Could not generate the feedback report. Please try again.",
      );
    } finally {
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <div className="text-[10px] font-semibold" style={{ color: GREEN }}>
              natural chat
            </div>
          </div>
        </div>
        ...
        <main className="flex-1 flex flex-col items-center justify-center px-4 py-10 gap-8">
          <div className="text-center max-w-md">
            <h1 className="text-3xl sm:text-4xl font-black text-white mb-2 tracking-tight">
              ...
              <span style={{ color: GREEN }}>talk about?</span>
            </h1>
            <p className="text-white/60 text-sm">
              Pick a topic from our list, or bring your own.
            </p>
          </div>
          <div className="w-full max-w-2xl grid sm:grid-cols-2 gap-4">
            <button
              ...
              style={{
                background: `linear-gradient(160deg, rgba(107,47,230,0.18), rgba(107,47,230,0.05))`,
            ...
            >
              ...
              <p className="text-white/60 text-sm">
                30 popular conversation starters — from Travel to Technology.
              </p>
              <div
                className="mt-auto flex items-center gap-2 text-sm font-bold pt-2"
                style={{ color: TEAL }}
              >
                Browse topics{" "}
            ...
            <button
              ...
              className="group flex flex-col gap-3 p-6 rounded-3xl text-left transition-all hover:scale-[1.015]"
              style={{
                background: `linear-gradient(160deg, rgba(245,197,24,0.18), rgba(245,197,24,0.05))`,
            ...
            >
              ...
              <p className="text-white/60 text-sm">
                Type anything you want to discuss — work, hobbies, current
                events.
              </p>
              <div
                className="mt-auto flex items-center gap-2 text-sm font-bold pt-2"
        ...
        <main className="flex-1 flex flex-col items-center justify-center px-4 py-10 gap-6">
          ...
          <div className="w-full max-w-md space-y-3">
            ...
            <button
              ...
              style={{
                background: `linear-gradient(135deg, ${YELLOW}, #FFD93B)`,
                color: NAVY,
              }}
            >
  ...
  if (stage === "mode-pick") {
    return (
      ...
      >
        <Header
          leftAction={() =>
            setStage(topicSource === "custom" ? "topic-input" : "topic-pick")
          }
        />
        <main className="flex-1 flex flex-col items-center justify-center px-4 py-10 gap-7">
          <div className="text-center max-w-md">
            ...
            <p className="text-white/55 text-sm">
              You can switch later by ending the session.
            </p>
          </div>
          <div className="w-full max-w-2xl grid sm:grid-cols-2 gap-4">
            <button
            ...
            >
              ...
              <p className="text-white/60 text-sm">
                Hands-free conversation. Just talk — Churchill listens, replies
                out loud, and you can interrupt him anytime.
              </p>
              <div
                className="mt-auto flex items-center gap-2 text-sm font-bold pt-2"
                style={{ color: GREEN }}
            ...
            >
              ...
              <p className="text-white/60 text-sm">
                Type your responses. Best for quiet places or when you prefer
                reading.
              </p>
              <div
                className="mt-auto flex items-center gap-2 text-sm font-bold pt-2"
    ...
    return (
      ...
      >
        ...
        <main className="flex-1 overflow-y-auto px-4 py-5">
          <div className="max-w-2xl mx-auto space-y-4">
            ...
            {isAssistantTyping && !streaming && (
              <div
                className="flex items-center gap-2 px-4 py-3 rounded-2xl text-white/50 text-sm"
                style={{
                  background: "rgba(255,255,255,0.04)",
        ...
        <footer
          ...
          style={{
            borderTop: "1px solid rgba(107,47,230,0.15)",
      ...
      <main className="flex-1 overflow-y-auto px-4 py-5">
        <div className="max-w-2xl mx-auto space-y-4">
          ...
          {!feedbackLoading &&
            ...
            messages.length === 0 && (
              <div
                className="rounded-2xl p-6 text-center text-white/60"
                style={{
                  background: "rgba(255,255,255,0.04)",
                  border: "1px solid rgba(255,255,255,0.08)",
                }}
          ...
          {feedback && (
            <>
              <BandHero
                ...
                previousBest={
                  pastSessions.find(
                    (s) => s.id !== undefined && s.bandScore != null,
                  )?.bandScore ?? null
                }
              />
              ...
              <SectionCard
                ...
                icon={<Sparkles className="w-4 h-4" style={{ color: TEAL }} />}
              >
              ...
              {feedback.grammarMistakes?.length > 0 && (
                <SectionCard
                  title={`Grammar Corrections (${feedback.grammarMistakes.length})`}
                  icon={
                ...
                >
                  <div className="space-y-3">
                    {feedback.grammarMistakes.map((g, i) => (
                      ...
                      >
                        <div className="text-xs text-red-300 line-through opacity-80">
              ...
              {feedback.vocabularyUpgrades?.length > 0 && (
                <SectionCard
                  title={`Better Vocabulary (${feedback.vocabularyUpgrades.length})`}
                  icon={
              ...
              {feedback.sentenceUpgrades?.length > 0 && (
                <SectionCard
                  title={`Stronger Sentences (${feedback.sentenceUpgrades.length})`}
                  icon={
              ...
              {feedback.tips?.length > 0 && (
                <SectionCard
                  ...
                  icon={
                    <Lightbulb className="w-4 h-4" style={{ color: YELLOW }} />
                  }
                >
          ...
          {pastSessions.length > 0 && (
            <SectionCard
              title="Your Progress"
              icon={<TrendingUp className="w-4 h-4" style={{ color: GREEN }} />}
            >
              ...
              <div className="mt-4 space-y-2">
                {pastSessions.slice(0, 5).map((s) => (
                  <PastSessionRow
                    key={s.id}
                    s={s}
                    highlight={savedThisSession && s.id === pastSessions[0].id}
                  />
                ))}
                {pastLoading && (
          ...
          <div className="grid sm:grid-cols-2 gap-3 pt-2 pb-6">
            <button
              ...
              style={{
                background: `linear-gradient(135deg, ${GREEN}, #2ed968)`,
                color: NAVY,
              }}
            >
...
function BandHero({
  ...
  return (
    <div
      ...
      style={{
        background: `linear-gradient(160deg, ${color}25, ${color}08)`,
    ...
    >
      ...
      {delta != null && (
        <div
          className={`text-xs font-bold mt-1 ${delta >= 0 ? "text-green-400" : "text-red-400"}`}
        >
      ...
      <div className="flex items-center justify-center gap-4 mt-3 text-xs text-white/45">
        ...
        {feedback.userTurns != null && (
          <span className="flex items-center gap-1">
            <MessageCircle className="w-3 h-3" />
            {feedback.userTurns} turns
          </span>
        )}
...
function PastSessionRow({
  ...
  return (
    <div
      className="flex items-center gap-3 py-2 px-1 rounded-xl transition-colors"
      style={highlight ? { background: "rgba(29,185,84,0.06)" } : {}}
    >
      <div
...
function ProgressChart({ sessions }: { sessions: SessionSummary[] }) {
  const withBand = sessions
    .filter((s) => s.bandScore != null)
    .slice(0, 10)
    .reverse();
      ...
      >
        ...
        {withBand.map((s, i) => (
          <circle
            key={i}
            cx={i * w}
            cy={100 - ((s.bandScore! - min) / range) * 100}
            r="2"
            fill={TEAL}
            vectorEffect="non-scaling-stroke"
          />
    ...
    return (
      ...
      >
        <div className="text-center space-y-4 max-w-sm">
          ...
          <p className="text-white/60 text-sm">
            Free Conversation with Churchill is available in the Intro and
            Comprehensive plans.
          </p>
          <p className="text-white/40 text-xs" dir="rtl" lang="ar">
