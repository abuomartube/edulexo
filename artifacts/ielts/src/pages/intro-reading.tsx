...
function getAuthHeaders(): Record<string, string> {
  try {
    ...
    if (raw) {
      const { email, token } = JSON.parse(raw) as {
        email?: string;
        token?: string;
      };
      if (email && token)
    ...
    if (introEmail && introToken)
      return { "x-student-email": introEmail, "x-student-token": introToken };
  } catch {
    /* ignore */
  }
  return {};
      ...
      <Layout>
        <div className="min-h-[60vh] flex flex-col items-center justify-center text-center gap-4 px-4">
          ...
          <p className="text-muted-foreground max-w-sm">
            Hemingway AI Reading is available in the Intro and Comprehensive
            plans.
          </p>
          <p
            className="text-muted-foreground/60 text-sm max-w-sm"
            dir="rtl"
            lang="ar"
          >
...
function LevelPicker({
  ...
  return (
    <div
      ...
      style={{
        background: `linear-gradient(160deg, #0d0a22 0%, ${NAVY} 50%, #1a1238 100%)`,
      }}
    >
      <header
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (attemptsData.attempts) {
        ...
        for (const a of attemptsData.attempts as {
          level: string;
          type: string;
        }[]) {
          if (a.level === level) {
      ...
      <main className="flex-1 px-4 pt-6 pb-10 flex flex-col items-center gap-6">
        <div className="text-center space-y-2">
          ...
          <p className="text-white/55 text-sm">
            5 passages each. Complete all 5 to see your bucket score.
          </p>
        </div>
            ...
            {types.map((t) => {
              ...
              return (
                ...
                >
                  ...
                  <p className="text-xs text-white/55 leading-relaxed min-h-[32px]">
                    {t.tagline}
                  </p>
                  <div className="mt-1">
...
function ReadingPlayer({
  ...
  const [bucketDoneSlugs, setBucketDoneSlugs] = useState<Set<string>>(
    new Set(),
  );
    ...
    (async () => {
      try {
        const res = await fetch(
          `${BASE_URL}/api-ielts/reading/items?level=${level}&type=${type}`,
          {
            headers: getAuthHeaders(),
          },
        );
        ...
        setBucketDoneSlugs(
          new Set(list.filter((i) => i.completed).map((i) => i.slug)),
        );
        const next =
          list.find((i) => !i.completed)?.slug ?? list[0]?.slug ?? null;
        setActiveSlug(next);
      } catch {
  ...
  const loadItem = useCallback(async (s: string) => {
    ...
    setAnswers({});
    try {
      const res = await fetch(`${BASE_URL}/api-ielts/reading/items/${s}`, {
        headers: getAuthHeaders(),
      });
      const data = await res.json();
      if (!res.ok) {
  ...
  const submit = async () => {
    ...
    try {
      const res = await fetch(
        `${BASE_URL}/api-ielts/reading/items/${activeSlug}/submit`,
        {
          method: "POST",
  ...
  const positionLabel =
    activeSlug && bucketSlugs.length > 0
    ...
    >
      ...
      <main className="flex-1 px-4 py-6 flex flex-col items-center">
        {loading ? (
          ...
          <div
            className="max-w-2xl w-full p-5 rounded-2xl text-sm text-red-300"
            style={{
          ...
          <ItemView
            ...
            isLastUnsolved={
              ...
              new Set([...bucketDoneSlugs, activeSlug]).size ===
                bucketSlugs.length
            }
          />
        ) : null}
...
function ItemView({
  ...
  return (
    <div className="w-full max-w-3xl flex flex-col gap-5">
      ...
      {(item.type === "matching_headings" ||
        item.type === "matching_features") &&
      ...
      <div
        className="sticky bottom-0 -mx-4 px-4 py-4 flex items-center gap-3 justify-end"
        style={{
    ...
    return (
      ...
      >
        {item.paragraphs.map((p) => (
          <p key={p.label}>
            <span className="font-black mr-2" style={{ color: VIOLET }}>
              {p.label}.
            </span>
            {p.text}
          </p>
      ...
      <div className="flex items-start gap-3">
        <div
          ...
          style={{
            ...
            color: result ? (result.isCorrect ? SOFT_GREEN : RED) : VIOLET,
          }}
        >
          {index}
        </div>
        ...
        >
          {!result.isCorrect && (
            <div className="text-white/85 mb-1">
              <span className="font-bold" style={{ color: SOFT_GREEN }}>
                Correct:
              </span>{" "}
...
function QuestionInput({
  ...
  if (
    ...
    return (
      <RadioGroup
        options={sub.mcqOptions.map((opt, i) => ({
          value: i,
          label: opt,
  ...
  if (item.type === "matching_headings" || item.type === "matching_features") {
    ...
    return (
      ...
      >
        <option value="" style={{ background: NAVY }}>
          — select —
        </option>
        {item.options.map((opt, i) => (
    ...
    <div className={compact ? "flex gap-2 flex-wrap" : "space-y-2"}>
      {options.map((opt) => {
        ...
        return (
          ...
          >
            {opt.prefix && (
              <span className="font-black mr-1" style={{ color: VIOLET }}>
                {opt.prefix}.
              </span>
            )}
            <span>{opt.label}</span>
...
function ReadingSummary({
  ...
  const totalScore = completedItems.reduce(
    (acc, i) => acc + (i.result?.score ?? 0),
    0,
  );
  const totalQuestions = completedItems.reduce(
    (acc, i) => acc + (i.result?.total ?? 0),
    0,
  );
  const avgPercent =
        ...
        {loading ? (
          ...
          <div className="w-full max-w-2xl flex flex-col gap-6">
            ...
            >
              <Trophy
                className="w-12 h-12"
                style={{ color: isPerfect ? SOFT_GREEN : YELLOW }}
              />
              <div className="text-center">
                ...
                <div
                  ...
                  style={{ color: isPerfect ? SOFT_GREEN : VIOLET }}
                >
                  {avgPercent}% across {completedItems.length} of {items.length}{" "}
                  passages
                </div>
              </div>
            </div>
            ...
            >
              <div className="text-[10px] font-black tracking-[0.2em] uppercase text-white/40 mb-1">
                Per passage
              </div>
              {items.map((it, idx) => (
                <div
                ...
                >
                  <div className="flex items-center gap-3">
                    <div
                      ...
                      style={{
                        background: it.result
                          ? "rgba(167,139,250,0.18)"
                          : "rgba(255,255,255,0.05)",
                        color: it.result ? VIOLET : "rgba(255,255,255,0.3)",
                      }}
                    >
                      {idx + 1}
                    </div>
