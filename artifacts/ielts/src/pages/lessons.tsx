...
export default function Lessons() {
  ...
  useEffect(() => {
    try {
      if (selectedCourse)
        sessionStorage.setItem(SELECTED_COURSE_KEY, selectedCourse);
  ...
  const fetchLessons = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch("/api-ielts/lessons", {
    ...
    return (
      <Layout>
        <div className="space-y-5">
          ...
          <div
            className={cn(
              "relative overflow-hidden rounded-3xl p-6 sm:p-7 text-white bg-gradient-to-br",
              meta.gradient,
            )}
          >
            ...
            <div className="relative">
              ...
              <h1 className="text-2xl sm:text-3xl font-extrabold leading-tight mb-1.5">
                {meta.title}
              </h1>
          ...
          {totalCount === 0 && (
            <div className="bg-card border border-dashed border-border rounded-3xl p-10 text-center space-y-3">
              ...
              <p className="text-sm text-muted-foreground max-w-md mx-auto">
                The lessons for <b>{meta.title}</b> are being prepared. Check
                back here shortly — they'll appear automatically as soon as your
                instructor adds them.
              </p>
              <p className="text-xs text-muted-foreground" dir="rtl" lang="ar">
                قريباً — سيتم إضافة الدروس قريباً
              </p>
            </div>
          )}
