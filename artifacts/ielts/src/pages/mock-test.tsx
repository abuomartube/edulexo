...
import {
  readingTests,
  calculateBand as calculateReadingBand,
  type ReadingTest,
  ...
  return (
    <div className="flex items-center gap-2">
      <Timer
        className={cn(
          "w-4 h-4",
          urgent ? "text-red-500 animate-pulse" : "text-primary",
        )}
      />
      <span
        className={cn(
      ...
      <div className="bg-card border border-border rounded-2xl p-4 space-y-3">
        ...
        {part.tableHtml && (
          <div
            ...
            dangerouslySetInnerHTML={{ __html: part.tableHtml }}
          />
        )}
            ...
            {sec.type === "multiSelect" &&
              sec.questions.map((q) => {
                ...
                const toggle = (label: string) => {
                  ...
                  else if (set.size < 2) set.add(label);
        ...
        <div className="bg-card border border-border rounded-2xl p-4 lg:sticky lg:top-36 lg:max-h-[70vh] lg:overflow-y-auto space-y-3">
          ...
          {passage.paragraphs.map((p, i) => (
            <div key={i} className="text-sm text-foreground leading-relaxed">
              {p.label && (
                <span className="font-bold text-primary mr-1">{p.label}</span>
              )}
              {p.text}
            </div>
          ))}
        ...
        <div className="space-y-4">
          ...
          {passage.questionSections.map((sec, si) => (
            ...
            >
              ...
              {sec.type === "fill" &&
                sec.questions.map((q) => (
                  <div key={q.num} className="flex items-start gap-2">
                    <span className="text-xs font-bold text-primary w-6 mt-1">
                      {q.num}.
                    </span>
                    <span className="text-sm text-foreground flex-1">
                      {q.text}
              ...
              {sec.type === "tfng" &&
                sec.questions.map((q) => {
                  const opts = isYesNo(sec.instruction)
                    ? ["YES", "NO", "NOT GIVEN"]
              ...
              {(sec.type === "mcParagraph" || sec.type === "mcMatch") &&
                sec.questions.map((q) => (
                  <div key={q.num} className="space-y-1">
...
function WritingSection({
  ...
  mockSet: MockTestSet;
  task1Text: string;
  setTask1Text: (v: string) => void;
  task2Text: string;
  setTask2Text: (v: string) => void;
  activeTask: 1 | 2;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="relative">
        ...
        <div
          className={cn(
            "absolute bottom-3 right-3 text-xs font-bold rounded-full px-2 py-0.5",
            count(text) >= minWords
    ...
    <div className="max-w-2xl mx-auto space-y-6 animate-in fade-in duration-500">
      ...
      <div className="bg-card border border-border rounded-2xl p-5 space-y-3">
        ...
        <div className="space-y-2 text-sm">
          ...
          <div className="border-t border-border pt-2 flex items-center gap-3">
            ...
            <span className="font-extrabold text-primary w-8 text-right">
              {overall.toFixed(1)}
            </span>
          </div>
        </div>
      ...
      <div className="bg-muted/30 border border-border rounded-2xl p-4 text-sm text-muted-foreground space-y-2">
        ...
        <ul className="list-disc list-inside space-y-1">
          <li>
            Listening & Reading scores are auto-graded based on your answers
          </li>
          <li>Writing score is AI-estimated based on your essay analysis</li>
          <li>
            Speaking score is self-assessed — for a more accurate score, use
            Churchill AI
          </li>
          <li>
            This is an <span className="font-bold">estimate</span> — your actual
            IELTS score may vary
          </li>
        </ul>
      </div>
      ...
      for (const [text, taskType] of [
        ...
        try {
          ...
          if (res.ok) {
            ...
            const avg = [
              s.taskResponse?.band,
              s.coherenceCohesion?.band,
              s.lexicalResource?.band,
              s.grammaticalRange?.band,
      ...
      <Layout>
        <div className="max-w-3xl mx-auto space-y-6 animate-in fade-in duration-300">
          <div className="bg-card border border-border rounded-3xl p-6 text-center space-y-4">
            ...
            <p className="text-muted-foreground">
              Complete all 4 skills with official timing
            </p>
            <p className="text-sm text-muted-foreground" dir="rtl" lang="ar">
              اختبار IELTS تجريبي كامل — ٤ مهارات بالتوقيت الرسمي
            </p>
          </div>
          ...
          <div className="bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800 rounded-2xl px-4 py-3 text-sm text-amber-700 dark:text-amber-400">
            <p className="font-bold">
              ⏱️ Total test time: approximately 2 hours 45 minutes
            </p>
              ...
              >
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-xs text-muted-foreground mt-0.5">
                      4 skills • 160 questions • ~2h 45min
                    </div>
                  </div>
      ...
      <div className="space-y-4 animate-in fade-in duration-200">
        ...
        <div className="max-w-5xl mx-auto">
          {currentSection === "listening" && (
            <ListeningSection
              test={listeningTests[listeningTestIdx]}
              answers={listeningAnswers}
              setAnswer={(n, v) =>
                setListeningAnswers((p) => ({ ...p, [n]: v }))
              }
          ...
          {currentSection === "reading" && (
            <ReadingSection
              test={readingTests[readingTestIdx]}
          ...
          {currentSection === "speaking" && (
            <div className="space-y-6">
              <SpeakingSection
                ...
                setSpeakingResponse={(i, v) =>
                  setSpeakingResponses((p) => ({ ...p, [i]: v }))
                }
                currentQ={currentSpeakingQ}
                setCurrentQ={setCurrentSpeakingQ}
              />
              <div className="max-w-2xl mx-auto bg-card border border-border rounded-2xl p-4 space-y-3">
                <h4 className="font-bold text-foreground text-sm">
                  Self-Assessment: Estimated Speaking Band
                </h4>
                <p className="text-xs text-muted-foreground">
                  Rate your speaking performance honestly. For AI-graded
                  speaking practice, use Churchill AI.
                </p>
                <div className="flex items-center gap-3">
                  <input
                    type="range"
