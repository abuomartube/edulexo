...
function useActivityStats(activity: string) {
  return useQuery<{ known: number; unknown: number }>({
    ...
    queryFn: async () => {
      try {
        const data = await customFetch<ActivityPosition>(
          `/api-ielts/activity-position/${activity}`,
        );
...
export default function ProgressPage() {
  ...
  const { data: levelStats, isLoading: statsLoading } =
    useGetFlashcardLevelStats();
  ...
  const resetMutation = useMutation({
    mutationFn: () =>
      customFetch("/api-ielts/progress/reset", { method: "DELETE" }),
    onSuccess: () => {
  ...
  const storiesPct =
    storiesTotal > 0 ? Math.round((storiesCompleted / storiesTotal) * 100) : 0;
      ...
      <div className="space-y-8 animate-in fade-in duration-500">
        <div>
          ...
          <p className="text-muted-foreground mt-2">
            Track your progress across all IELTS preparation features.
          </p>
        </div>
                ...
                <div className="flex items-center gap-4 mb-4">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-muted-foreground">
                      {storiesPct === 100
                        ? "All stories completed! Amazing work!"
                        : storiesCompleted > 0
                          ? `You've read ${storiesPct}% of the stories. Keep going!`
          ...
          <div className="border-t border-border pt-8 mt-4">
            {!showResetConfirm ? (
              ...
              <div className="bg-destructive/5 border border-destructive/20 rounded-2xl p-6 max-w-md mx-auto text-center space-y-4">
                ...
                <p className="text-sm text-muted-foreground">
                  This will permanently erase all your progress, bookmarks, and
                  review history. You cannot undo this.
                </p>
