...
async function speak(text: string) {
  ...
  try {
    ...
    void audio.play().catch(() => {
      /* ignore — user gesture or network issue */
    });
  } catch {
    /* swallow — silent fail keeps quiz UX smooth */
  }
}
...
export default function Quiz() {
  ...
  const {
    ...
    isLoading: mcLoading,
    refetch: refetchMc,
  } = useQuiz(level === "ALL" ? undefined : level, mcCount);
  const {
    data: fbQuestions,
    isLoading: fbLoading,
    refetch: refetchFb,
  } = useFillBlank(level === "ALL" ? undefined : level, fbCount);

  const rawQuestions =
    mode === "multiple-choice" ? (mcQuestions ?? []) : (fbQuestions ?? []);
  const questions = rawQuestions.filter((q: any) =>
    isLevelAllowed(q?.flashcard?.level),
  );
  ...
  const handleMcSelect = (idx: number) => {
    if (selectedOption !== null || !("correctIndex" in (currentQ as any)))
      return;
  ...
  const checkFillAnswer = () => {
    ...
    const q = currentQ as any;
    const correct =
      fillAnswer.trim().toLowerCase() === q.flashcard.english.toLowerCase();
        ...
        <div className="max-w-xl mx-auto mt-8 animate-in fade-in duration-500">
          ...
          <div className="text-center mb-10">
            ...
            <p className="text-muted-foreground">
              Test your vocabulary with multiple choice or fill-in-the-blank
              questions.
            </p>
          </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                ...
                >
                  ...
                  <div className="font-semibold text-foreground text-sm flex items-center gap-1.5">
                    Spell It
                    <span className="text-[10px] uppercase font-bold bg-emerald-500 text-white px-1.5 py-0.5 rounded">
                      NEW
                    </span>
                  </div>
                  <div className="text-xs text-muted-foreground mt-1">
                    Hear the word, then race the clock to spell it letter by
                    letter
                  </div>
                </Link>
              </div>
                      ...
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                          <div className="text-sm font-semibold text-foreground truncate">
                            {s.mode === "multiple-choice"
                              ? "Multiple Choice"
                              : "Fill in the Blank"}{" "}
                            · {s.level === "ALL" ? "All" : s.level}
                          </div>
        ...
        <div className="max-w-lg mx-auto mt-12 animate-in fade-in zoom-in duration-500">
          <div className="bg-card border border-border rounded-3xl p-10 text-center shadow-lg">
            ...
            <p className="text-muted-foreground mb-8">
              You answered {results.total} questions.
            </p>
  ...
  const exitToSetup = () => {
    if (results.total > 0) {
      const ok = window.confirm(
        "Leave this quiz? Your progress will not be saved.",
      );
      if (!ok) return;
    }
    setStarted(false);
      ...
      <div className="max-w-2xl mx-auto animate-in fade-in duration-500">
        ...
        {isLoading ? (
          ...
          <div className="text-center py-12 text-muted-foreground">
            No questions available for the selected level.
          </div>
        ) : mode === "multiple-choice" ? (
          <MultipleChoiceQuestion
            ...
            onNext={goNext}
            onMarkWeak={() => {
              addWeakWords([(currentQ as any).flashcard.id]);
              wrongIdsRef.current.push((currentQ as any).flashcard.id);
            }}
          />
        ) : (
          <FillBlankQuestion
    ...
    <div className="bg-card border border-border rounded-3xl p-8 shadow-sm animate-in fade-in duration-300">
      <div className="text-center mb-8">
        ...
        {question.flashcard.exampleSentence && (
          <p className="text-muted-foreground italic mt-3 text-sm">
            {question.flashcard.exampleSentence}
          </p>
        )}
      </div>
...
function FillBlankQuestion({
  ...
  return (
    <div className="bg-card border border-border rounded-3xl p-8 shadow-sm animate-in fade-in duration-300">
      <p className="text-sm font-medium text-muted-foreground mb-4 uppercase tracking-widest text-center">
        Fill in the blank
      </p>
      ...
      <div className="text-center mb-2">
        ...
        <p className="text-xl arabic-text font-bold text-primary">
          {question.flashcard.arabic}
        </p>
      </div>

      {question.flashcard.exampleSentenceArabic && (
        <p className="text-sm text-muted-foreground arabic-text text-right mb-6 px-2">
          {question.flashcard.exampleSentenceArabic}
        </p>
      )}
      ...
      {checked && (
        <div className="space-y-4">
          {correct ? (
            ...
            <div className="text-center text-red-600 font-bold text-lg">
              ❌ The answer was:{" "}
              <span className="underline">{question.flashcard.english}</span>
            </div>
          )}
          <Button onClick={onNext} className="w-full rounded-full">
            Next Question <ArrowRight className="w-4 h-4 ml-2" />
