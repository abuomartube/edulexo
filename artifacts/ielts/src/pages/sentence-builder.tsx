...
function performanceRating(report: SessionReport): {
  label: string;
  color: string;
  emoji: string;
} {
  ...
  const accuracy =
    report.totalWords > 0 ? report.firstAttemptCorrect / report.totalWords : 0;
  const score = avg * 0.7 + accuracy * 9 * 0.3;
  if (score >= 7.5)
    return { label: "Excellent", color: "text-green-600", emoji: "🎉" };
  if (score >= 6.5)
    return { label: "Strong", color: "text-emerald-600", emoji: "💪" };
  if (score >= 5.5)
    return { label: "Good Progress", color: "text-primary", emoji: "👍" };
  if (score >= 4.5)
    return { label: "Keep Practising", color: "text-amber-600", emoji: "📚" };
...
export default function SentenceBuilder() {
  ...
  const progressPct =
    words.length > 0 ? Math.round((index / words.length) * 100) : 0;

  async function loadHistory() {
    setLoadingHistory(true);
    try {
      const data = await customFetch<HistoryRow[]>(
        "/api-ielts/sentence-sessions",
      );
      setHistory(data);
    } catch {
      setHistory([]);
  ...
  async function startSession() {
    ...
    try {
      const all = await customFetch<Flashcard[]>(
        `/api-ielts/flashcards?level=${level}`,
      );
  ...
  async function analyze() {
    ...
    setErr(null);
    try {
      const data = await customFetch<CheckResult | { error: string }>(
        "/api-ielts/sentence-check",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  async function saveReport(
    allItems: SentenceItem[],
    endedEarly: boolean,
  ): Promise<SessionReport | null> {
    if (allItems.length === 0) return null;
    setSavingReport(true);
    try {
      const saved = await customFetch<SessionReport>(
        "/api-ielts/sentence-sessions",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  function buildLocalReport(
    ...
    const firstAttemptCorrect = allItems.filter(
      (i) => i.firstAttemptCorrect,
    ).length;
    return {
      id: 0,
      level,
      ...
      avgVocabBand: total
        ? allItems.reduce((s, i) => s + i.vocabBand, 0) / total
        : 0,
      avgGrammarBand: total
        ? allItems.reduce((s, i) => s + i.grammarBand, 0) / total
        : 0,
      commonMistakes: [],
      items: allItems,
      endedEarly,
  ...
  const highlightedSentence = useMemo(() => {
    ...
    const idx = original
      .toLowerCase()
      .indexOf(result.errorHighlight.toLowerCase());
        ...
        <div className="max-w-xl mx-auto mt-8 animate-in fade-in duration-300">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-muted-foreground">
              Write your own sentences and get instant AI feedback to improve
              your vocabulary and grammar.
            </p>
            <p
              className="text-sm text-muted-foreground mt-2 font-cairo"
              dir="rtl"
            >
          ...
          {openReport ? (
            ...
            <div className="bg-card border border-border rounded-3xl p-6 sm:p-8 shadow-sm">
              <h2 className="text-xl font-extrabold flex items-center gap-2 mb-4">
                <History className="w-5 h-5 text-primary" /> Your Sentence
                Builder History
              </h2>
              ...
              {loadingHistory ? (
                ...
                <div className="text-center py-10 text-sm text-muted-foreground">
                  No past sessions yet. Finish a session to see your report
                  here.
                </div>
              ) : (
                <div className="space-y-2">
                  {history.map((h) => {
                    const accuracy =
                      h.totalWords > 0
                        ? Math.round(
                            (h.firstAttemptCorrect / h.totalWords) * 100,
                          )
                        : 0;
                    return (
                      <button
                        ...
                        onClick={async () => {
                          try {
                            const r = await customFetch<SessionReport>(
                              `/api-ielts/sentence-sessions/${h.id}`,
                            );
                            setOpenReport(r);
                          } catch {}
                        }}
                        ...
                        <div className="flex items-center gap-3 min-w-0">
                          ...
                          <div className="min-w-0">
                            ...
                            <div className="text-xs text-muted-foreground flex items-center gap-1 mt-0.5">
                              ...
                              {new Date(h.completedAt).toLocaleString("en-GB", {
                                day: "numeric",
                                month: "short",
                                hour: "2-digit",
                                minute: "2-digit",
                              ...
                              Bands{" "}
                              {(
                                (h.avgVocabBand + h.avgGrammarBand) /
                                2
                              ).toFixed(1)}
                            </div>
                          </div>
      ...
      <div className="max-w-2xl mx-auto mt-6 animate-in fade-in duration-300">
        ...
        <div className="flex items-center justify-between mb-3 gap-3 flex-wrap">
          <span className="text-sm text-muted-foreground">
            Word {index + 1} of {words.length} · {level}
          </span>
          <button
        ...
        {confirmEnd && (
          <div className="fixed inset-0 bg-background/80 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
            <div className="bg-card border border-border rounded-3xl p-6 max-w-sm w-full shadow-xl">
              ...
              <p className="text-sm text-muted-foreground mb-5">
                You'll see your full report so far.{" "}
                {items.length === 0 && !result
                  ? "Nothing to save yet."
                  : `${items.length + (result ? 1 : 0)} sentence${items.length + (result ? 1 : 0) === 1 ? "" : "s"} will be saved.`}
              </p>
        ...
        {currentWord && (
          <div className="bg-card border border-border rounded-3xl p-6 sm:p-8 shadow-sm">
            ...
            <div
              className="text-xl font-bold text-primary font-cairo mb-2"
              dir="rtl"
            >
              {currentWord.arabic}
            </div>
            ...
            <div className="mt-6">
              <label className="text-sm font-semibold mb-2 block">
                ...
                <span className="text-primary">{currentWord.english}</span>:
              </label>
              <textarea
                value={sentence}
          ...
          <div className="mt-5 bg-card border border-border rounded-3xl p-6 sm:p-8 shadow-sm animate-in fade-in slide-in-from-bottom-2 duration-400">
            ...
            <div className="bg-primary/5 border border-primary/20 rounded-2xl p-4 mb-4">
              ...
              {result.arabicCorrected && (
                <p
                  className="text-sm font-cairo text-muted-foreground mt-2 pt-2 border-t border-primary/10"
                  dir="rtl"
                >
                  {result.arabicCorrected}
                </p>
...
function ReportView({
  ...
  const accuracy =
    report.totalWords > 0
      ? Math.round((report.firstAttemptCorrect / report.totalWords) * 100)
      : 0;
  const avgBand = ((report.avgVocabBand + report.avgGrammarBand) / 2).toFixed(
    1,
  );
            ...
            >
              <div className="flex items-center justify-between gap-2 mb-2">
                <div className="font-extrabold text-foreground flex items-center gap-2">
                  ...
                  {it.word}
                  <span
                    className="text-xs font-normal font-cairo text-primary"
                    dir="rtl"
                  >
                    {it.arabic}
                  </span>
              ...
              {it.corrected &&
                it.corrected.toLowerCase() !==
                  it.finalSentence.toLowerCase() && (
                  <>
                    <div className="flex items-center justify-between gap-2 mb-1">
                    ...
                    {it.arabicCorrected && (
                      <p
                        className="text-xs font-cairo text-muted-foreground"
                        dir="rtl"
                      >
                        {it.arabicCorrected}
                      </p>
