...
export default function SpellIt() {
  ...
  const drawNextCard = useCallback(
    (source: Flashcard[], seen: Set<number>): Flashcard | null => {
  ...
  const preloadCard = useCallback(
    (target: Flashcard, lvl: Level): Preload => {
      ...
      entry.promise = (async () => {
        ...
        try {
          ...
          const lettersOnly = target.english
            .split("")
  ...
  const revealAnswerImpl = useCallback(
    (
      correct: boolean,
      currentCard: Flashcard,
      reason: "correct" | "submit" | "timeout" = correct
        ? "correct"
        : "timeout",
    ) => {
      clearTimers();
  ...
  const beginCountdown = useCallback(
    (forCard: Flashcard, forSeq: number) => {
      ...
      revealTimerRef.current = setTimeout(() => {
        const currentLetters = inputRefs.current.map((el) =>
          (el?.value ?? "").toLowerCase(),
        );
        const target = forCard.english
          .split("")
          .filter((ch) => /[a-z]/i.test(ch))
          .map((c) => c.toLowerCase());
        const isCorrect =
          currentLetters.length === target.length &&
          currentLetters.every((l, i) => l === target[i]);
  ...
  const startCard = useCallback(
    (nextCard: Flashcard) => {
      ...
      const lettersOnly = nextCard.english
        .split("")
        .filter((ch) => /[a-z]/i.test(ch));
      ...
      const entry =
        preloadsRef.current.get(nextCard.id) ?? preloadCard(nextCard, level);
      setClue(entry.clue ?? null);
      if (!entry.clue) {
        entry.promise.then(() => {
        ...
        <div className="max-w-xl mx-auto mt-8 animate-in fade-in duration-300">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-muted-foreground">
              Listen to the word, the definition, and a hint — then race the
              clock to spell it correctly.
            </p>
            <p
              className="text-sm text-muted-foreground mt-2 font-cairo"
              dir="rtl"
            >
              استمع إلى الكلمة والتعريف والتلميح، ثم تسابق مع الوقت لتهجئتها
              بشكل صحيح.
            </p>
          </div>
  ...
  const showSpinner =
    audioLoading || (isReady && !audioReady && !audioErrored && !audioPlaying);
      ...
      <div className="max-w-xl mx-auto mt-4 sm:mt-6 animate-in fade-in duration-300">
        <div className="flex items-center justify-between mb-4 gap-2 flex-wrap">
          <div className="text-xs text-muted-foreground">
            <span className="font-bold text-foreground">{wordsCorrect}</span> /{" "}
            {wordsAttempted} correct · Level {level}
          </div>
          <button
            onClick={endSession}
        ...
        {card && (
          <div className="bg-card border-2 border-border rounded-3xl shadow-lg p-6 sm:p-8">
            ...
            <div className="flex justify-center mb-6">
              <button
                ...
                className={cn(
                  ...
                  isReady &&
                    !audioPlaying &&
                    "ring-2 ring-emerald-300 ring-offset-2 ring-offset-background animate-pulse",
                )}
              >
                {showSpinner ? (
                ...
                <span>
                  {showSpinner ? "Loading…" : audioPlaying ? "Stop" : "Hear It"}
                </span>
              </button>
            </div>
            ...
            {/* Letter boxes */}
            <div
              className="flex flex-wrap gap-1.5 sm:gap-2 justify-center mb-2"
              dir="ltr"
            >
              {slots.map((s, slotIdx) => {
                if (!s.isLetter) {
                ...
                return (
                  <input
                    key={slotIdx}
                    ref={(el) => {
                      inputRefs.current[letterIdx] = el;
                    }}
                    type="text"
                    inputMode="text"
                    autoComplete="off"
