...
function splitContent(text: string): {
  ...
  const MAX = 350;
  if (text.length <= MAX)
    return { firstEnd: text.length, chunk1: text, chunk2: "" };
  for (let i = MAX; i >= 80; i--) {
    if (text[i] === "." || text[i] === "!" || text[i] === "?") {
      const end = i + 1;
      return {
        firstEnd: end,
        chunk1: text.slice(0, end),
        chunk2: text.slice(end),
      };
    }
  }
...
function LevelBadge({ level }: { level: string }) {
  return (
    <span
      className={`text-xs font-bold px-2 py-0.5 rounded-full ${levelColors[level] ?? "bg-muted text-muted-foreground"}`}
    >
      {level}
    </span>
  );
...
function VoiceReader({ content }: { content: string }) {
  const { tokens, wordSpans } = useMemo(
    () => tokenizeForHighlight(content),
    [content],
  );
    ...
    const tick = () => {
      ...
      if (ctx && source && source.buffer) {
        ...
        const bufPos = Math.max(
          ...
          Math.min(
            dur,
            sourceStartOffsetRef.current +
              (ctx.currentTime - sourceStartCtxTimeRef.current),
          ),
        );
        ...
        if (found < 0) {
          ...
          for (let i = wordSpans.length - 1; i >= 0; i--) {
            if (wordSpans[i].end <= charPos) {
              found = wordSpans[i].idx;
              break;
            }
          }
        }
  ...
  useEffect(() => {
    ...
    const el = containerRef.current?.querySelector<HTMLSpanElement>(
      `[data-w-idx="${highlightIdx}"]`,
    );
  ...
  const prefetchOtherSpeeds = useCallback(
    (playingSpeed: SpeedValue, ctx: AudioContext) => {
      const others = (SPEEDS.map((s) => s.value) as SpeedValue[]).filter(
        (s) => s !== playingSpeed,
      );
  ...
  const handleSpeedChange = async (newSpeed: SpeedValue) => {
    ...
    const absoluteContentPos =
      chunkContentOffsetRef.current +
      currentBufferPos * fetchedSpeedRef.current;
    ...
    if (cached) {
      const safeOffset = Math.max(
        0,
        Math.min(newOffset, cached.duration - 0.1),
      );
  ...
  return (
    ...
    >
      ...
      {error && (
        <p className="px-4 pt-2 text-xs text-destructive font-medium">
          {error}
        </p>
      )}
...
function StoryReader({
  ...
  return (
    <div className="animate-in fade-in duration-300">
      ...
      <div
        className={`bg-card border-2 rounded-3xl overflow-hidden ${levelBorder[story.level]}`}
      >
        {/* Header */}
        <div className="p-6 md:p-8 border-b border-border">
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div>
              ...
              <p
                className="text-lg text-muted-foreground mt-1 font-medium"
                dir="rtl"
                lang="ar"
              >
                {story.titleArabic}
...
function StoryCard({
  ...
  return (
    ...
    >
      ...
      <p
        className="text-xs text-muted-foreground font-medium mb-3 font-cairo"
        dir="rtl"
        lang="ar"
      >
        {story.titleArabic}
...
export default function StoriesPage() {
  ...
  const { data: completedKeys = {} } = useQuery<Record<string, string>>({
    ...
    queryFn: async () => {
      const res = await customFetch<{ keys: Record<string, string> }>(
        "/api-ielts/user-data-prefix/story_completed_",
      );
      return res.keys;
    },
  });
  ...
  const markCompleteMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        ...
        description: "Great job finishing this story.",
        duration: 1500,
      });
    },
  });
    ...
    <Layout>
      <div className="max-w-5xl mx-auto animate-in fade-in duration-500">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Read and practise with stories built from your 3,000-word IELTS
              vocabulary — with Arabic translation
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-2 mb-6">
          {LEVELS.map((lvl) => {
            const count =
              lvl === "All" ? stories.length : (levelCounts[lvl] ?? 0);
