...
export default function Study() {
  const [levelFilter, setLevelFilter] = useState<ListFlashcardsLevel | "ALL">(
    "ALL",
  );
  ...
  const {
    ...
    save: savePosition,
    loadedRef,
  } = useActivityPosition("study", filtersKey);
  ...
  const { data: srsCards, isLoading: srsLoading } = useSrsDue(
  ...
  const cards = useMemo(() => {
    ...
    if (studyMode === "srs")
      return (srsCards ?? []).filter((c) => isLevelAllowed(c.level));
  ...
  const safeIndex =
    cards && cards.length > 0 ? Math.min(currentIndex, cards.length - 1) : 0;
  ...
  const currentProgress = useMemo(() => {
    ...
    return [...(progress ?? [])]
      .filter((p) => p.flashcardId === currentCard.id)
      .sort(
        (a, b) =>
  ...
  const progressPercent =
    cards && cards.length > 0
      ? Math.round((safeIndex / cards.length) * 100)
      : 0;

  useEffect(() => {
    if (welcomeBack && cards && cards.length > 0) {
      setWelcomeBack((prev) =>
        prev ? { ...prev, total: cards.length } : null,
      );
    }
        ...
        <div className="max-w-lg mx-auto mt-12 animate-in fade-in zoom-in duration-500">
          <div className="bg-card border border-border rounded-3xl p-10 text-center shadow-lg">
            ...
            <p className="text-muted-foreground mb-8">
              Great work reviewing {cards?.length ?? 0} cards today.
            </p>
        ...
        <div className="max-w-md mx-auto mt-16 animate-in fade-in zoom-in duration-500">
          <div className="bg-card border border-border rounded-3xl p-8 text-center shadow-lg">
            ...
            <p className="text-muted-foreground mb-6">
              You were on{" "}
              <span className="font-bold text-foreground">Card {pos + 1}</span>{" "}
              of {cards.length}. Would you like to continue where you left off?
            </p>
    ...
    <Layout>
      <div className="flex flex-col h-full max-w-2xl mx-auto animate-in fade-in duration-500">
        ...
        <div className="flex flex-col gap-4 mb-6">
          ...
          {/* Study Mode Tabs */}
          <div className="flex gap-2 flex-wrap">
            {(["all", "srs", "unknown", "bookmarks"] as StudyMode[]).map(
              (mode) => {
                const labels: Record<
                  StudyMode,
        ...
        {isLoading ? (
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-center p-8 bg-card rounded-2xl border border-dashed min-h-[400px]">
            ...
            <p className="text-muted-foreground">
              {studyMode === "srs"
                ? "You've reviewed all your due cards. Come back later!"
                : studyMode === "bookmarks"
                  ? "Bookmark cards from the Browse page to study them here."
                  : studyMode === "unknown"
                    ? "You know all the cards in this filter. Great job!"
                    ...
                    <button
                      onClick={() => {
                        if (!isWeak) {
                          ...
                          setMarkedWeakIds(
                            (prev) => new Set([...prev, currentCard.id]),
                          );
                          toast({
                            title: "Added to Weak Words",
                            description: `"${currentCard.english}" flagged for extra review.`,
                          });
                        }
                      }}
                      className={cn(
