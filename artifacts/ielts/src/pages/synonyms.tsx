...
import { useAddWeakWordByWord } from "@workspace/ielts-api-client-react";
...
export default function Synonyms() {
  ...
  const [markedWeakWords, setMarkedWeakWords] = useState<Set<string>>(
    new Set(),
  );
  ...
  const {
    ...
    save: savePosition,
    loadedRef,
  } = useActivityPosition("synonyms", skillFilter);

  const cards = useMemo(() => {
    if (skillFilter === "All Skills") return allSynonyms;
    return allSynonyms.filter(
      (c) => c.skill === skillFilter || c.skill === "All Skills",
    );
  }, [skillFilter]);
  ...
  const markCard = (known: boolean) => {
    ...
    toast({
      ...
      description: known
        ? "Marked as known."
        : "We'll show this card again soon.",
      duration: 1500,
    });
    handleNext();
  };
  ...
  if (sessionDone) {
    ...
    const grade =
      pct >= 90
        ? { label: "Excellent!", color: "text-green-600", icon: "🏆" }
        : pct >= 70
          ? { label: "Great work!", color: "text-teal-600", icon: "⭐" }
          : pct >= 50
            ? { label: "Good effort!", color: "text-amber-600", icon: "💪" }
      ...
      <div className="flex flex-col h-full max-w-2xl mx-auto animate-in fade-in duration-500">
        ...
        <div className="flex flex-col gap-3 mb-5">
          ...
          <p className="text-sm text-muted-foreground">
            {cards.length} cards · See the word → guess its synonym → flip to
            check
          </p>
        </div>
        ...
        {card ? (
          <div className="flex-1 flex flex-col">
            ...
            >
              ...
              >
                ...
                >
                  {/* Skill badge */}
                  <span
                    className={`text-[11px] font-bold uppercase tracking-widest px-3 py-1 rounded-full mb-8 ${skillColors[card.skill] ?? skillColors["All Skills"]}`}
                  >
                    {card.skill}
                  </span>
                  ...
                  <div className="mt-4 text-center">
                    <p className="text-muted-foreground text-base font-medium">
                      Do you know a synonym?
                    </p>
                    <p
                      className="text-muted-foreground text-sm mt-1 arabic-text"
                      dir="rtl"
                      lang="ar"
                    >
                      هل تعرف مرادفاً لهذه الكلمة؟
                    </p>
                  </div>
                ...
                >
                  ...
                  <div className="px-8 pt-7 pb-5 border-b border-primary-foreground/20">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <PronounceButton word={card.synonym} variant="inverted" />
                    </div>
                  </div>
                  ...
                  <div className="flex-1 px-8 py-5 flex flex-col justify-center gap-4">
                    ...
                    <div className="text-center">
                      ...
                      <p
                        className="text-2xl font-bold arabic-text"
                        dir="rtl"
                        lang="ar"
                      >
                        {card.translation}
                    ...
                    {/* Example */}
                    <div>
                      <p className="text-xs text-primary-foreground/50 uppercase tracking-widest mb-1.5">
                        Example · مثال
                      </p>
                      ...
                      <p
                        className="text-sm text-primary-foreground/75 leading-relaxed arabic-text text-right"
                        dir="rtl"
                        lang="ar"
                      >
                        «{card.exampleAr}»
            ...
            <div className="mt-6 flex flex-col gap-3">
              ...
              <div
                className={`flex gap-3 justify-center transition-opacity duration-300 ${isFlipped ? "opacity-100" : "opacity-0 pointer-events-none"}`}
              >
                <Button
                  variant="outline"
                  size="lg"
                      ...
                      <button
                        onClick={() => {
                          if (!isWeak) {
                            ...
                            setMarkedWeakWords(
                              (prev) => new Set([...prev, card.word]),
                            );
                            toast({
                              title: "Added to Weak Words",
                              description: `"${card.word}" flagged for extra review.`,
                            });
                          }
                        }}
                        className={cn(
