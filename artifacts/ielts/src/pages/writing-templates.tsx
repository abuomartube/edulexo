    ...
    <span>
      {parts.map((p, i) =>
        p.highlight ? (
          <mark
            key={i}
            className="bg-primary/15 text-primary font-semibold rounded px-0.5"
          >
            {p.text}
          </mark>
        ) : (
  ...
  return (
    <div className="space-y-2 mt-3">
      ...
      <div className="bg-green-50 dark:bg-green-950/20 border border-green-200 dark:border-green-800 rounded-xl px-4 py-3 text-sm leading-relaxed text-foreground">
        <HighlightedText
          text={sample.text}
          phrases={sample.highlightedPhrases}
        />
      </div>
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex flex-wrap gap-1.5">
              {step.sentenceStarters.map((s, i) => (
                <span
                  key={i}
                  className="inline-block bg-primary/10 text-primary text-xs font-medium rounded-lg px-2.5 py-1.5 italic"
                >
                  "{s}"
                </span>
              ))}
      ...
      {activeTab === "structure" && (
        <div className="space-y-3">
          ...
          <p className="text-center text-xs text-muted-foreground pt-2">
            💡 Click each step to see sentence starters and a Band 7-8 sample
            paragraph
          </p>
        </div>
      )}
        ...
        <div className="px-4 pb-4 space-y-2 animate-in fade-in slide-in-from-top-2 duration-200">
          {category.phrases.map((p, i) => {
            ...
            return (
              <div
                key={i}
                className="bg-muted/30 border border-border rounded-xl px-3 py-2.5 space-y-1"
              >
                <div className="flex items-center justify-between">
                ...
                <p className="text-xs text-foreground italic bg-muted/50 rounded-lg px-2.5 py-1.5">
                  "{p.example}"
                </p>
              </div>
            );
...
export default function WritingTemplatesPage() {
  ...
  const [activeSection, setActiveSection] = useState<"templates" | "linking">(
    "templates",
  );
  ...
  const totalPhrases = LINKING_PHRASES.reduce(
          ...
          <div className="flex justify-center gap-4 text-sm">
            <div className="bg-muted/50 rounded-xl px-4 py-2 text-center">
              <div className="text-2xl font-extrabold text-primary">
                {ESSAY_TEMPLATES.length}
              </div>
              <div className="text-xs text-muted-foreground">Templates</div>
            </div>
            <div className="bg-muted/50 rounded-xl px-4 py-2 text-center">
              <div className="text-2xl font-extrabold text-primary">
                {LINKING_PHRASES.length}
              </div>
