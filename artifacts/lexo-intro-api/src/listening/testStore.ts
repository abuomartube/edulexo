...
import { listeningTests } from "@workspace/lexo-intro-db/schema";
...
export async function ensureListeningTestsTable(): Promise<void> {
  try {
    await db.execute(sql`
      CREATE TABLE IF NOT EXISTS intro_listening_tests (
    ...
    `);
    await db.execute(sql`
      CREATE UNIQUE INDEX IF NOT EXISTS intro_listening_tests_slug_uniq ON intro_listening_tests (slug)
    `);
  } catch (err) {
