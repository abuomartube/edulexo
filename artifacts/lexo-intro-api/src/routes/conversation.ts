...
router.post("/sessions", async (req, res) => {
  ...
  try {
    ...
    const inserted = await db.execute(sql`
      INSERT INTO intro_conversation_sessions (student_id, topic, topic_source, mode, messages, feedback, band_score, duration_seconds)
      VALUES (
        ${tok.id},
        ${topic.trim()},
...
router.get("/sessions", async (req, res) => {
  ...
  try {
    const result = await db.execute(sql`
      ...
      FROM intro_conversation_sessions
...
router.get("/sessions/:id", async (req, res) => {
  ...
  try {
    ...
    const result = await db.execute(sql`
      SELECT * FROM intro_conversation_sessions
