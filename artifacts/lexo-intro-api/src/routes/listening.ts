...
import { listeningAttempts } from "@workspace/lexo-intro-db/schema";
import { and, desc, eq } from "drizzle-orm";
import { sql } from "drizzle-orm";
import { openai } from "@workspace/lexo-intro-integrations-openai";
...
async function ensureTable() {
  try {
    await db.execute(sql`
      CREATE TABLE IF NOT EXISTS intro_listening_attempts (
    ...
    `);
    await db.execute(sql`
      CREATE UNIQUE INDEX IF NOT EXISTS intro_listening_attempts_student_test_uniq
      ON intro_listening_attempts (student_id, test_id)
    `);
  } catch (err) {
