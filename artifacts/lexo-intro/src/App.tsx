...
function LeaveCommentModal({ onDone }: { onDone: () => void }) {
  ...
  const submit = async () => {
    ...
    setSubmitting(true);
    try {
      await fetch(`${BASE_URL}/api-intro/auth/comments`, {
        method: "POST",
  ...
  return (
    ...
    >
      <div
        ...
        style={{ background: `linear-gradient(160deg, #252861 0%, ${NAVY} 100%)`, border: "1px solid rgba(107,47,230,0.25)", boxShadow: "0 24px 60px rgba(0,0,0,0.6)" }}
      >
        <button
          onClick={onDone}
        ...
        {done ? (
          ...
          <>
            ...
            <input
              ...
              style={{ background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.1)", ["--tw-ring-color" as string]: TEAL } as React.CSSProperties}
            />
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                style={{ background: `linear-gradient(135deg, ${TEAL}, #4F7FFF)` }}
              >
...
export default function App() {
  ...
  const checkAuth = useCallback(async () => {
    try {
      const res = await fetch(`${BASE_URL}/api-intro/auth/me`, { credentials: "include" });
  ...
  if (screen === "loading") {
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ background: "#1E2155" }}>
        <div className="w-8 h-8 border-2 rounded-full animate-spin" style={{ borderColor: "#6B2FE6", borderTopColor: "transparent" }} />
      </div>
    );
  }
