...
function AdminLogin({ onLogin }: { onLogin: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      const res = await fetch(`${BASE_URL}/api-intro/auth/admin/login`, {
        method: "POST",
...
function PasswordSection() {
  ...
  const changePassword = async (type: "student" | "admin", newPassword: string) => {
    ...
    setSuccess(null);
    try {
      const res = await fetch(`${BASE_URL}/api-intro/auth/admin/password`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
...
export default function Admin({ onBack }: Props) {
  ...
  const checkAdmin = useCallback(async () => {
    try {
      const res = await fetch(`${BASE_URL}/api-intro/auth/admin/me`, { credentials: "include" });
      const data = await res.json();
  ...
  const fetchStudents = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`${BASE_URL}/api-intro/auth/admin/students`, { credentials: "include" });
      const data = await res.json();
  ...
  const fetchFeedback = useCallback(async () => {
    ...
    try {
      const res = await fetch(`${BASE_URL}/api-intro/auth/admin/feedback`, { credentials: "include" });
      const data = await res.json();
  ...
  const fetchComments = useCallback(async () => {
    ...
    try {
      const res = await fetch(`${BASE_URL}/api-intro/auth/admin/comments`, { credentials: "include" });
      const data = await res.json();
  ...
  const updateCommentStatus = async (id: number, status: string) => {
    try {
      await fetch(`${BASE_URL}/api-intro/auth/admin/comments/${id}/status`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
  ...
  const deleteComment = async (id: number) => {
    ...
    try {
      await fetch(`${BASE_URL}/api-intro/auth/admin/comments/${id}`, {
        method: "DELETE",
        credentials: "include",
      });
      fetchComments();
  ...
  const fetchCodes = useCallback(async () => {
    ...
    try {
      const res = await fetch(`${BASE_URL}/api-intro/auth/admin/access-codes`, { credentials: "include" });
      const data = await res.json();
  ...
  const generateCodes = async () => {
    ...
    try {
      await fetch(`${BASE_URL}/api-intro/auth/admin/access-codes`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
  ...
  const deleteCode = async (code: string) => {
    ...
    try {
      await fetch(`${BASE_URL}/api-intro/auth/admin/access-codes/${code}`, {
        method: "DELETE",
        credentials: "include",
      });
      fetchCodes();
  ...
  const updateStatus = async (id: number, status: string) => {
    setActionLoading(id);
    try {
      await fetch(`${BASE_URL}/api-intro/auth/admin/students/${id}/status`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
        body: JSON.stringify({ status }),
  ...
  const deleteStudent = async (id: number) => {
    ...
    setActionLoading(id);
    try {
      await fetch(`${BASE_URL}/api-intro/auth/admin/students/${id}`, {
        method: "DELETE",
        credentials: "include",
      });
      fetchStudents();
  ...
  const setExpiration = async (id: number, expiresAt: string | null) => {
    setActionLoading(id);
    try {
      await fetch(`${BASE_URL}/api-intro/auth/admin/students/${id}/expiration`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
  ...
  const bulkUpdateStatus = async (status: string) => {
    ...
    setBulkLoading(true);
    try {
      await fetch(`${BASE_URL}/api-intro/auth/admin/students/bulk-status`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
  ...
  const logout = async () => {
    await fetch(`${BASE_URL}/api-intro/auth/admin/logout`, { method: "POST", credentials: "include" });
    setIsAdmin(false);
    onBack();
  };
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: `linear-gradient(160deg, #071422 0%, ${NAVY} 40%, #0C2040 100%)` }}>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-5 max-w-3xl mx-auto w-full">
        ...
        <div className="grid grid-cols-6 gap-2 mb-2">
          {([
            ...
            <button
              ...
              style={
                adminTab === t.key
                  ? { background: `rgba(107,47,230,0.15)`, border: `1px solid rgba(107,47,230,0.35)`, color: TEAL }
        ...
        <div className="flex items-center gap-2">
          <a
            href={`${BASE_URL}/api-intro/auth/admin/students/export/excel`}
          ...
          <a
            href={`${BASE_URL}/api-intro/auth/admin/students/export/pdf`}
            className="flex-1 flex items-center justify-center gap-2 px-3 py-2.5 rounded-xl text-xs font-bold transition-all hover:scale-[1.02] active:scale-[0.98]"
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        <span
                          ...
                          style={{
                            background: f.tool === "speaking" ? "rgba(107,47,230,0.15)" : "rgba(245,197,24,0.15)",
                            color: f.tool === "speaking" ? TEAL : YELLOW,
                            border: `1px solid ${f.tool === "speaking" ? "rgba(107,47,230,0.3)" : "rgba(245,197,24,0.3)"}`,
                          }}
                        >
                          {f.tool}
                        </span>
