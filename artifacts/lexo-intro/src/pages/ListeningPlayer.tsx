...
export default function ListeningPlayer({ testId, onBack, onLogout, expiresAt, onSubmitted }: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      try {
        const res = await fetch(`${BASE_URL}/api-intro/listening/tests/${testId}`, { credentials: "include" });
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`${BASE_URL}/api-intro/listening/tests/${test.id}/audio`, { credentials: "include" });
        const data = await res.json();
        if (cancelled) return;
        if (!res.ok) {
  ...
  const submit = async () => {
    ...
    setSubmitError(null);
    try {
      const res = await fetch(`${BASE_URL}/api-intro/listening/tests/${test.id}/submit`, {
        method: "POST",
