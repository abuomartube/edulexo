...
export default function ReadingTypePicker({ level, onBack, onLogout, expiresAt, onChooseType, onSeeSummary }: Props) {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const metaRes = await fetch(`${BASE_URL}/api-intro/reading/levels-types`, { credentials: "include" });
      const meta = await metaRes.json();
      if (meta.types) setTypes(meta.types);
      if (meta.counts) setCounts(meta.counts);

      const attemptsRes = await fetch(`${BASE_URL}/api-intro/reading/attempts`, { credentials: "include" });
