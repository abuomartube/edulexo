...
import { useAudioPlayback } from "@workspace/lexo-intro-integrations-openai-react/audio";
...
class SpeakingErrorBoundary extends Component<{ children: ReactNode; onBack: () => void }, { hasError: boolean; errorMsg: string; errorStack: string }> {
  ...
  componentDidCatch(error: unknown, errorInfo: { componentStack?: string }) {
    console.error("[Churchill] Speaking crash:", error);
    if (errorInfo?.componentStack) console.error("[Churchill] Component stack:", errorInfo.componentStack);
  ...
  render() {
    if (this.state.hasError) {
      return (
        <div className="min-h-screen flex flex-col items-center justify-center px-4" style={{ background: "linear-gradient(160deg, #071422 0%, #1E2155 40%, #0C2040 100%)" }}>
          <div className="text-center space-y-4 max-w-sm">
            ...
            <div className="space-y-2">
              <button onClick={() => this.setState({ hasError: false, errorMsg: "", errorStack: "" })} className="w-full py-3 rounded-2xl font-bold text-sm transition-all hover:opacity-90" style={{ background: "linear-gradient(135deg, #6B2FE6, #8B5FF6)", color: "#1E2155" }}>
                Try Again
              </button>
...
const VOICE_INTRO = "Welcome to Churchill. I am your personal IELTS Speaking Examiner. Please relax, speak naturally, and let's begin your practice session.";
const GOODBYE_MESSAGE = "This has been Churchill. Thank you for practising today. Keep going, stay consistent, and your band score will improve. Goodbye and good luck.";
...
async function callMessageStream(messages: Message[], topic: string, part: number, questionNum: number, isStart: boolean, onChunk: (t:string)=>void): Promise<string> {
  const res = await fetch(`${BASE_URL}/api-intro/message`, { method:"POST", headers:{"Content-Type":"application/json"}, body: JSON.stringify({ messages, topic, part, questionNum, isStart }) });
...
async function callReport(messages: Message[], topic: string): Promise<ReportData> {
  const res = await fetch(`${BASE_URL}/api-intro/report`, { method:"POST", headers:{"Content-Type":"application/json"}, body: JSON.stringify({ messages, topic }) });
...
function Part2CueCard({ topic }: { topic: string }) {
  ...
  return (
    <div className="rounded-2xl p-5 space-y-3" style={{ background:`rgba(107,47,230,0.08)`, border:`2px solid rgba(107,47,230,0.3)` }}>
...
function FinalReport({ report, topic, onNewSession }: { report:ReportData; topic:string; onNewSession:()=>void }) {
  ...
  return (
    <div className="space-y-5">
      <div className="rounded-3xl p-6 text-center" style={{ background:`rgba(107,47,230,0.08)`, border:`2px solid rgba(107,47,230,0.3)` }}>
      ...
      <div className="rounded-2xl p-5" style={{ background:"rgba(255,255,255,0.06)", border:"1px solid rgba(255,255,255,0.1)" }}>
        ...
        <div className="flex flex-wrap gap-2">{report.topVocab.map((w,i) => <span key={i} className="px-3 py-1 text-sm font-medium rounded-full" style={{ background:`rgba(107,47,230,0.12)`, color: TEAL, border:`1px solid rgba(107,47,230,0.3)` }}>{w}</span>)}</div>
      </div>
      ...
      {report.recommendation && (
        <div className="rounded-2xl p-5" style={{ background:`linear-gradient(135deg, rgba(107,47,230,0.1), rgba(0,212,240,0.05))`, border:`1px solid rgba(107,47,230,0.3)` }}>
      ...
      <button onClick={onNewSession} className="w-full flex items-center justify-center gap-2 py-3.5 rounded-2xl font-bold text-base transition-all hover:opacity-90" style={{ background:`linear-gradient(135deg,${TEAL},#8B5FF6)`, color: NAVY }}>
    ...
    <div className="fixed inset-0 z-50 flex flex-col" style={{ background:"#0D1B3E" }}>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {entries.map((e,i) => {
          ...
          return (
            <div key={i}>
              {isNewPart && <h3 className="text-sm font-bold mb-2 pb-1" style={{ color: TEAL, borderBottom:`1px solid rgba(107,47,230,0.2)` }}>{partLabels[e.part]}</h3>}
        ...
        {report && (
          <div className="rounded-2xl p-4 text-sm space-y-1.5" style={{ background:`rgba(107,47,230,0.07)`, border:`1px solid rgba(107,47,230,0.2)` }}>
...
function SpeakingInner({ onBack, expiresAt }: Props) {
  ...
  const playTts = useCallback(async (text: string): Promise<boolean> => {
    ...
    try {
      const res = await fetch(`${BASE_URL}/api-intro/tts`, { method:"POST", headers:{"Content-Type":"application/json"}, body: JSON.stringify({ text:clean, speed:ttsSpeedRef.current }) });
  ...
  const playStreamingTts = useCallback(async (text: string): Promise<void> => {
    ...
    try {
      ...
      const res = await fetch(`${BASE_URL}/api-intro/tts-stream`, {
        method: "POST",
  ...
  const processVoiceAnswer = useCallback(async (audioBlob: Blob) => {
    ...
    try {
      ...
      const res = await fetch(`${BASE_URL}/api-intro/voice-message`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          audio: base64,
  ...
  const startSession = useCallback(async (mode:SessionMode) => {
    ...
    try {
      ...
      setStreamingContent(null); setError("Could not connect to Churchill. Please try again.");
        ...
        <div className="w-full max-w-md space-y-6 text-center">
          ...
          <div className="space-y-3">
            <button
              ...
              style={{ background:`rgba(107,47,230,0.08)`, border:`2px solid rgba(107,47,230,0.35)` }}
            >
              <div className="w-12 h-12 rounded-xl flex items-center justify-center shrink-0" style={{ background:`linear-gradient(135deg,${TEAL},#8B5FF6)` }}>
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background:`linear-gradient(160deg, #071422 0%, ${NAVY} 40%, #0C2040 100%)` }}>
      ...
      <header className="shrink-0 flex items-center justify-between px-4 py-3" style={{ borderBottom: "1px solid rgba(107,47,230,0.15)" }}>
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              {sessionNumber > 0 && <span className="text-xs px-2 py-0.5 rounded-full" style={{ background:`rgba(107,47,230,0.15)`, color: TEAL }}>Session #{sessionNumber}</span>}
            </div>
      ...
      {(session.phase as string) !== "complete" && (session.phase as string) !== "report-loading" && (session.phase as string) !== "idle" && (
                ...
                <div className="flex gap-1">
                  {SPEED_OPTIONS.map(opt => (
                    <button
                      ...
                      style={ttsSpeed === opt.value ? { background:`rgba(107,47,230,0.25)`, color: TEAL } : { color:"rgba(255,255,255,0.3)", background:"rgba(255,255,255,0.05)" }}
                    >
                      {opt.label}
                    </button>
                  ))}
          ...
          {!session.partDone && session.phase !== "part2-prep" && sessionMode !== "voice" && (
            <>
              ...
              <button
                ...
                style={{ background:`linear-gradient(135deg,${TEAL},#8B5FF6)` }}
              >
