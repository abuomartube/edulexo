...
import {
  ArrowLeft, KeyRound, Languages, Mail, Loader2,
  CheckCircle2, AlertCircle, Bell, Megaphone, Eye, EyeOff,
} from "lucide-react";
...
function ChangePasswordSection({ t }: { t: (k: never) => string }) {
  ...
  async function onSubmit(e: FormEvent) {
    ...
    setState({ kind: "idle" });
    if (next.length < 8) {
      setState({ kind: "err", msg: (t as (k: string) => string)("settings.password.errShort") });
      return;
    }
    if (next !== confirm) {
      setState({ kind: "err", msg: (t as (k: string) => string)("settings.password.errMismatch") });
      return;
    }
    if (next === current) {
      setState({ kind: "err", msg: (t as (k: string) => string)("settings.password.errSame") });
      return;
    }
  ...
  return (
    ...
    >
      <SectionHeader icon={<KeyRound size={18} />} title={tt("settings.password.title")} subtitle={tt("settings.password.subtitle")} />
      <form onSubmit={onSubmit} className="mt-5 space-y-4">
        ...
        />
        {state.kind === "err" && (
          <p role="alert" data-testid="password-error" className="text-sm font-medium text-rose-600 dark:text-rose-400 bg-rose-50 dark:bg-rose-900/20 rounded-lg px-3 py-2 border border-rose-200 dark:border-rose-900/40">
            {state.msg}
          </p>
        )}
        {state.kind === "ok" && (
          <p role="status" data-testid="password-success" className="inline-flex items-center gap-2 text-sm font-medium text-emerald-700 dark:text-emerald-400">
        ...
        >
          {state.kind === "saving" && <Loader2 size={15} className="animate-spin" />}
          {state.kind === "saving" ? tt("settings.password.saving") : tt("settings.password.submit")}
        </button>
      </form>
    </section>
  );
...
function PasswordField({
  id, label, value, onChange, show, onToggleShow, autoComplete, required, hint, testId,
}: {
  id: string; label: string; value: string; onChange: (v: string) => void;
  show: boolean; onToggleShow: () => void;
  autoComplete: string; required?: boolean; hint?: string; testId?: string;
}) {
  return (
    <div>
      <label htmlFor={id} className="block text-xs font-bold uppercase tracking-wider text-slate-600 dark:text-slate-300 mb-1.5">
        {label}
      </label>
...
function LanguageSection({
  t, currentServer, currentClient, onChangeClient,
}: {
  t: (k: never) => string;
  currentServer: "en" | "ar";
  currentClient: "en" | "ar";
  ...
  async function pick(next: "en" | "ar") {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : tt("settings.language.errFailed");
      setState({ kind: "err", msg });
    } finally {
      setPending(null);
    }
  ...
  return (
    <section
      ...
      className="bg-white/80 dark:bg-gray-900/70 backdrop-blur rounded-2xl p-6 ring-1 ring-slate-200/70 dark:ring-gray-800 shadow"
    >
      <SectionHeader icon={<Languages size={18} />} title={tt("settings.language.title")} subtitle={tt("settings.language.subtitle")} />
      ...
      {state.kind === "ok" && (
        <p role="status" data-testid="language-success" className="mt-3 inline-flex items-center gap-2 text-sm font-medium text-emerald-700 dark:text-emerald-400">
...
function EmailPreferencesSection({
  t, notifyExpiry, notifyMarketing, email,
}: {
  t: (k: never) => string;
  ...
  const [state, setState] = useState<SaveState>({ kind: "idle" });
  const [pendingField, setPendingField] = useState<null | "notifyExpiry" | "notifyMarketing">(null);

  useEffect(() => { setExpiry(notifyExpiry); }, [notifyExpiry]);
  useEffect(() => { setMarketing(notifyMarketing); }, [notifyMarketing]);

  async function persist(field: "notifyExpiry" | "notifyMarketing", value: boolean) {
    ...
    if (field === "notifyExpiry") setExpiry(value); else setMarketing(value);
    ...
    try {
      ...
      if (field === "notifyExpiry") setExpiry(prev); else setMarketing(prev);
      const msg = err instanceof Error ? err.message : tt("settings.emails.errFailed");
      setState({ kind: "err", msg });
    } finally {
  ...
  return (
    <section
      ...
      className="bg-white/80 dark:bg-gray-900/70 backdrop-blur rounded-2xl p-6 ring-1 ring-slate-200/70 dark:ring-gray-800 shadow"
    >
      <SectionHeader icon={<Mail size={18} />} title={tt("settings.emails.title")} subtitle={tt("settings.emails.subtitle")} />
      <p className="mt-2 text-xs text-slate-500 dark:text-slate-400 ltr:font-mono rtl:text-end break-all" dir="ltr">
        {email}
      </p>
      ...
      {state.kind === "ok" && (
        <p role="status" data-testid="emails-success" className="mt-3 inline-flex items-center gap-2 text-sm font-medium text-emerald-700 dark:text-emerald-400">
          <CheckCircle2 size={16} /> {tt("settings.emails.saved")}
        </p>
      )}
      {state.kind === "err" && (
        <p role="alert" className="mt-3 inline-flex items-center gap-2 text-sm font-medium text-rose-600 dark:text-rose-400">
...
function ToggleRow({
  icon, label, desc, checked, onChange, disabled, testId,
}: {
  icon: React.ReactNode; label: string; desc: string;
  checked: boolean; onChange: (v: boolean) => void; disabled?: boolean; testId?: string;
}) {
  return (
    <label className={`flex items-start gap-3 p-3 rounded-xl border border-slate-200 dark:border-gray-700 hover:border-indigo-300 dark:hover:border-indigo-700 transition ${disabled ? "opacity-60 cursor-wait" : "cursor-pointer"}`}>
