...
const PAYMENT_BANNER_KEYS: Record<string, { key: TranslationKey; tone: "success" | "warning" | "error" }> = {
  ...
  pending_bank_transfer: { key: "checkout.banner.pendingBankTransfer", tone: "warning" },
};
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-indigo-50/60 dark:from-gray-950 dark:via-indigo-950/50 dark:to-slate-950 text-slate-900 dark:text-slate-100">
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
        ...
        <section className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-4">
          <ActionCard href="/ielts" icon={<GraduationCap size={22} />}
            title={t("dashboard.action.ielts.title")} description={t("dashboard.action.ielts.desc")}
            tone="from-purple-600 to-indigo-700" />
          <ActionCard href="/english" icon={<BookOpen size={22} />}
            title={t("dashboard.action.english.title")} description={t("dashboard.action.english.desc")}
            tone="from-blue-600 to-indigo-600" />
          <ActionCard href="/assessment" icon={<Sparkles size={22} />}
            title={t("dashboard.action.assessment.title")} description={t("dashboard.action.assessment.desc")}
...
function SummarySection({ t }: { t: (k: TranslationKey) => string }) {
  ...
  const ielts = useQuery({ queryKey: ["my-enrollments"], queryFn: fetchMyEnrollments });
  const english = useQuery({ queryKey: ["my-english-enrollments"], queryFn: fetchMyEnglishEnrollments });
  const certs = useQuery({ queryKey: ["my-certificates"], queryFn: fetchMyCertificates });

  const summary = useMemo(() => {
    const allEnrollments = [
      ...(ielts.data ?? []).map((e) => ({ active: e.isActive, expiresAt: e.expiresAt, status: e.status })),
      ...(english.data ?? []).map((e) => ({ active: e.isActive, expiresAt: e.expiresAt, status: e.status })),
    ...
    return { totalOwned, activeCount, minDays, certCount: certs.data?.length ?? 0 };
  ...
  const expiryText = loading
    ...
    : summary.minDays === null
      ? (summary.activeCount === 0 ? t("dashboard.summary.nextExpiryNone") : "∞")
      : summary.minDays === 1 ? t("dashboard.summary.dayShort")
      : t("dashboard.summary.daysShort").replace("{n}", String(summary.minDays));

  const expirySoon = !loading && summary.minDays !== null && summary.minDays <= 7;
    ...
    <section className="mt-6" aria-label={t("dashboard.summary.title")}>
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <SummaryCard
          ...
          note={loading ? "" : t("dashboard.summary.activeCoursesNote").replace("{total}", String(summary.totalOwned))}
        ...
        <SummaryCard
          ...
          tone={expirySoon ? "from-rose-500 to-red-600" : "from-emerald-500 to-teal-600"}
...
function SummaryCard({
  icon, tone, label, value, note, testId,
}: {
  icon: React.ReactNode; tone: string; label: string; value: string; note?: string; testId: string;
}) {
  return (
    <div
      data-testid={testId}
    ...
    >
      <div className={`w-11 h-11 rounded-xl bg-gradient-to-br ${tone} text-white flex items-center justify-center shadow-md shrink-0`}>
        {icon}
      </div>
...
type ProfileUser = {
  ...
  role: "student" | "admin"; bio?: string | null; avatarUrl?: string | null;
};

function ProfileCard({
  user, memberSince, t,
}: { user: ProfileUser; memberSince: string; t: (k: TranslationKey) => string }) {
    ...
    >
      ...
      <div className="flex flex-col items-center text-center">
        ...
        <p className="text-xs text-slate-500 dark:text-slate-400">
          {user.role === "admin" ? t("dashboard.profile.admin") : t("dashboard.profile.student")}
        </p>
        {avatarSrc && (
          <button
            type="button"
...
function EditProfileModal({
  user, t, onClose, onSaved,
}: {
  user: ProfileUser; t: (k: TranslationKey) => string;
    ...
    >
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-bold">{t("dashboard.profile.editTitle")}</h3>
        ...
        <div className="space-y-4">
          <div>
            <label htmlFor="profile-name" className="block text-xs font-semibold uppercase tracking-wider text-slate-500 mb-1">
              {t("dashboard.profile.fieldName")}
            </label>
            <input
              id="profile-name" ref={nameRef}
              type="text" value={name} onChange={(e) => setName(e.target.value)}
              maxLength={120} required minLength={2}
          ...
          <div>
            <label htmlFor="profile-phone" className="block text-xs font-semibold uppercase tracking-wider text-slate-500 mb-1">
              {t("dashboard.profile.fieldPhone")}
            </label>
            <input
              id="profile-phone"
              type="tel" value={phone} onChange={(e) => setPhone(e.target.value)}
              maxLength={32} dir="ltr"
              data-testid="input-profile-phone"
              className="w-full px-3 py-2 rounded-lg border border-slate-300 dark:border-gray-700 bg-white dark:bg-gray-950 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
            />
          </div>
          <div>
            <label htmlFor="profile-bio" className="block text-xs font-semibold uppercase tracking-wider text-slate-500 mb-1">
            ...
            <p className="mt-1 text-[11px] text-slate-500 dark:text-slate-400">
              {t("dashboard.profile.bioCounter").replace("{n}", String(bio.length))}
            </p>
          </div>
          {error && (
...
function PaymentBanner({
  status, text, tone, onDismiss,
}: { status: string; text: string; tone: "success" | "warning" | "error"; onDismiss: () => void }) {
  ...
  return (
    <div role="status" data-payment-status={status}
...
}: { href: string; icon: React.ReactNode; title: string; description: string; tone: string }) {
  return (
    <Link href={href}
