...
import { demoWords, demoWordsByLevel, DEMO_WORDS_PER_LEVEL } from "@/data/demo-words";
...
const arabicFont = '"Cairo", "Tajawal", "Segoe UI Arabic", system-ui, sans-serif';
...
export default function DemoFlashcards() {
  ...
  const filteredWords = useMemo<OxfordWord[]>(() => {
    ...
    return demoWordsByLevel[selectedLevel].map((word) => ({ word, level: selectedLevel }));
  }, [selectedLevel]);
    ...
    <div className="min-h-screen bg-gradient-to-br from-violet-50 via-white to-orange-50/40 dark:from-gray-950 dark:via-violet-950/40 dark:to-gray-950 transition-colors duration-300">
      <div className="max-w-3xl mx-auto px-4 py-6 min-h-screen flex flex-col">
        <header className="flex items-center justify-between mb-6">
          <Link href="/" className="flex items-center gap-3 group" aria-label="Back to landing page">
            <img
              src={lexoLogo}
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="mt-2 text-xs text-gray-600 dark:text-gray-300 leading-relaxed">
                You're previewing a tiny sample of LEXO. Register with an access code to unlock all <span className="font-bold text-violet-700 dark:text-violet-300">2,988 Oxford words</span>, 75 word families, and the full learning platform.
              </p>
            </div>
            <Link
              href="/"
