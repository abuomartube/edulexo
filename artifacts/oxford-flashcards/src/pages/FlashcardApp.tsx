...
import { Moon, Sun, GraduationCap, Layers, Sparkles, ArrowLeft } from "lucide-react";
...
export default function FlashcardApp() {
  ...
  const familyWords = useMemo<OxfordWord[]>(() => {
    ...
    return selectedFamily.words.map((word) => ({ word, level: levelOfWord(word) }));
  }, [selectedFamily]);
  ...
  const handlePrev = useCallback(() => {
    if (isFamilyDeck) {
      setFamilyCardIndex((i) => (i - 1 + familyWords.length) % familyWords.length);
    } else {
