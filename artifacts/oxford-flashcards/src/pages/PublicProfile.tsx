...
import {
  ArrowLeft, UserRound, Award, Calendar, Loader2, AlertCircle,
} from "lucide-react";
import Header from "@/components/Header";
import { useT, useLanguage } from "@/lib/i18n";
import { useAuth } from "@/lib/auth-context";
import { fetchPublicProfile, avatarViewUrl, type PublicProfileCert } from "@/lib/platform-api";
          ...
          <>
            ...
            >
              ...
              <div className="flex-1 min-w-0 text-center sm:text-start">
                ...
                <p className="mt-1 inline-flex items-center gap-1.5 text-xs font-semibold text-slate-500 dark:text-slate-400">
                  <Calendar size={13} />
                  {t("publicProfile.memberSince")} {fmtMonth(query.data.memberSince, lang)}
                </p>
                {query.data.bio ? (
                  <p
                  ...
                  >
                    {isMe ? t("publicProfile.bioEmptySelf") : t("publicProfile.bioEmptyOther")}
                  </p>
                )}
                {isMe && (
                  <Link
            ...
            >
              <header className="flex items-center gap-2 mb-4">
                ...
                <h2 className="text-lg font-bold">{t("publicProfile.certsTitle")}</h2>
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-bold">
                          {t(COURSE_LABEL[c.course])} · <span className="capitalize">{c.tier}</span>
                        </p>
                        <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                          {t("publicProfile.completedOn")} {fmtDate(c.completionDate, lang)}
                        </p>
                        <p className="text-[11px] font-mono text-slate-400 dark:text-slate-500 mt-0.5 break-all" dir="ltr">
                          {c.certificateId}
                        </p>
                      </div>
                    </li>
