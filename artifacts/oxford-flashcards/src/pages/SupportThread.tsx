...
import {
  Loader2, Send, AlertCircle, ArrowLeft, Paperclip, Download, Shield, UserRound,
} from "lucide-react";
import Header from "@/components/Header";
import { useT, useLanguage } from "@/lib/i18n";
import { useAuth } from "@/lib/auth-context";
import {
  fetchTicket, replyToTicket, uploadSupportAttachment, attachmentDownloadUrl,
  setTicketStatus,
  type SupportMessage, type SupportTicket, type SupportStatus,
      ...
      <main className="max-w-3xl mx-auto px-4 sm:px-6 py-6 sm:py-10">
        ...
        {data && (
          <ThreadView
            ...
            t={t}
            isAdmin={isAdmin}
            onChanged={() => qc.invalidateQueries({ queryKey: ["ticket", ticketId] })}
          />
        )}
      </main>
    </div>
...
function ThreadView({
  ticket, messages, lang, t, isAdmin, onChanged,
}: {
  ...
  return (
    <>
      <header className="mb-4">
        ...
        <h1 className="mt-2 text-xl sm:text-2xl font-extrabold">{ticket.subject}</h1>
        {isAdmin && (
          <div className="mt-3 flex flex-wrap gap-2">
            {(["awaiting_user", "awaiting_admin", "resolved", "closed"] as SupportStatus[]).map((s) => (
              <button
                key={s}
                type="button"
...
function MessageBubble({
  ...
  return (
    <li className={`flex ${isAdmin ? "justify-start" : "justify-end"}`}>
      ...
      >
        <div className={`flex items-center gap-1.5 text-xs font-semibold mb-1.5 ${isAdmin ? "text-indigo-700 dark:text-indigo-300" : "text-indigo-100"}`}>
          {isAdmin ? <Shield size={12} /> : <UserRound size={12} />}
          {t(`support.role.${message.authorRole}` as never)}
          <span className={`font-normal opacity-70 ml-1 ${!isAdmin ? "text-indigo-100" : ""}`}>· {time}</span>
        </div>
