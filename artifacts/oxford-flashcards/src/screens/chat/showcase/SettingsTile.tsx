...
function SettingRow({ icon, label, value, toggle, on, tone = "default" }: RowProps) {
  return (
  ...
  return (
    <>
      <Header
        title="الإعدادات"
        subtitle={
          <span className="text-[10px] text-slate-400">إدارة حسابك وتفضيلاتك</span>
        }
      />
      <ChatScrollBg className="px-4 pt-3 pb-3 space-y-2.5">
        <Card title="عام">
          <SettingRow icon={<Globe size={13} />} label="اللغة" value="العربية" />
          <SettingRow icon={<Moon size={13} />} label="المظهر الداكن" toggle on />
          <SettingRow icon={<Bell size={13} />} label="الإشعارات" toggle on />
        </Card>

        <Card title="المحادثات">
          <SettingRow icon={<Mic size={13} />} label="الميكروفون" value="افتراضي" />
          <SettingRow icon={<Globe size={13} />} label="English Only nudge" toggle on />
        </Card>
