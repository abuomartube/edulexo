...
if (VAPID_PUBLIC && VAPID_PRIVATE) {
  try {
    ...
    logger.error(
      { err },
      "Failed to set VAPID details for admin notifications",
    );
  }
}
...
router.post("/admin/notifications", async (req, res): Promise<void> => {
  ...
  const level =
    typeof body.level === "string" ? body.level.trim().toUpperCase() : null;
  ...
  if (audience !== "all" && audience !== "level") {
    res.status(400).json({ error: "audience must be 'all' or 'level'" });
    return;
  }
  if (audience === "level" && (!level || !VALID_LEVELS.has(level))) {
    res.status(400).json({ error: "Invalid level (A1, A2, B1, B2, C1)" });
    return;
  }
  ...
  if (scheduledAtRaw) {
    ...
    if (Number.isNaN(d.getTime())) {
      res.status(400).json({ error: "Invalid scheduledAt timestamp" });
      return;
    }
    scheduledAt = d;
  }
  ...
  try {
    ...
    if (created && created.sent_at) {
      dispatchPushForNotification(created.id).catch((err) =>
        logger.error(
          { err, id: created.id },
          "Push dispatch failed (immediate)",
        ),
      );
    }
...
router.get("/admin/notifications", async (req, res): Promise<void> => {
  ...
  try {
    const result = await db.execute<
      NotificationRow & { open_count: number }
    >(sql`
...
async function dispatchPushForNotification(
  ...
  if (!VAPID_PUBLIC || !VAPID_PRIVATE) {
    logger.warn(
      { notificationId },
      "VAPID keys not configured; skipping push fan-out",
    );
    return;
  }
  ...
  if (targets.rows.length === 0) {
    logger.info(
      { notificationId, audience: notif.audience, level: notif.level },
      "No push targets for notification",
    );
    return;
  }
    ...
    targets.rows.map(async (sub) => {
      ...
      try {
        ...
        if (statusCode === 410 || statusCode === 404) {
          ...
          try {
            await db.execute(
              sql`DELETE FROM push_subscriptions WHERE id = ${sub.sub_id}`,
            );
            removed++;
          } catch (delErr) {
            logger.error(
              { delErr, subId: sub.sub_id },
  ...
  try {
    ...
    if (result.rows.length > 0) {
      logger.info(
        ...
        "Delivered scheduled notifications",
      );
...
export function startNotificationsScheduler() {
  ...
  cron.schedule("* * * * *", () => {
    flushDueNotifications().catch((err) =>
      logger.error({ err }, "Notifications scheduler error"),
    );
  });
