...
router.post("/access/request", async (req, res): Promise<void> => {
  ...
  if (!codeRow) {
    res.status(401).json({
      error: "Invalid access code. Please check with your instructor.",
    });
    return;
  }
  ...
  const claimed = await db
    ...
    .where(
      and(
        eq(accessCodesTable.id, codeRow.id),
        isNull(accessCodesTable.usedByEmail),
      ),
    )
...
router.post("/access/login", async (req, res): Promise<void> => {
  ...
  if (row.status === "rejected") {
    res.status(403).json({
      status: "rejected",
      error: "Your request was not approved. Please contact your instructor.",
    });
    return;
  }
  if (row.status === "approved" && isExpired(row)) {
    res
      .status(403)
      .json({ status: "expired", error: "Your subscription has expired." });
    return;
  }
  ...
  if (!row.passwordHash) {
    res.status(403).json({
      ...
      error:
        "Your account is from a previous version of LEXO. Please reopen the app on the device where you originally signed in, or contact your instructor.",
    });
    return;
  }
...
router.post("/access/setup-password", async (req, res): Promise<void> => {
  ...
  if (!safeEqual(setupToken, expectedSession)) {
    res.status(401).json({
      error:
        "Authentication failed. Please reopen LEXO from your usual device.",
    });
    return;
  }
  ...
  if (row.passwordHash) {
    res
      .status(403)
      .json({ error: "Password is already set. Please log in instead." });
    return;
  }
...
router.post("/access/change-password", async (req, res): Promise<void> => {
  ...
  const headerEmail = ((req.headers["x-student-email"] as string) || "")
    .trim()
    .toLowerCase();
  ...
  if (
    !headerEmail ||
    !headerToken ||
    !safeEqual(headerToken, makeToken(headerEmail))
  ) {
    res
      .status(401)
      .json({ error: "Please log in again to change your password." });
    return;
  }
  ...
  const [row] = await db
    .select()
    .from(accessRequestsTable)
    .where(eq(accessRequestsTable.email, headerEmail));
  if (!row) {
    res.status(404).json({ error: "Account not found" });
    return;
  }
  ...
  if (!row.passwordHash) {
    res.status(403).json({
      error:
        "Your account has no password yet — please use Setup Password instead.",
    });
    return;
  }
...
router.post("/admin/access-codes", async (req, res): Promise<void> => {
  ...
  const cleanNote =
    note && typeof note === "string" ? note.trim().slice(0, 200) : null;
...
router.post("/admin/change-access-code", async (req, res): Promise<void> => {
  ...
  if (!newCode || typeof newCode !== "string" || newCode.trim().length < 4) {
    res
      .status(400)
      .json({ error: "Access code must be at least 4 characters" });
    return;
  }
  await db
...
router.get("/admin/access-code", async (req, res): Promise<void> => {
  ...
  const rows = await db
    .select()
    .from(settingsTable)
    .where(eq(settingsTable.key, "site_password"));
...
router.get("/admin/avatar", async (_req, res): Promise<void> => {
  const [row] = await db
    .select()
    .from(settingsTable)
    .where(eq(settingsTable.key, "admin_avatar"))
    .limit(1);
...
router.post("/admin/avatar", async (req, res): Promise<void> => {
  ...
  if (!/^data:image\/(png|jpeg|jpg|webp|gif);base64,/.test(dataUrl)) {
    res.status(400).json({ error: "Must be a base64-encoded image data URL" });
    return;
  }
  ...
  if (dataUrl.length > 420_000) {
    res
      .status(413)
      .json({ error: "Image too large — please use a photo under 300 KB" });
    return;
  }
  await db
    .insert(settingsTable)
...
router.get("/registration-video", async (_req, res): Promise<void> => {
  const [row] = await db
    .select()
    .from(settingsTable)
    .where(eq(settingsTable.key, "registration_video_url"))
    .limit(1);
  ...
  const results = await Promise.all(
    students.map(async (s) => {
      const [xpRow] = await db
        .select({
          total: sql<number>`coalesce(sum(${xpEventsTable.xp}),0)::int`,
        })
        .from(xpEventsTable)
      ...
      if (streakDays.length > 0) {
        ...
        let current =
          days[0] === today || days[0] === yesterday ? days[0] : null;
        if (current) {
          for (const day of days) {
            const diff = Math.round(
              (new Date(current).getTime() - new Date(day).getTime()) /
                86400000,
            );
            if (diff <= 1) {
              streak++;
      ...
      const [assignRow] = await db
        ...
        .where(
          and(
            eq(orwellSubmissionsTable.email, s.email),
...
router.post("/session/check", async (req, res): Promise<void> => {
  ...
  const [row] = await db
    ...
    .where(
      and(
        eq(userDataTable.email, normalizedEmail),
        eq(userDataTable.key, SESSION_KEY),
      ),
    )
    .limit(1);
  if (!row || !row.value) {
