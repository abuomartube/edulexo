...
export function warmUpPdf(): void {
  ...
  generatePdf()
    .then(() => {
      generatingPdf = false;
      logger.info("PDF pre-generation complete");
    })
    .catch((err) => {
      generatingPdf = false;
      pdfGenerationError = err;
      logger.error({ err }, "PDF pre-generation failed");
    });
}
...
router.get("/vocab-pdf", async (req, res): Promise<void> => {
  if (fs.existsSync(CACHED_PDF)) {
    ...
    res.setHeader(
      "Content-Disposition",
      'attachment; filename="IELTS-Vocabulary-2198-Words-EN-AR.pdf"',
    );
...
router.get("/vocab-pdf/status", (_req, res): void => {
  if (fs.existsSync(CACHED_PDF)) {
    ...
    res
      .status(500)
      .json({ status: "error", message: pdfGenerationError.message });
  } else {
