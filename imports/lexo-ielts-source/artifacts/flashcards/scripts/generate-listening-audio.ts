...
async function main() {
  ...
  const countTotal = LISTENING_SKILL_TESTS.reduce(
    (s, t) => s + t.audioLines.length,
    0,
  );
    ...
    for (let i = 0; i < test.audioLines.length; i++) {
      ...
      try {
        process.stdout.write(
          `[${total}/${countTotal}] ${test.id}-${i} (${voice}) ... `,
        );
      ...
      if (generated >= maxBatch) {
        ...
        console.log(
          `Progress: total=${total} generated=${generated} skipped=${skipped} failed=${failed}`,
        );
        return;
      }
