...
const LEVEL_OPTIONS: {
...
}[] = [
  {
    code: "A1",
    label: "A1 — Beginner",
    blurb: "I know a few words and basic phrases.",
    blurbAr: "أعرف بعض الكلمات والعبارات الأساسية.",
  },
  {
    code: "A2",
    label: "A2 — Elementary",
    blurb: "I can handle simple everyday conversations.",
    blurbAr: "يمكنني إجراء محادثات يومية بسيطة.",
  },
  {
    code: "B1",
    label: "B1 — Intermediate",
    blurb: "I can talk about familiar topics and read simple texts.",
    blurbAr: "يمكنني التحدث عن مواضيع مألوفة وقراءة نصوص بسيطة.",
  },
  {
    code: "B2",
    label: "B2 — Upper-Intermediate",
    blurb: "I can discuss many topics and understand most articles.",
    blurbAr: "يمكنني مناقشة مواضيع متعددة وفهم معظم المقالات.",
  },
  {
    code: "C1",
    label: "C1 — Advanced",
    blurb: "I express myself fluently and read complex material.",
    blurbAr: "أعبر عن نفسي بطلاقة وأقرأ مواد معقدة.",
  },
];
...
const TARGET_BAND_OPTIONS: {
...
}[] = [
  {
    band: 6,
    label: "Band 6",
    description: "Competent user — common university requirement",
  },
  {
    band: 7,
    label: "Band 7",
    description: "Good user — strong career & study opportunities",
  },
  {
    band: 8,
    label: "Band 8",
    description: "Very good user — top-tier programs & professions",
  },
];
...
function studyZoneFor(level: CefrLevel): {
  primary: CefrLevel;
  stretch: CefrLevel;
} {
  switch (level) {
    case "A1":
      return { primary: "A1", stretch: "A2" };
    case "A2":
      return { primary: "A2", stretch: "B1" };
    case "B1":
      return { primary: "B1", stretch: "B2" };
    case "B2":
      return { primary: "B2", stretch: "C1" };
    case "C1":
      return { primary: "C1", stretch: "C1" };
  }
}

function gapScore(current: CefrLevel, targetBand: 6 | 7 | 8): number {
  const currentIndex: Record<CefrLevel, number> = {
    A1: 0,
    A2: 1,
    B1: 2,
    B2: 3,
...
function generateStudyPlan(
    ...
    schedule.push(
      {
        day: "Daily",
        tasks: [
          `Learn ${wordsPerDay} new words at your level (${vocabFocus})`,
          "Review weak words deck",
          "Complete 1 Reading passage",
          "Complete 1 Listening section",
        ],
      },
      {
        day: "Mon/Wed/Fri",
        tasks: [
          "Write 1 full essay (Task 2) — check with Orwell AI",
          "Practice Speaking Part 2 with Churchill AI",
        ],
      },
      {
        day: "Tue/Thu",
        tasks: [
          "Write 1 Task 1 report — check with Orwell AI",
          "Practice Speaking Parts 1 & 3 with Churchill AI",
        ],
      },
      {
        day: "Weekend",
        tasks: [
          ...
          "Review all weak words",
          "Re-read Writing Templates",
        ],
      },
    );
    ...
    schedule.push(
      {
        ...
        tasks: [
          `Learn ${wordsPerDay} new words at your level (${vocabFocus})`,
          "Review weak words deck",
          "15 min Reading or Listening practice",
        ],
      },
      {
        day: "Mon/Wed/Fri",
        tasks: [
          "Write 1 essay — check with Orwell AI",
          "Practice Speaking with Churchill AI",
        ],
      },
      {
        day: "Tue/Thu",
        tasks: [
          "Complete 1 full Reading test",
          "Complete 1 full Listening test",
        ],
      },
      {
        day: "Weekend",
        tasks: [
          ...
          "Review Writing Templates & Topic Bank",
        ],
      },
    );
    ...
    schedule.push(
      {
        day: "Daily",
        tasks: [
          `Learn ${wordsPerDay} new words (${vocabFocus})`,
          "Review weak words (10 min)",
        ],
      },
      {
        day: "Mon/Wed",
        tasks: [
          "1 Reading passage + 1 Listening section",
          "Practice Speaking with Churchill AI",
        ],
      },
      {
        ...
        tasks: [
          "Write 1 essay — check with Orwell AI",
          "Study Writing Templates",
        ],
      },
      {
        day: "Weekend",
        tasks: [
          "Take a Mock Test once a month",
          "Review synonyms & phrasal verbs",
          "Read 1 short story",
        ],
      },
    );
    ...
    schedule.push(
      {
        ...
        tasks: [
          `Learn ${wordsPerDay} new words (${vocabFocus})`,
          "Quick weak words review",
        ],
      },
      {
        day: "Mon/Wed",
        tasks: ["1 Reading or Listening section", "Browse Topic Bank"],
      },
      {
        day: "Tue/Thu",
        tasks: [
          "Write 1 paragraph — check with Orwell AI",
          "Practice 1 Speaking topic",
        ],
      },
      {
        day: "Weekend",
        tasks: [
          "1 full Reading or Listening test",
  ...
  if (weeksLeft >= 8)
    milestones.push(`Weeks 1–2: Master ${zone.primary} vocabulary foundation`);
  if (weeksLeft >= 6)
    milestones.push(
      `Weeks ${weeksLeft >= 8 ? "3–4" : "1–2"}: Reading & Listening at ${zone.primary} level, stretch into ${zone.stretch}`,
    );
  if (weeksLeft >= 4)
    milestones.push(
      `Weeks ${weeksLeft >= 8 ? "5–6" : "3–4"}: Writing & Speaking practice with AI feedback`,
    );
  milestones.push(
    `Final ${Math.min(weeksLeft, 2)} week(s): Mock tests + push toward Band ${targetBand}`,
  );
...
export function Onboarding({ onComplete }: { onComplete: () => void }) {
  ...
  const plan =
    currentLevel && canProceedExam
      ? generateStudyPlan(currentLevel, targetBand, effectiveExamDate)
      : null;
        ...
        <div className="w-full max-w-md space-y-6 py-8">
          <div className="text-center space-y-2">
            ...
            <p className="text-muted-foreground text-sm">
              We'll use this on your study plan and PDF reports.
            </p>
            <p className="text-muted-foreground text-xs" dir="rtl" lang="ar">
              ما اسمك؟ سنستخدمه في خطة دراستك وتقارير PDF.
            </p>
          </div>
          ...
          <div className="bg-card border border-border rounded-2xl p-5 space-y-3">
            ...
            {trimmedName && !nameValid ? (
              <p className="text-xs text-red-600 dark:text-red-400">
                Please use English letters only (2–40 chars). Hyphens and
      ...
      <div className="fixed inset-0 bg-background/95 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-300 overflow-y-auto">
        <div className="w-full max-w-md space-y-6 py-8">
          <div className="text-center space-y-2">
            ...
            <h1 className="text-2xl font-extrabold text-foreground">
              Welcome, {trimmedName.split(/\s+/)[0]}!
            </h1>
            <p className="text-muted-foreground text-sm">
              Let's set up your personalized study plan
            </p>
            <p className="text-muted-foreground text-xs" dir="rtl" lang="ar">
              مرحباً! دعنا نضع خطة دراسية مخصصة لك
            </p>
          </div>
          ...
          <div className="bg-card border border-border rounded-2xl p-5 space-y-4">
            ...
            <p className="text-[11px] text-muted-foreground text-center pt-1">
              Your content will start at this level — we'll raise the difficulty
              as you improve.
            </p>
          </div>
      ...
      <div className="fixed inset-0 bg-background/95 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-300 overflow-y-auto">
        <div className="w-full max-w-md space-y-6 py-8">
          <div className="text-center space-y-2">
            ...
            <p className="text-muted-foreground text-xs" dir="rtl" lang="ar">
              ما هي درجة الآيلتس التي تستهدفها؟
            </p>
          </div>
          ...
          <div className="bg-card border border-border rounded-2xl p-5 space-y-4">
            ...
            <div className="text-center text-xs text-muted-foreground">
              {
                TARGET_BAND_OPTIONS.find((o) => o.band === targetBand)
                  ?.description
              }
            </div>

            <div className="rounded-xl bg-muted/40 border border-border px-3 py-2 text-[11px] text-muted-foreground">
              💡 Your target band is used for motivation & progress tracking. It
              doesn't change the difficulty of your study material — that's
              based on your current level ({currentLevel}).
            </div>
          </div>
      ...
      <div className="fixed inset-0 bg-background/95 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-300 overflow-y-auto">
        <div className="w-full max-w-md space-y-6 py-8">
          <div className="text-center space-y-2">
            ...
            <p className="text-muted-foreground text-sm">
              When are you taking the IELTS exam?
            </p>
            <p className="text-muted-foreground text-xs" dir="rtl" lang="ar">
              متى موعد اختبار الآيلتس الخاص بك؟
            </p>
          </div>
          ...
          <div className="bg-card border border-border rounded-2xl p-5 space-y-4">
            ...
            <input
              ...
              disabled={noExamDate}
              className={cn(
                "w-full px-4 py-3 bg-background border border-border rounded-xl text-foreground text-lg font-medium focus:outline-none focus:ring-2 focus:ring-primary/50",
                ...
                return (
                  <div className="text-center space-y-1">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {days <= 14
                        ? "Time is short — we'll create an intensive plan!"
                        : days <= 30
                          ? "Good timing — a focused study plan awaits!"
                          : days <= 60
                            ? "Great! Plenty of time for balanced preparation."
            ...
            {noExamDate && (
              <p className="text-xs text-muted-foreground text-center">
                No problem! We'll create a balanced 3-month study plan you can
                adjust later.
              </p>
            )}
          </div>
      ...
      <div className="fixed inset-0 bg-background/95 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-300 overflow-y-auto">
        <div className="w-full max-w-md space-y-6 py-8">
          <div className="text-center space-y-2">
            ...
            <p className="text-muted-foreground text-xs" dir="rtl" lang="ar">
              اختر مدة الخطة التي تناسب جدولك.
            </p>
          </div>
          ...
          <div className="bg-card border border-border rounded-2xl p-4 space-y-3">
            <div className="flex items-center gap-2">
              <Clock className="w-5 h-5 text-blue-500" />
              <h2 className="font-bold text-foreground">
                Choose your study plan duration
              </h2>
            </div>
            ...
            <p className="text-[11px] text-muted-foreground text-center pt-1">
              You can reset to a different length any time from{" "}
              <span className="font-semibold text-foreground">My Plan</span>.
            </p>
          </div>
      ...
      <div className="max-w-lg mx-auto py-8 px-4 space-y-5">
        <div className="text-center space-y-2">
          ...
          <p className="text-muted-foreground text-sm">
            ...
            {" · "}
            Target{" "}
            <span className="font-bold text-primary">Band {targetBand}</span>
            {" · "}
        ...
        <div className="bg-muted/30 border border-border rounded-2xl p-4 space-y-2">
          <h3 className="font-bold text-foreground text-sm">
            💡 Key tools to reach Band {targetBand}:
          </h3>
          <div className="grid grid-cols-2 gap-2 text-xs">
            ...
            <div className="flex items-center gap-1.5 text-foreground">
              <FileText className="w-3.5 h-3.5 text-amber-500" /> Orwell AI
              (Writing)
            </div>
            <div className="flex items-center gap-1.5 text-foreground">
              <Mic className="w-3.5 h-3.5 text-rose-500" /> Churchill AI
              (Speaking)
            </div>
            <div className="flex items-center gap-1.5 text-foreground">
        ...
        <div className="flex gap-3">
          ...
          >
            {saving ? "Saving..." : saveError ? "Retry" : "Start Studying!"}{" "}
      ...
      <div className="w-full max-w-sm bg-card border border-border rounded-2xl shadow-2xl p-6 space-y-4 animate-in zoom-in-95 duration-200">
        <div className="text-center space-y-2">
          ...
          <p className="text-sm text-muted-foreground">
            What should we call you on your study plan?
          </p>
          <p className="text-xs text-muted-foreground" dir="rtl" lang="ar">
            ما اسمك؟ سنستخدمه في خطة دراستك.
          </p>
        </div>
