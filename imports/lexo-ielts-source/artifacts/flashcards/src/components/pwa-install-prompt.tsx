...
function isIOS() {
  if (typeof navigator === "undefined") return false;
  return (
    /iPad|iPhone|iPod/.test(navigator.userAgent) && !(navigator as any).MSStream
  );
}
...
export function PwaInstallPrompt() {
  ...
  const [variant, setVariant] = useState<"android" | "ios" | "desktop" | null>(
    null,
  );
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="font-extrabold text-base leading-tight">
              Add LEXO to your home screen
            </p>
            {variant === "android" ? (
              <p className="text-sm text-muted-foreground mt-1">
                Get instant access and a full-screen experience — just like a
                native app.
              </p>
            ) : (
