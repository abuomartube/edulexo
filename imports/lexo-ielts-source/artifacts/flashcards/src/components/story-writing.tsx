...
export function StoryWriting({ storyId, onComplete }: StoryWritingProps) {
  ...
  async function handleSubmit() {
    ...
    if (trimmed.length < MIN_CHARS) {
      setError(
        `Please write at least ${MIN_CHARS} characters (a few sentences).`,
      );
      return;
    }
    ...
    try {
      ...
      setError(
        e?.data?.error ?? e?.message ?? "Could not analyse your response.",
      );
    } finally {
      setSubmitting(false);
    }
        ...
        <div className="text-center">
          ...
          {feedback.xpAwarded > 0 && (
            <p className="mt-1 inline-flex items-center gap-1 text-sm font-medium text-amber-600 dark:text-amber-400">
              <Sparkles className="h-4 w-4" />+{feedback.xpAwarded} XP earned
            </p>
          )}
        </div>
            ...
            {feedback.grammar.map((g, i) => (
              ...
              >
                <div className="flex flex-wrap items-baseline gap-2">
                  <span className="line-through text-rose-600 dark:text-rose-400">
                    {g.original}
                  </span>
        ...
        <FeedbackSection title="Coherence & Cohesion" emoji="🧩">
          <div className="text-sm rounded-md border border-border/60 bg-background p-3">
            ...
            <div className="font-semibold mb-1">
              {feedback.coherence.rating}
            </div>
            <p className="text-sm text-foreground/90">
              {feedback.coherence.comment}
            </p>
          </div>
        </FeedbackSection>
      ...
      <div className="flex items-start gap-2">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">
            Your response will be analysed for grammar, vocabulary, coherence,
            and linking words.
          </p>
        </div>
      </div>
