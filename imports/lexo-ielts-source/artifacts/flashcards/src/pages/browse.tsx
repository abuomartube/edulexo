    ...
    <Layout>
      <div className="space-y-6 animate-in fade-in duration-500">
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-muted-foreground mt-2">
              Search and filter through all vocabulary words.
            </p>
          </div>
          <button
          ...
          >
            ...
            {bookmarkSet.size > 0 && (
              <span
                className={`text-xs px-1.5 py-0.5 rounded-full font-bold ${bookmarkedOnly ? "bg-white/20" : "bg-primary/10 text-primary"}`}
              >
                {bookmarkSet.size}
              </span>
            )}
        ...
        <div className="flex flex-col md:flex-row gap-4 bg-card p-4 rounded-2xl shadow-sm border border-border">
          ...
          <Select
            value={levelFilter}
            onValueChange={(v) => setLevelFilter(v as any)}
          >
        ...
        {isLoading ? (
          ...
          <div className="text-center py-12 text-muted-foreground bg-card rounded-2xl border border-dashed">
            {bookmarkedOnly
              ? "No bookmarked cards yet. Click the bookmark icon on any card to save it."
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {filteredCards.map((card, i) => {
                const latestProgress = progress
                  ?.filter((p) => p.flashcardId === card.id)
                  .sort(
                    (a, b) =>
                  ...
                  >
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="pt-3 border-t border-border/50">
                        <h3 className="font-bold text-xl arabic-text text-primary text-right">
                          {card.arabic}
                        </h3>
                        {card.exampleSentenceArabic && (
                          <p className="text-sm text-muted-foreground arabic-text text-right mt-1 line-clamp-2">
                            {card.exampleSentenceArabic}
                          </p>
                        )}
                      </div>
