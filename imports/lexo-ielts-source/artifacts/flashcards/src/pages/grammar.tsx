  ...
  return (
    ...
    >
      <div className="flex items-start gap-4">
        <div
          className={`w-12 h-12 rounded-xl ${topic.iconBg} flex items-center justify-center shrink-0`}
        >
      ...
      {topic.rules.map((r, i) => (
        ...
        >
          <p className="text-sm font-semibold text-foreground mb-1.5">
            <span className={`${topic.color} font-extrabold mr-1.5`}>
              {i + 1}.
            </span>
            {r.rule}
          </p>
...
function ExerciseView({
  ...
  return (
    <div className="space-y-4">
      ...
      {answered && (
        <div
          className={`rounded-xl p-4 ${isCorrect ? "bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800" : "bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800"}`}
        >
...
function TopicDetail({
  ...
  const [answers, setAnswers] = useState<(number | null)[]>(
    new Array(topic.exercises.length).fill(null),
  );

  const score = answers.filter(
    (a, i) => a === topic.exercises[i].correctIndex,
  ).length;
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
          ...
          {answers[currentQ] !== null && (
            <div className="flex justify-end">
              <button
                ...
                className="flex items-center gap-2 px-6 py-3 rounded-xl bg-primary text-primary-foreground font-bold hover:opacity-90 transition-opacity"
              >
                {currentQ < topic.exercises.length - 1
                  ? "Next Question"
                  : "See Results"}
      ...
      {phase === "results" && (
        <div className="bg-card border border-border rounded-2xl p-8 text-center space-y-4 animate-in fade-in zoom-in-95 duration-300">
          <div
            className={`w-20 h-20 rounded-full mx-auto flex items-center justify-center ${score >= 4 ? "bg-green-100 dark:bg-green-900/20" : score >= 3 ? "bg-amber-100 dark:bg-amber-900/20" : "bg-red-100 dark:bg-red-900/20"}`}
          >
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="text-muted-foreground mt-1 leading-relaxed">
                  Master the most common grammar mistakes in IELTS Writing and
                  Speaking. Each topic includes key rules and 5 practice
                  exercises.
                </p>
                <p
                  ...
                  lang="ar"
                >
                  أتقن الأخطاء النحوية الأكثر شيوعاً في كتابة ومحادثة الآيلتس.
                  كل موضوع يتضمن قواعد رئيسية و5 تمارين تطبيقية.
                </p>
              </div>
            </div>
