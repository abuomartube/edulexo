    ...
    (async () => {
      try {
        const [lvl, band, exam, dur, start] = await Promise.all([
          customFetch<{ value: string }>("/api/user-data/current_level").catch(
            () => null,
          ),
          customFetch<{ value: string }>("/api/user-data/target_band").catch(
            () => null,
          ),
          customFetch<{ value: string }>("/api/user-data/exam_date").catch(
            () => null,
          ),
          customFetch<{ value: string }>(
            "/api/user-data/plan_duration_days",
          ).catch(() => null),
          customFetch<{ value: string }>(
            "/api/user-data/plan_start_date",
          ).catch(() => null),
        ]);
        if (cancelled) return;
      ...
      <Layout>
        <div className="max-w-md mx-auto px-4 py-12 text-center space-y-4">
          ...
          <p className="text-muted-foreground">
            Finish onboarding to generate your study plan.
          </p>
          <Link
            href="/"
            className="inline-block bg-primary text-primary-foreground font-bold px-5 py-2.5 rounded-xl"
          >
            Go home
          </Link>
        </div>
  ...
  return (
    <Layout>
      <div className="max-w-5xl mx-auto px-4 py-6 space-y-6">
          ...
          <div className="relative z-10 grid grid-cols-1 md:grid-cols-3 gap-6">
            <div>
              ...
              <p className="text-white/85 text-sm mt-1">
                {shortDate(parsePlanISO(plan.startISO))} →{" "}
                {shortDate(stats!.endDate)}
              </p>
            ...
            <div className="md:col-span-2 md:border-l md:border-white/20 md:pl-6">
              ...
              <div className="flex items-baseline gap-3">
                <span className="text-5xl sm:text-6xl font-extrabold leading-none">
                  {stats!.percent}%
                </span>
              ...
              <div className="mt-4 grid grid-cols-3 gap-3 text-center">
                ...
                <Mini
                  label="Today"
                  value={`Day ${stats!.todayIndex}`}
                  icon={<Sparkles className="w-3.5 h-3.5" />}
                />
                <Mini
                  label="Missed"
                  value={stats!.missedDays + stats!.partialDays}
          ...
          <div className="rounded-2xl border-2 border-amber-300 dark:border-amber-700/50 bg-amber-50 dark:bg-amber-950/20 p-4 flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-sm text-amber-800 dark:text-amber-300 mt-0.5">
                Try squeezing 1–2 extra short sessions (Word of the Day · Quiz
                Mode · Weak Words) into today and tomorrow to stay on track for
                your exam.
              </p>
            </div>
          </div>
        ...
        {/* ── Coverage panel ── */}
        {coverage && (
          <CoveragePanel coverage={coverage} duration={plan.duration} />
        )}
...
function CoveragePanel({
  ...
  coverage: Record<
    string,
    { scheduledCount: number; uniqueCovered: number; catalogSize: number }
  >;
  duration: PlanDuration;
}) {
  const orwellUnique =
  ...
  const readingSkillsUnique =
    (coverage.readingSkills?.uniqueCovered ?? 0) +
    (coverage.readingSkillsEasy?.uniqueCovered ?? 0);
  ...
  const rows: {
    ...
    },
    {
      ...
      pct:
        speakingSize > 0
          ? Math.round((speakingUnique / speakingSize) * 100)
          : 0,
    ...
    },
    {
      ...
      pct:
        readingSkillsSize > 0
          ? Math.round((readingSkillsUnique / readingSkillsSize) * 100)
          : 0,
    ...
    },
    {
      ...
      pct:
        listeningSkillsSize > 0
          ? Math.round((listeningSkillsUnique / listeningSkillsSize) * 100)
          : 0,
    ...
    },
    {
      ...
      pct:
        LIBRARY_SIZES.readingMockTests > 0
          ? Math.round(
              ((coverage.readingMock?.uniqueCovered ?? 0) /
                LIBRARY_SIZES.readingMockTests) *
                100,
            )
          : 0,
    ...
    },
    {
      ...
      pct:
        LIBRARY_SIZES.listeningMockTests > 0
          ? Math.round(
              ((coverage.listeningMock?.uniqueCovered ?? 0) /
                LIBRARY_SIZES.listeningMockTests) *
                100,
            )
          : 0,
...
function Legend({
  ...
  return (
    <div className="inline-flex items-center gap-1.5">
      <span
        className={cn(
          "inline-block w-3 h-3 rounded-md",
          swatch,
      ...
      <div className="p-3 sm:p-4 grid grid-cols-7 gap-2">
        {days.map((day) => (
          <DayCard
            key={day.iso}
            day={day}
            expanded={day.iso === expandedIso}
            onClick={() => onToggleExpand(day.iso)}
          />
        ))}
      </div>
          ...
          <div className="flex items-center justify-between mb-3">
            ...
            <span
              className={cn(
                ...
                expandedDay.state === "today" &&
                  "bg-sky-100 text-sky-700 dark:bg-sky-900/30 dark:text-sky-400",
                expandedDay.state === "upcoming" &&
                  "bg-muted text-muted-foreground",
                (expandedDay.state === "missed" ||
                  expandedDay.state === "partial") &&
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
      {day.tasks.map((task, i) => {
        ...
        const href =
          task.scheduled?.href ??
          (task.hash ? `${task.href}${task.hash}` : task.href);
        return (
          <Link
            key={task.id}
            href={href}
