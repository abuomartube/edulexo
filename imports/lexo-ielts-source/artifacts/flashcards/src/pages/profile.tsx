...
export default function ProfilePage() {
    ...
    (async () => {
      try {
        const [n, l, b, d, a] = await Promise.all([
          customFetch<{ value: string }>("/api/user-data/name").catch(() => ({
            value: "",
          })),
          ...
          customFetch<{ value: string }>("/api/user-data/avatar").catch(() => ({
            value: "",
          })),
        ]);
        if (cancelled) return;
  ...
  async function handleSaveProfile() {
    ...
    try {
      ...
      try {
        window.dispatchEvent(new CustomEvent("lexo:profile-updated"));
      } catch {
        /* ignore */
      }
    } catch {
  ...
  async function handleAvatarPick(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setProfileMsg({
        type: "err",
        text: "Could not upload photo. Please try a smaller image.",
      });
    } finally {
      setAvatarBusy(false);
    }
  ...
  async function handleChangePassword(e: React.FormEvent) {
    ...
    if (newPassword === currentPassword) {
      setPwMsg({
        type: "err",
        text: "Pick a new password different from your current one.",
      });
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        setPwMsg({
          type: "err",
          text: body?.error || "Could not change password.",
        });
      } else {
        ...
        <section className="rounded-3xl border border-border bg-card p-6 space-y-5">
          ...
          <div className="space-y-1.5">
            <label
              ...
              className="text-xs uppercase tracking-widest font-bold text-muted-foreground flex items-center gap-1.5"
            >
          ...
          <div className="space-y-1.5">
            ...
            <p className="text-[11px] text-muted-foreground">
              Email is locked. Contact your instructor to change it.
            </p>
          </div>
          ...
          <p className="text-[11px] text-muted-foreground -mt-2">
            Level and target band are set in your study plan. Open{" "}
            <a
              href="/plan"
