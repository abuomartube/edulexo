...
export const useToggleBookmark = () => {
  ...
  return useMutation<{ bookmarked: boolean }, unknown, number>({
    mutationFn: (id) =>
      customFetch<{ bookmarked: boolean }>(`/api/bookmarks/${id}`, {
        method: "POST",
      }),
...
export const useWordOfDay = () =>
  useQuery<WordOfDay | null>({
    ...
    queryFn: () =>
      customFetch<WordOfDay>("/api/word-of-day", { method: "GET" }),
  });
...
export const useWeakWords = () =>
  useQuery<WeakWordItem[]>({
    ...
    queryFn: () =>
      customFetch<WeakWordItem[]>("/api/weak-words", { method: "GET" }),
  });
...
export const useMasterWeakWord = () => {
  ...
  return useMutation<{ mastered: boolean }, unknown, number>({
    mutationFn: (id) =>
      customFetch<{ mastered: boolean }>(`/api/weak-words/${id}/master`, {
        method: "POST",
      }),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["/api/weak-words"] }),
  });
...
export const usePhrasalVerbWeak = () => {
  ...
  const toggle = useMutation<{ weak: boolean }, unknown, number>({
    ...
    onSuccess: () =>
      qc.invalidateQueries({ queryKey: ["/api/phrasal-verbs/weak"] }),
  });
...
export const useQuiz = (level?: string, count = 10) =>
  useQuery<QuizQuestion[]>({
    ...
    queryFn: () => {
      ...
      return customFetch<QuizQuestion[]>(`/api/quiz?${params}`, {
        method: "GET",
      });
    },
  });
...
export const useFillBlank = (level?: string, count = 10) =>
  useQuery<FillBlankQuestion[]>({
    ...
    queryFn: () => {
      ...
      if (level && level !== "ALL") params.set("level", level);
      params.set("count", String(count));
      return customFetch<FillBlankQuestion[]>(`/api/fill-blank?${params}`, {
        method: "GET",
      });
    },
  });
...
export const useSrsDue = (level?: string) =>
  useQuery<WordOfDay[]>({
    ...
    queryFn: () => {
      ...
      return customFetch<WordOfDay[]>(`/api/srs/due${params}`, {
        method: "GET",
      });
    },
  });
