...
function main() {
  if (!fs.existsSync(INPUT_JSON)) {
    console.error(
      "Error: vocabulary-translated.json not found. Run parse-vocabulary.cjs first.",
    );
    process.exit(1);
  }
  ...
  if (total < EXPECTED_TOTAL * 0.95) {
    console.error(
      `Validation error: Only ${total} entries found, expected at least ${Math.floor(EXPECTED_TOTAL * 0.95)}. Aborting HTML generation.`,
    );
    process.exit(1);
  }
  console.log(
    `Validation passed: ${total} entries (expected ${EXPECTED_TOTAL})`,
  );
  ...
  console.log(
    `File size: ${(fs.statSync(OUTPUT_HTML).size / 1024 / 1024).toFixed(2)} MB`,
  );
}
