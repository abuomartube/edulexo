...
function parseVocabularyText(text) {
  ...
  const levelHeaderPattern =
    /^4ielts\.com\s+IELTS Vocabulary\s+·\s+(A1|A2|B1|B2|C1)\s+·/;
  const POS_OPTIONS =
    "(?:adj\\.|adv\\.|arti\\.|conj\\.|det\\.|excl\\.|idiom\\.|moda\\.|n\\.|n\\.pl\\.|prep\\.|pron\\.|v\\.)";
  const fullEntryPattern = new RegExp(
    `^(\\d+)\\s+(\\S+(?:\\s+\\S+)?)\\s+(${POS_OPTIONS})\\s+(.+)$`,
  );
  const partialEntryPattern = new RegExp(
    `^(\\d+)\\s+(\\S+(?:\\s+\\S+)?)\\s+(${POS_OPTIONS})\\s*$`,
  );
  ...
  const isSkipLine = (l) => {
    return (
      !l ||
      l.startsWith("www.4ielts.com") ||
      l.startsWith("4ielts.com") ||
      l.startsWith("-- ") ||
      l.startsWith("# WORD") ||
      l.match(/^Page \d+/) ||
      l.includes("3,000 Words Total") ||
      l === "CEFR VOCABULARY SERIES" ||
      l === "A1 A2 B1 B2 C1" ||
      l.match(
        /^(A1|A2|B1|B2|C1)\s*·\s*(Foundational|Elementary|Intermediate|Upper|Advanced)\s*—/,
      ) ||
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (fullMatch) {
      ...
      if (
        nextLine &&
        !isSkipLine(nextLine) &&
        !levelHeaderPattern.exec(nextLine) &&
        !fullEntryPattern.exec(nextLine) &&
        !partialEntryPattern.exec(nextLine)
      ) {
        pendingEntry = {
          num: entryNum,
          word: word.trim(),
    ...
    if (partialMatch) {
      ...
      if (
        ...
        if (
          nextNextLine &&
          !isSkipLine(nextNextLine) &&
          !levelHeaderPattern.exec(nextNextLine) &&
          !fullEntryPattern.exec(nextNextLine) &&
          !partialEntryPattern.exec(nextNextLine)
        ) {
  ...
  for (const [level, data] of Object.entries(levels)) {
    ...
    console.log(
      `  ${level}: ${data.entries.length} / ${data.totalWords} entries parsed`,
    );
  }
...
async function translateAll(levels) {
  ...
  for (const [level, data] of Object.entries(levels)) {
    ...
    const existingNums = new Set(
      (result[level].entries || []).map((e) => e.num),
    );
    const toTranslate = data.entries.filter(
      (e) => !existingNums.has(e.num) && e.sentence,
    );
    ...
    for (let i = 0; i < toTranslate.length; i += BATCH_SIZE) {
      ...
      console.log(
        `  Batch ${batchNum}/${totalBatches}: entries ${batch[0].num}–${batch[batch.length - 1].num}...`,
      );
      ...
      translations.forEach((t) => {
        translationMap[t.num] = t;
      });

      batch.forEach((entry) => {
        const trans = translationMap[entry.num] || {
          wordAr: "—",
          sentenceAr: "—",
        };
    ...
    const noSentence = data.entries.filter(
      (e) => !e.sentence && !existingNums.has(e.num),
    );
