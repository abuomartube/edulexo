    ...
    try {
      ...
      const wait =
        status === 429 || status === 529
          ...
          : 1500 * (attempt + 1);
      console.error(
        `[claude] retry ${attempt + 1} in ${wait}ms (${e.message?.slice(0, 80)})`,
      );
...
async function main() {
  ...
  console.log(
    `Loaded: ${flashcards.length} flashcards, ${syns.length} synonyms, ${ants.length} antonyms, ${phrs.length} phrasal verbs`,
  );
  ...
  for (const fix of flashFixes) {
    ...
    log.push(
      `flashcard #${fix.id} "${row.english}": "${row.arabic}" -> "${fix.ar}"  (${fix.reason || ""})`,
    );
    row.arabic = fix.ar;
    flashApplied++;
  }
  for (const fix of synFixes) {
    ...
    log.push(
      `synonym #${fix.id} "${row.word}": "${row.translation}" -> "${fix.ar}"  (${fix.reason || ""})`,
    );
  ...
  for (const fix of antFixes) {
    ...
    if (
      fix.wordAr &&
      typeof fix.wordAr === "string" &&
      row.translation !== fix.wordAr
    ) {
      log.push(
        `antonym #${fix.id} word "${row.word}": "${row.translation}" -> "${fix.wordAr}"  (${fix.reason || ""})`,
      );
      row.translation = fix.wordAr;
      touched = true;
    }
    if (
      fix.antAr &&
      typeof fix.antAr === "string" &&
      row.antonymTranslation !== fix.antAr
    ) {
      log.push(
        `antonym #${fix.id} antonym "${row.antonym}": "${row.antonymTranslation}" -> "${fix.antAr}"  (${fix.reason || ""})`,
      );
      row.antonymTranslation = fix.antAr;
      touched = true;
    }
  ...
  for (const fix of phrFixes) {
    ...
    if (
      fix.verbAr &&
      typeof fix.verbAr === "string" &&
      row.phrasalVerbAr !== fix.verbAr
    ) {
      log.push(
        `phrasal #${fix.id} "${row.phrasalVerb}" verbAr: "${row.phrasalVerbAr}" -> "${fix.verbAr}"  (${fix.reason || ""})`,
      );
      row.phrasalVerbAr = fix.verbAr;
      touched = true;
    }
    if (
      fix.meaningAr &&
      typeof fix.meaningAr === "string" &&
      row.meaningAr !== fix.meaningAr
    ) {
      log.push(
        `phrasal #${fix.id} "${row.phrasalVerb}" meaningAr: "${row.meaningAr}" -> "${fix.meaningAr}"  (${fix.reason || ""})`,
      );
      row.meaningAr = fix.meaningAr;
      touched = true;
    }
  ...
  console.log(
    `TOTAL:                      ${flashApplied + synApplied + antApplied + phrApplied}`,
  );
