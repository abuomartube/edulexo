...
import { students, settings, accessCodes } from "@workspace/db/schema";
...
const SESSION_SECRET = process.env.SESSION_SECRET || "churchill-secret-key";
...
async function ensureTablesAndSeed() {
  try {
    ...
    `);
    await db.execute(sql`
      ALTER TABLE students ADD COLUMN IF NOT EXISTS expires_at TIMESTAMP WITH TIME ZONE
    `);
    await db.execute(sql`
      ALTER TABLE students ADD COLUMN IF NOT EXISTS password VARCHAR(255)
    `);
    await db.execute(sql`
      ALTER TABLE students ADD COLUMN IF NOT EXISTS access_code VARCHAR(64)
    `);
    await db.execute(sql`
    ...
    `);
    await db.execute(sql`
      ALTER TABLE comments ADD COLUMN IF NOT EXISTS name VARCHAR(255) NOT NULL DEFAULT ''
    `);
    await db.execute(sql`
    ...
    `);
    await db.execute(sql`
      CREATE INDEX IF NOT EXISTS conversation_sessions_student_idx ON conversation_sessions (student_id, created_at DESC)
    `);
    await db.execute(sql`
      INSERT INTO settings (key, value) VALUES ('student_password', 'churchill2025') ON CONFLICT (key) DO NOTHING
    `);
    await db.execute(sql`
      INSERT INTO settings (key, value) VALUES ('admin_password', 'admin2025') ON CONFLICT (key) DO NOTHING
    `);
  } catch (err) {
...
export function getStudentToken(req: { cookies?: Record<string, string> }): { id: number; email: string } | null {
  const token = req.cookies?.["churchill_student"];
...
export function getAdminToken(req: { cookies?: Record<string, string> }): boolean {
  const token = req.cookies?.["churchill_admin"];
...
router.post("/login", async (req, res) => {
  try {
    ...
    res.cookie("churchill_student", token, cookieOpts);
...
router.get("/me", async (req, res) => {
  ...
  try {
    const [student] = await db.select().from(students).where(eq(students.id, tokenData.id));
    if (!student) {
      res.clearCookie("churchill_student");
      res.json({ user: null });
      return;
    }
    res.json({ user: { id: student.id, email: student.email, status: student.status, expiresAt: student.expiresAt } });
...
router.post("/admin/login", async (req, res) => {
  try {
    ...
    res.cookie("churchill_admin", token, adminCookieOpts);
    res.json({ ok: true });
  } catch (err) {
...
router.post("/admin/logout", (_req, res) => {
  res.clearCookie("churchill_admin");
  res.json({ ok: true });
});
...
router.get("/admin/students/export/pdf", async (req, res) => {
  ...
  try {
    ...
    doc.fontSize(20).fillColor("#0A1A30").text("4 IELTS — Student Report", { align: "center" });
    doc.moveDown(0.3);
...
router.post("/feedback", async (req, res) => {
  ...
  try {
    ...
    await db.execute(sql`
      INSERT INTO feedback (student_id, email, tool, message) VALUES (${tokenData.id}, ${tokenData.email}, ${tool}, ${message.trim()})
    `);
    res.json({ ok: true });
  } catch (err) {
...
router.get("/admin/feedback", async (req, res) => {
  ...
  try {
    const rows = await db.execute(sql`SELECT * FROM feedback ORDER BY created_at DESC`);
...
router.post("/comments", async (req, res) => {
  ...
  try {
    ...
    const existing = await db.execute(sql`SELECT id FROM comments WHERE student_id = ${tokenData.id}`);
    if (existing.rows.length > 0) {
      res.status(409).json({ error: "You have already submitted a review." });
      return;
    }
    await db.execute(sql`
      INSERT INTO comments (student_id, email, name, text, rating) VALUES (${tokenData.id}, ${tokenData.email}, ${displayName}, ${text.trim()}, ${rating})
    `);
    res.json({ ok: true });
  } catch (err) {
...
router.get("/comments/approved", async (_req, res) => {
  try {
    const rows = await db.execute(sql`SELECT id, name, text, rating, created_at FROM comments WHERE status = 'approved' ORDER BY created_at DESC`);
...
router.get("/admin/comments", async (req, res) => {
  ...
  try {
    const rows = await db.execute(sql`SELECT * FROM comments ORDER BY created_at DESC`);
    res.json({ comments: rows.rows });
  } catch (err) {
...
router.post("/admin/comments/:id/status", async (req, res) => {
  ...
  try {
    ...
    await db.execute(sql`UPDATE comments SET status = ${status} WHERE id = ${id}`);
    res.json({ ok: true });
  } catch (err) {
