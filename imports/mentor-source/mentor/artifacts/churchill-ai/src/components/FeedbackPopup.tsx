...
export default function FeedbackPopup({ tool, onClose }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    setSending(true);
    try {
      await fetch(`${BASE_URL}/api/churchill/auth/feedback`, {
        method: "POST",
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.6)", backdropFilter: "blur(4px)" }}>
      <div
        ...
        style={{ background: NAVY, border: "1px solid rgba(0,180,200,0.25)", boxShadow: "0 25px 50px rgba(0,0,0,0.5)" }}
      >
        ...
        {sent ? (
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ background: `rgba(0,180,200,0.15)` }}>
