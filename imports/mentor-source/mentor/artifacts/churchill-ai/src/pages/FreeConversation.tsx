...
class FreeConvErrorBoundary extends Component<{ children: ReactNode; onBack: () => void }, { hasError: boolean; msg: string }> {
  ...
  componentDidCatch(error: unknown) {
    console.error("[Churchill AI] FreeConversation crash:", error);
  }
  render() {
      ...
      return (
        <div className="min-h-screen flex flex-col items-center justify-center px-4" style={{ background: `linear-gradient(160deg, #071422 0%, ${NAVY} 40%, #0C2040 100%)` }}>
          <div className="text-center space-y-4 max-w-sm">
            ...
            <button onClick={this.props.onBack} className="w-full py-3 rounded-2xl font-bold text-sm" style={{ background: `linear-gradient(135deg, ${TEAL}, #00D4F0)`, color: NAVY }}>
              Go Back
            </button>
          </div>
        </div>
...
function FreeConversationInner({ onBack, onLogout, expiresAt }: Props) {
  ...
  const fetchPast = useCallback(async () => {
    setPastLoading(true);
    try {
      const res = await fetch(`${BASE_URL}/api/churchill/conversation/sessions`, { credentials: "include" });
  ...
  const playNextInQueue = useCallback(async () => {
    ...
    setIsSpeaking(true);
    try {
      const res = await fetch(`${BASE_URL}/api/churchill/tts`, {
        method: "POST",
  ...
  const transcribeAndSend = useCallback(async (audio: Float32Array) => {
    ...
    try {
      ...
      const res = await fetch(`${BASE_URL}/api/churchill/whisper`, {
        method: "POST",
        body: fd,
        credentials: "include",
        signal: ctrl.signal,
  ...
  const sendUserMessage = useCallback(async (text: string) => {
    ...
    try {
      const res = await fetch(`${BASE_URL}/api/churchill/conversation/chat`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
        signal: ctrl.signal,
  ...
  const endConversation = useCallback(async () => {
    ...
    setFeedbackError(null);
    try {
      const res = await fetch(`${BASE_URL}/api/churchill/conversation/feedback`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
      ...
      // Persist the session
      try {
        await fetch(`${BASE_URL}/api/churchill/conversation/sessions`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
          body: JSON.stringify({
  ...
  const Header = ({ leftAction }: { leftAction?: () => void }) => (
    <header className="flex items-center justify-between px-4 py-3 shrink-0" style={{ borderBottom: "1px solid rgba(0,180,200,0.15)" }}>
        ...
        <main className="flex-1 flex flex-col items-center justify-center px-4 py-10 gap-8">
          ...
          <div className="w-full max-w-2xl grid sm:grid-cols-2 gap-4">
            <button
              ...
              style={{ background: `linear-gradient(160deg, rgba(0,180,200,0.18), rgba(0,180,200,0.05))`, border: `1px solid rgba(0,180,200,0.35)` }}
            >
  ...
  if (stage === "chat") {
    return (
      <div className="min-h-screen flex flex-col" style={{ background: `linear-gradient(160deg, #071422 0%, ${NAVY} 40%, #0C2040 100%)` }}>
        ...
        <footer className="shrink-0 px-4 py-3" style={{ borderTop: "1px solid rgba(0,180,200,0.15)", background: "rgba(0,0,0,0.2)" }}>
  ...
  return (
    <div className={`flex ${isUser ? "justify-end" : "justify-start"}`}>
      <div
        ...
        style={
          isUser
            ? { background: `linear-gradient(135deg, ${TEAL}, #00D4F0)`, color: NAVY, fontWeight: 600, borderBottomRightRadius: 6 }
