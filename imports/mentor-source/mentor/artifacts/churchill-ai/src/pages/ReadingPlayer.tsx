...
export default function ReadingPlayer({ level, type, slug, onBack, onLogout, expiresAt, onAllDone }: Props) {
    ...
    (async () => {
      try {
        const res = await fetch(
          `${BASE_URL}/api/churchill/reading/items?level=${level}&type=${type}`,
          { credentials: "include" },
        );
  ...
  const loadItem = useCallback(async (s: string) => {
    ...
    setAnswers({});
    try {
      const res = await fetch(`${BASE_URL}/api/churchill/reading/items/${s}`, { credentials: "include" });
      const data = await res.json();
      if (!res.ok) {
  ...
  const submit = async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`${BASE_URL}/api/churchill/reading/items/${activeSlug}/submit`, {
        method: "POST",
