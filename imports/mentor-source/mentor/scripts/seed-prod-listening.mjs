...
const bankPath = path.resolve("artifacts/api-server/src/listening/testBank.ts");
const bank = await import(pathToFileURL(bankPath).href);
const TESTS = bank.TESTS;
if (!Array.isArray(TESTS) || TESTS.length === 0) {
  console.error("Could not load TESTS array from testBank.ts");
  process.exit(1);
}
...
if (existing > 0) {
  console.log("Table is NOT empty — skipping seed (matches seedListeningTestsIfEmpty behaviour)");
  await client.end();
  process.exit(0);
}
...
for (const t of TESTS) {
  const publicQuestions = t.questions.map((q) => {
  ...
  for (const q of t.questions) {
    ...
    else if (q.type === "matching") answerKey[q.id] = { type: "matching", answer: q.answer };
    else
      answerKey[q.id] = {
        type: q.type,
        answer: q.answer,
