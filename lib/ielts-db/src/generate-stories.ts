...
const TOPICS_C1 = [
  ...
  "catastrophising and media framing",
  "language and power asymmetry",
  "collective action problems in climate governance",
  "the neuroscience of bias",
  "financial globalisation and sovereignty",
  "the epistemology of expertise",
  "posthumanism and ethics of care",
  "contested narratives of progress",
  "the limits of free speech",
  "meritocracy as ideology",
  "resilience discourse and structural neglect",
  "the social determinants of health",
  "big data and democratic legitimacy",
  "the moral economy",
  "performativity and gender identity theory",
  "extractive capitalism in the Global South",
  "intergenerational justice and climate ethics",
  "the philosophy of punishment",
  "narrative and collective memory",
  "complexity theory and governance",
  "the philosophy of science and paradigm shifts",
  "the commons and digital enclosure",
  "cosmopolitanism and refugee law",
  "the attention economy and cognition",
  "labour alienation in platform capitalism",
  "urban informality and the right to the city",
...
Return only the JSON array, no other text.`;
  ...
  if (!jsonMatch)
    throw new Error(`No JSON found in response for level ${level}`);
...
async function generateForLevel(
  ...
  for (let i = 0; i < topics.length; i += BATCH_SIZE) {
    ...
    console.log(
      `  Generating ${level} stories ${i + 1}-${Math.min(i + BATCH_SIZE, topics.length)} of ${topics.length}...`,
    );
    ...
    try {
      ...
      console.log(
        `  ✓ Inserted ${stories.length} stories (total orderIndex now ${orderIndex})`,
      );
...
async function main() {
  ...
  for (const [level, topics] of Object.entries(levelMap)) {
    ...
    console.log(
      `${level}: has ${existing} stories, generating ${Math.min(needed, topics.length)} more...`,
    );
