...
export function useAudioPlayback(workletPath: string) {
  ...
  const init = useCallback(async () => {
    ...
    const ctx = new AudioCtxClass({ sampleRate: 24000 }) as AudioContext;
  ...
  return useMemo(() => ({ state, init, pushAudio, pushSequencedAudio, signalComplete, clear }), [state, init, pushAudio, pushSequencedAudio, signalComplete, clear]);
}
