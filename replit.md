...
- **Iteration 2.5 — DONE**: Brand logos placed across the three landing pages. Master logo (`src/assets/edulexo-master-transparent.png`) headlines `/` in a dark navy/indigo gradient showcase panel above the "Two powerful courses" hero. Product logos appear large on each detail page: on `/ielts` the LEXO-for-IELTS logo (`src/assets/lexo-ielts.png`) replaces the "Coming Soon" video placeholder in the right hero column (the Vocab/Tests/AI stat tiles are kept), and on `/english` a LEXO-for-English brand panel (`src/assets/lexo-english.png`) sits above the existing flashcard preview card. The two homepage course cards keep their lucide icon chips (BookOpen / Trophy) — at 64px the marketing-poster logos read as illegible micro-text, and the brand identity is already delivered strongly by the master hero panel + the product detail pages. The `Header` keeps the small `edulexo-logo.png` AE icon. All logo panels use dark navy/teal/violet gradients matching the logos' native palette (white text on dark background).
