...
- **MENTOR Integration — Status (post-L6):** The Mentor 6-tool system was hidden from the UI in L6 but intentionally kept reachable: `/dashboard/english/:tool` (LexoTool iframe), `/lexo/tools/{speaking,writing,listening,reading,lessons}?embed=1` embed pages, and `/api/english/*` + `/api/english-mentor/*` API routes still respond. **Decision (kept hidden but live):** while the new Lexo English Dashboard is being built (see the dedicated open task), Mentor stays in this hidden-but-live state so we can selectively repackage tools (e.g. Lessons, Listening) into the new dashboard without re-implementing them. Once the new dashboard ships and the repackaging decisions are final, a follow-up cleanup pass will remove the unused Mentor pages, routes, schemas, and i18n keys. **Recovery commit (last fully-linked Mentor UI):** `9c25ef45`.
- **MENTOR Integration (Phase 1-6) — Hidden but live:** AI-powered English practice tools integrated into LEXO for English at `/lexo-english/tools`. Includes:
