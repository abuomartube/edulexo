...
- **Word Families** (`src/data/word-families.ts` + `src/components/WordFamilies.tsx`) — 30 themed categories of 10 related Oxford 3000 words each (300 words). Each category has an English title + Arabic title (RTL), a lucide icon, and a tailwind gradient. Selecting a category opens a flashcard deck of those 10 words; "All Families" returns to the grid. The flashcard component is reused for both modes.
