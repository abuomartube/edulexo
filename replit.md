...
- **IELTS Tiering:** Supports "Complete" (A2→C1) and "Advance" (B1→C1) tiers inside one unified app at `/lexo-ielts/?tier=<tier>`, with the tier persisted in `localStorage`. The legacy `/app-ielts/` URL has been retired in favor of `/lexo-ielts/`. The `Intro` tier voice/VAD conversation feature is now merged into the unified IELTS app.
- **Free Conversation (Churchill):** Ported from lexo-intro into ielts at route `/lexo-ielts/free-conversation`. VAD-based hands-free voice mode using `@ricky0123/vad-web` (CDN WASM) + Whisper STT. Gated for intro-tier students. COOP/COEP headers added to `artifacts/ielts/vite.config.ts`. Backend at `artifacts/ielts-api/src/routes/conversation.ts` (`/api-ielts/conversation/chat|feedback|sessions`), using OpenAI gpt-4o. Session transcripts stored in `introConversations` + `introMessages` (ielts-db). Auth via `verifyStudentEmail` (HMAC x-student-email/x-student-token headers).
